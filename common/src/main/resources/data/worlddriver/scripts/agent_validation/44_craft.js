...
function cleanup() {
    ...
    Agent.invoke("mc.action.runCommand", { cmd: "kill @e[type=item]" });
}

function give(item, n) {
    Agent.invoke("mc.action.runCommand", { cmd: "give @s " + item + " " + (n || 1) });
}
...
function giveAndWait(item, n) {
    give(item, n);
    for (var i = 0; i < 40 && hotbarHas(item) < (n || 1); i += 2) Agent.system.waitTicks(2);
}
...
function awaitSlot(slot, maxTicks) {
    ...
    while (waited < maxTicks) {
        var st = Agent.invoke("mc.bot.status", {});
...
ScriptTest.run("44_craft: mc.bot.craft reports missing materials, not a crash", function(t) {
    ...
    var r = Agent.invoke("mc.bot.craft", { item: "minecraft:iron_pickaxe", count: 1 });
...
if (!clientAvailable()) {
    ...
    ScriptTest.run("44_craft: 2x2 inventory craft makes a crafting table from planks", function(t) {
        ...
        var r = Agent.invoke("mc.bot.craft", { item: "minecraft:crafting_table", count: 1 });
    ...
    ScriptTest.run("44_craft: 3x3 table craft makes a wooden pickaxe (places + opens a table)", function(t) {
        ...
        var r = Agent.invoke("mc.bot.craft", { item: "minecraft:wooden_pickaxe", count: 1 });
        t.assertEqual(r.ok, true, "started");
    ...
    ScriptTest.run("44_craft: smelt raw iron into an iron ingot in a furnace", function(t) {
        ...
        var r = Agent.invoke("mc.bot.smelt", { item: "minecraft:raw_iron", count: 1 });
        t.assertEqual(r.ok, true, "started");
