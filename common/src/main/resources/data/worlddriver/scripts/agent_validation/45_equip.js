...
function cleanup() {
    ...
    Agent.invoke("mc.bot.setting", { autoEquip: false });
    Agent.system.waitTicks(4);
}

function give(item) {
    Agent.invoke("mc.action.runCommand", { cmd: "give @s " + item });
}
...
function equipUntil(profile, slot, wantNeedle) {
    ...
    for (var i = 0; i < 14; i++) {
        Agent.system.waitTicks(4);
        r = Agent.invoke("mc.bot.equip", { profile: profile });
...
if (!clientAvailable()) {
    ...
    ScriptTest.run("45_equip: equips the best armor on every slot (diamond over iron)", function(t) {
        ...
        var me = null, r = null;
        for (var i = 0; i < 16; i++) {
            r = Agent.invoke("mc.bot.equip", { profile: "best" });
            Agent.system.waitTicks(4);
            me = Agent.invoke("mc.observe.player", {});
            var a = me.armor || {};
    ...
    ScriptTest.run("45_equip: autoEquip + threshold settings round-trip", function(t) {
        ...
        var bad = Agent.invoke("mc.bot.setting", { equipDurabilityThreshold: 5 });
        t.assertTrue(Array.isArray(bad.rejected) && bad.rejected.length > 0, "out-of-range threshold rejected");
        Agent.invoke("mc.bot.setting", { autoEquip: false, equipDurabilityThreshold: 0.1 });
    });
}
