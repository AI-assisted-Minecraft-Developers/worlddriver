...
ScriptTest.run("59_proj: entity effects/uuid/id round-trip", function(t) {
    ...
    Driver.invoke("mc.action.runCommand", { cmd: "kill @e[tag=t59]" });
    ...
    var g = Driver.invoke("mc.action.runCommand",
...
ScriptTest.run("59_proj: unknown select key is rejected", function(t) {
    var threw = false, msg = "";
    try {
        Driver.query({ q: "entities", filter: { in_radius: 2 }, select: ["type", "bogus_key"] });
    } catch (e) {
        threw = true;
        msg = String(e);
    }
    ...
    threw = false;
    try {
        Driver.query({ q: "blocks", filter: { in_radius: 1 }, select: ["pos", "nope"] });
    } catch (e) { threw = true; }
...
ScriptTest.run("59_proj: blocks rows carry blockstate properties", function(t) {
    ...
    var kill = Driver.invoke("mc.action.runCommand", { cmd: "kill @e[tag=t59]" });
