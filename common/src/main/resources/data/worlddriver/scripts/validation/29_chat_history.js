...
if (!clientAvailable()) {
    ...
    ScriptTest.run("29_chat_history: history returns shape {ok, count, nextSeq, messages}",
        function(t) {
            var r = Driver.invoke("mc.client.chat.history", {});
    ...
    ScriptTest.run("29_chat_history: limit caps result", function(t) {
        var r = Driver.invoke("mc.client.chat.history", { limit: 1 });
        t.assertEqual(r.ok, true, "ok");
    ...
    ScriptTest.run("29_chat_history: sinceSeq filters out older messages",
        function(t) {
            var full = Driver.invoke("mc.client.chat.history", {});
            var nextSeq = full.nextSeq;
            // Asking for messages beyond nextSeq → empty.
            var empty = Driver.invoke("mc.client.chat.history", { sinceSeq: nextSeq + 100 });
    ...
    ScriptTest.run("29_chat_history: chat.send accepts awaitReplyMs without throwing",
        function(t) {
            ...
            var r = Driver.invoke("mc.client.chat.send",
    ...
    ScriptTest.run("29_chat_history: overlays default-true returns {ok}", function(t) {
        var r = Driver.invoke("mc.client.overlays", {});
        t.assertEqual(r.ok, true, "ok");
    ...
    ScriptTest.run("29_chat_history: overlays{tutorial:false,toasts:false} skips both",
        function(t) {
            var r = Driver.invoke("mc.client.overlays", { tutorial: false, toasts: false });
            t.assertEqual(r.ok, true, "ok");
    ...
    ScriptTest.run("29_chat_history: 3-transport parity on history shape",
        function(t) {
            var direct = Driver.invoke("mc.client.chat.history", { limit: 5 });
            var viaTcp = Driver.system.rpcRoundtrip("mc.client.chat.history", { limit: 5 });
            var viaMcp = Driver.system.mcpRoundtrip("mc.client.chat.history", { limit: 5 });
