...
if (!clientAvailable()) {
    ...
    ScriptTest.run("27_sleep: accepts no-arg form (uses default radius)",
        function(t) {
            var r = Driver.invoke("mc.bot.sleep", {});
            ...
            if (r.ok) {
                ...
                Driver.invoke("mc.bot.cancel", { process: "goto" });
            } else {
    ...
    ScriptTest.run("27_sleep: clamps oversize radius into [1,64]",
        function(t) {
            ...
            var r = Driver.invoke("mc.bot.sleep", { radius: 9999 });
            if (r.ok) {
    ...
    ScriptTest.run("27_sleep: byte-identical across in-JVM/RPC/MCP transports",
        function(t) {
            ...
            // on player position.
            var args = { radius: 8 };
            var direct = Driver.invoke("mc.bot.sleep", args);
            var viaTcp = Driver.system.rpcRoundtrip("mc.bot.sleep", args);
            var viaMcp = Driver.system.mcpRoundtrip("mc.bot.sleep", args);
