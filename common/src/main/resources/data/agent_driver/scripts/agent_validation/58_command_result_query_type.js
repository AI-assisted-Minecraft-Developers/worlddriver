...
AgentTest.run("58_cmd_result: execute-if count lands in value", function(t) {
    ...
    Agent.invoke("mc.action.runCommand", { cmd: "kill @e[tag=t58]" });
    ...
    var hit = Agent.invoke("mc.action.runCommand", { cmd: "execute if entity @e[tag=t58]" });
    ...
    var miss = Agent.invoke("mc.action.runCommand", { cmd: "execute if entity @e[tag=t58_nope]" });
...
AgentTest.run("58_query_type: entities filter.type restricts rows", function(t) {
    ...
    var kill = Agent.invoke("mc.action.runCommand", { cmd: "kill @e[tag=t58]" });
