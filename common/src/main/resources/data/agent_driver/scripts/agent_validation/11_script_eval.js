...
function call(source, timeoutMs) {
    ...
    return Agent.invoke("mc.script.eval", args);
}

AgentTest.run("11_script_eval: last expression is returned as result", function(t) {
    var out = call("1 + 1");
...
AgentTest.run("11_script_eval: object literals survive the JSON round-trip", function(t) {
...
AgentTest.run("11_script_eval: console.log is captured into log array", function(t) {
...
AgentTest.run("11_script_eval: thrown errors surface in error, not result", function(t) {
...
AgentTest.run("11_script_eval: scope is isolated between calls", function(t) {
...
AgentTest.run("11_script_eval: snippet can call Agent.invoke recursively", function(t) {
    var out = call("var v = Agent.invoke('mc.system.version', {}); v.modid");
    t.assertEqual(out.error, null);
    t.assertEqual(out.result, "agent_driver");
});

AgentTest.run("11_script_eval: Agent.system / observe helpers are wired in the eval scope", function(t) {
    var out = call("var o = Agent.system.testOrigin(); ({x:o.x, y:o.y, z:o.z})");
    t.assertEqual(out.error, null);
...
AgentTest.run("11_script_eval: tight loop is interrupted by timeout", function(t) {
...
AgentTest.run("11_script_eval: sandbox still applies inside eval", function(t) {
...
AgentTest.run("11_script_eval: reachable via WebSocket and MCP transports", function(t) {
    var args = { source: "Agent.invoke('mc.system.version', {}).modid", timeoutMs: 2000 };
    var direct = Agent.invoke("mc.script.eval", args);
    var viaTcp = Agent.system.rpcRoundtrip("mc.script.eval", args);
    var viaMcp = Agent.system.mcpRoundtrip("mc.script.eval", args);
    t.assertEqual(direct.result, "agent_driver");
    t.assertEqual(viaTcp.result, "agent_driver", "WS path must reach the evaluator");
    t.assertEqual(viaMcp.result, "agent_driver", "MCP path must reach the evaluator");
});
