 ...
 *   Driver.invoke(method, params)     // in-JVM via DriverApi.invokeJson
 *   Driver.invokeRpc(method, params)  // TCP round-trip via RpcBridge.callJson
 *
...
public final class ScriptManager {
    ...
    public int loadAll() throws IOException {
        ...
        // Driver.observe.player() and the entire Driver.bot.* sugar (tunnel/…) that
        ...
        // (Driver.invokeRpc/invokeMcp + Driver.system.rpcRoundtrip/mcpRoundtrip),
        // Driver.world.* read/snapshot/restore, and a console that prints to the
        ...
        String harnessExtras =
            ...
            "};" +
            "Driver.invokeRpc = function(method, params) {" +
            "  if (typeof __rpc === 'undefined') throw 'RPC bridge not installed';" +
            "  var json = __rpc.callJson(method, JSON.stringify(params || {}));" +
            "  return JSON.parse(json);" +
            "};" +
            "Driver.invokeMcp = function(method, params) {" +
            "  if (typeof __mcp === 'undefined') throw 'MCP bridge not installed';" +
            "  var json = __mcp.callJson(method, JSON.stringify(params || {}));" +
            "  return JSON.parse(json);" +
            "};" +
            "Driver.system.rpcRoundtrip = function(m, p) { return Driver.invokeRpc(m, p); };" +
            "Driver.system.mcpRoundtrip = function(m, p) { return Driver.invokeMcp(m, p); };" +
            "Driver.world = {" +
            "  block:    function(p) { return Driver.invoke('mc.world.block',    p); }," +
            "  snapshot: function(p) { return Driver.invoke('mc.world.snapshot', p); }," +
            "  restore:  function(p) { return Driver.invoke('mc.world.restore',  p); }" +
            "};";
