...
/**
 * Where the server-agent sim core gets its players: players that have joined the server.
 *
 * <p>Every bot player is a {@link JoinedPlayerBodies.JoinedBody}, placed through
 ...
 * players. Every gate, ladder and rehearsal topology already ran with that switch on, so the switch
 * and the fake players were deleted together; {@code docs/dev/fake-player-parity.md} keeps what the
 * fake players lacked.
 *
 * <p>Common rather than per loader because joining is vanilla. Nothing here needs a loader API,
 * which is also what lets another mod create a driven player the same way on either loader.
 */
public final class ServerAvatarBodies {
    ...
    /** @return the per-level shared player; every caller in a level gets the same player. */
    public static ServerPlayer shared(ServerLevel level) { return BODIES.shared(level); }

    /** @return a player of its own for {@code profile}, joining it if it is not in the world. */
