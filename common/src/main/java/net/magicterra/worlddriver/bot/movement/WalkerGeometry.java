 ...
 *  point of it living here: "is this bot standing on something" must have exactly ONE answer in
...
public final class WalkerGeometry {
     ...
     * bot INTO that wall and sustains horizontalCollision (the condition vanilla
     ...
     *  {@link #isOpenSurfaceWater}: standable (solid floor, player-height clearance, no hazard) and
    ...
    /** A cell a buoyant bot can swim across at the surface: water at the foot, a CLEAR
    ...
    /** Find the breakable surface obstruction (lily pad / instabreak plant) the floating bot
     *  actually OVERLAPS but the head-on pad scan in the drive missed. The player AABB is ~0.6
     *  wide (half-width 0.3), so at the surface the bot can straddle up to four XZ columns; a pad
     *  in any of them blocks horizontal motion even though it isn't the single cell directly toward
     *  the waypoint. Scans the columns spanned by the bot footprint ({@code x±0.3, z±0.3}) at the
     *  HEAD cell ({@code floor(p.y)+1} — where a pad resting on the water surface sits) and returns
     *  the one nearest the bot centre. {@code isBreakableObstruction} is instabreak-by-hand only
     ...
     * its support is whatever its bounding box happens to overlap. A bot can be grounded with a
     ...
     * for a bot flush on a full cube and the block itself for one on a shorter shape. The cells are
     * enumerated OUTWARD (vanilla's own 1e-7), so a sliver of overlap counts as the sliver it is
     * rather than being rounded away: a bot walking off a ledge really is held by 0.0004 of a block
     * for one tick, and a reading that discarded that would report "no support" about a bot vanilla
    ...
    /** How far a pillar-up has to lift the bot before the cell it jumped from can be filled: one
    ...
    /**
     * Every cell that would stop this bot from rising {@link #PILLAR_RISE} — empty when nothing does.
     *
     * <p><b>A bot is 0.6 wide, so "the cell above the head" is not one cell.</b> Asking
     * {@code blockPosition().above(2)} answers about the column {@code floor(x), floor(z)} names, and
     * a bot standing anywhere within 0.3 of a cell boundary also has to lift a corner of itself
     ...
     * because that is roughly the face a bot rising at terminal buoyancy is about to meet, and a
     ...
     *
     * @param rise how far up to sweep the bot's own box, in blocks
     */
     ...
     *  — the block below for a bot flush on a full cube, the block itself for one on a shorter
    ...
    /** What {@link #eachSoleCell} hands back: one cell of the sole row and how much of the bot's
    ...
    /**
     * Every cell of {@link #soleRowY}'s row that the bot's own bounding box overlaps, with that
     ...
     * sliver of overlap is reported as the sliver it is (a bot walking off a ledge really is held
     ...
     *  pins the bot to this block in every direction. Lethal-only, so it never blocks
     ...
     *  SURVIVABLE drop never trips the lethal pin, so sprint momentum carries the bot off
     ...
     *  stuck). The sprint brake just drops sprint there — it does NOT pin the bot (sneak),
     ...
     * the hop go (a ladder bot took that one into a lava lake), and at ring 2 it holds — sixteen
    ...
    /**
     * "where the bot is pointed" beside "where the lethal cell is", as one row.
     *
     ...
     *
     * <p>Yaw and not the velocity vector: at the moment this fires the bot is stalled by
     ...
     * {@code p.getYRot()} is the CAMERA channel; the bot is pushed along {@code
     ...
     * AvatarInput's impulse"), which exists precisely so a slewing camera does not drag the bot.
     ...
     * {@code (-4.854,66,22.577)} put the bot at {@code (-8.257,66,23.234)} twelve ticks later —
     * <b>3.47 blocks</b>, against the "~3" that the retired gate's javadoc had carried in prose. A bot that launches slowed lands short, one that launches sprinting lands
     ...
     * in a lethal column along the bearing the bot is actually driven on.
     *
     ...
     * which is precisely the hop the rung-12 ladder bot rode into a lava lake; {@code ring ||
     ...
     * direction of travel is caught at r=1.0–2.0 — and the cells behind the bot, which the ring
     ...
     * the bot — which is why the row stays: the next real occasion files its own evidence.
     *
     ...
     * bot ends up. A hop released here can still be steered into the cell it was cleared of.
     *
     ...
     *  <p>Detect the riser the bot is pressed against: take the unit step toward the node and
