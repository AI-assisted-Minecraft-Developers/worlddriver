...
if (!clientAvailable()) {
    ...
    ScriptTest.run("19_setting_survival: autoEat / autoRespawn toggles round-trip", function(t) {
        var r = Driver.invoke("mc.bot.setting", { autoEat: true, autoRespawn: true });
        ...
        var off = Driver.invoke("mc.bot.setting", { autoEat: false, autoRespawn: false });
    ...
    ScriptTest.run("19_setting_survival: pathfinder.maxNodes / maxMs accept tunable values", function(t) {
        var r = Driver.invoke("mc.bot.setting", { "pathfinder.maxNodes": 50000, "pathfinder.maxMs": 2000 });
        t.assertEqual(r.ok, true, "write must succeed");
    ...
    ScriptTest.run("19_setting_survival: smoothLook toggle + rate round-trip", function(t) {
        var r = Driver.invoke("mc.bot.setting", { smoothLook: true, smoothLookDegPerTick: 30 });
        t.assertEqual(r.ok, true, "write must succeed");
        ...
        var bad = Driver.invoke("mc.bot.setting", { smoothLookDegPerTick: 999 });
        t.assertTrue(Array.isArray(bad.rejected) && bad.rejected.length > 0,
            "out-of-range rate must be rejected");
        Driver.invoke("mc.bot.setting", { smoothLook: false, smoothLookDegPerTick: 20 });
    });

    ScriptTest.run("19_setting_survival: out-of-range autoEatFoodThreshold rejected", function(t) {
        var r = Driver.invoke("mc.bot.setting", { autoEatFoodThreshold: 99 });
