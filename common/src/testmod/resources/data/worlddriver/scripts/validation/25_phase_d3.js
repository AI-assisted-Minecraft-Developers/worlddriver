...
// allowParkour4, Driver.bot.tunnel prelude helper that wraps clearArea).
...
if (!clientAvailable()) {
    ...
    ScriptTest.run("25_phase_d3: Driver.bot.tunnel rejects missing distance",
        function(t) {
            var r = Driver.bot.tunnel({ direction: "forward" });
    ...
    ScriptTest.run("25_phase_d3: Driver.bot.tunnel rejects unknown direction",
        function(t) {
            // distance present but direction garbage. Note: 'banana' isn't a
            // known absolute or relative direction; helper rejects it.
            var r = Driver.bot.tunnel({ direction: "banana", distance: 5 });
    ...
    ScriptTest.run("25_phase_d3: Driver.bot.tunnel forwards to clearArea with computed bbox",
        function(t) {
            ...
            var r = Driver.bot.tunnel({ direction: "north", distance: 3, height: 2 });
            if (!r.ok) {
