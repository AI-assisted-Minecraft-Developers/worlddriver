...
if (!clientAvailable()) {
    ...
    ScriptTest.run("20_clearArea_modes: rejects fill + replace together", function(t) {
        var r = Driver.invoke("mc.bot.clearArea", {
            from: { x: 0, y: 64, z: 0 },
            to:   { x: 1, y: 64, z: 1 },
    ...
    ScriptTest.run("20_clearArea_modes: rejects malformed replace", function(t) {
        ...
        var msg = null;
        try {
            Driver.invoke("mc.bot.clearArea", {
                from: { x: 0, y: 64, z: 0 },
                to:   { x: 1, y: 64, z: 1 },
    ...
    ScriptTest.run("20_clearArea_modes: byte-identical results across in-JVM, RPC, MCP transports",
        function(t) {
            ...
            var direct = Driver.invoke("mc.bot.clearArea", args);
            var viaTcp = Driver.system.rpcRoundtrip("mc.bot.clearArea", args);
            var viaMcp = Driver.system.mcpRoundtrip("mc.bot.clearArea", args);
