 ...
 * of rock, and the twenty walks the ten casts spend going up it and coming back down — while what
...
final class JourneyStairwell {
     ...
     * staircase instead of a shaft: the two trips of every cast become one {@code IntentProcess} walk
     ...
     * twice, the bot walked to {@code -10,66,34} — ground level directly above the staircase at
     * z=34 — and then re-searched for a cell eight blocks below it through untouched rock, every two
     * seconds, until the walk's time budget ran out. Naming the mouth fixed the first half and the second half did
     ...
     *
     * <p><b>The flight ends on the lowest step a bot can stand on, which is not always its bottom.</b>
    ...
    /**
     * Walk the waypoints in order, best effort. A segment that falls short is not failed here — the
     ...
     * the bot left the flight on a return walk, fell through the lake's one-block crust into the
     ...
     *
     * <p>These segments cross ground the rung cut with its own pick and nothing else, so a dig here is
     * never the answer to anything. A segment that genuinely cannot get through now stalls instead,
    ...
    /**
     * How far from a waypoint still counts as having arrived. A player standing on the right cell
     * reads one cell off when its 0.6-wide box straddles the edge — the {@code cell.5.standMissed}
     * lesson — and a segment that stops a cell short has still walked the flight.
     *
     * <p><b>With {@code LEG_ARRIVED = 1.5}, the log has never distinguished whether the bot stepped
     * down the last step or not.</b> The
     * flight's last waypoint IS {@link #stairBottom}, and a bot that stops in the cell directly above
     * it is exactly 1.0 away — inside this tolerance, so the segment reports arrival and
    ...
    /**
     * The FIRST segment of the current flight that did not arrive, or null. Reset per flight.
     *
     * <p>Both ends of the flight used to read only the height the bot finished at, which says the
    ...
    /**
     * What holds a cell up, what fills it, and its head room — plus, going UP only, the cell a player
     ...
     *
     * <p>Both trips of every cast go through here rather than straight into {@link #walkTheStairs},
     * because the damage is done BETWEEN trips and the only cheap moment to notice is just before
    ...
    private static void walkTheFlight(JourneyRig rig, String tag, boolean down, Runnable then) {
        ...
        // to triangulate from `forge.carved 67/67` and the bot having walked back down. Twenty
        ...
        // mouth and the bot reaches it across open ground beside the lake — which is the segment that
        ...
        // difference between a walk that comes home and a walk that stops four cells short of a
     ...
     * the same walk {@link #landOnFloor} stages for the rehearsal — that lever exists precisely
     ...
     *
     * <p>Silent when the bot is already at or below the terminal's row: on a dry flight this is the
     * step the tolerance let it skip, and skipping it was never wrong there.
     *
     * <p><b>The first walk is not enough, and the ladder of 2026-08-25 is why.</b> It moved the bot
     ...
     * </pre>
     *
     * The terminal is standable — the row says so. The bot is balanced on the lip of the step above:
     ...
     *
     * <p><b>A second walk aimed one step further down was tried and REMOVED</b>, 2026-08-25.
     ...
     * </ul>
     *
     * <p>The reason the earlier version of this note gave for the second walk — that re-asking for the
     ...
     * ball. What the first walk actually reports is {@code end=path-consumed} with no error and the bot
     * still on the lip: <b>a path was produced and walked to its end, and the bot never arrived</b>.
     * That is TODO J47's signature, and this scene is its second witness — a dry one, with no
     * floating bot anywhere in it, which is what makes it worth more than the water case it
     * consolidates with.
     *
     * <p>So the one walk stays, and it stays because of what it RECORDS: {@code flightLastStep} says
    ...
    static void finishTheFlight(JourneyRig rig, String tag, BlockPos ends, Runnable then) {
        ...
        // TURN THE BOT BEFORE ASKING IT TO WALK. This is the whole step, and every term of it was
    ...
    /** How long a bot falling INTO the terminal gets to land before {@link #finishTheFlight} asks
     *  again. Three times the fall of one block from rest, and the walk it follows has already spent
     *  its own budget — so this cannot pass off a walk that never happened as one that did. */
    private static final int LAND_TICKS = 20;

    /** How far above the terminal row's floor a bot may rest and still count as having arrived.
     *
     *  <p>A quarter of a block: every vanilla thing a player can legitimately stand on inside a cell
     *  either sits at the floor or a full step above it, so nothing benign lands in between — while
     *  the case this separates was measured at <b>0.92</b>, a bot whose feet had entered the row
     *  and had almost the whole block left to fall. Deliberately not {@code onGround}: that flag
     *  describes the previous {@code move()} (see {@link #finishTheFlight}'s own note), and a player
     *  that has just been placed reads {@code true} while resting on nothing. */
    private static final double SETTLED_SLACK = 0.25;

    /** SAY SO WHEN IT DID NOT LAND. A walk that quietly fails leaves {@code returnedY} to report the
    ...
    /** Is the bot at the terminal, or already past it downward? The flight only ever needs to get
     *  DOWN to a row — {@link #walkHome} judges the row and not the cell — so a bot that fell past
     ...
     *  the rung's own staircase and a walk that mines its way down destroys the way back up. */
    ...
    /** How long the last step-down gets. One ordinary +(-1) step inside a flight the bot has just
     ...
     * foot on the theory that a flooded foot floats the bot one row up. <b>That theory is dead</b>,
     * and its own staging rows are what killed it: on eight of ten casts the lever found exactly ONE
     * wet cell (one cell drained, {@code [2,56,19]}) — the two cells above it were already air — and a player
     * cannot float in a cell of air, yet {@code returnedY} was 57 all eight times. The correlation
     * failed at both ends too: the return that cleared nothing at all came back at 56, the one that
     * cleared two came back at 56, and the ones that cleared one or three came back at 57. Backfill is
     * real (a live connected body of water sits one cell away, {@code drain.6/7/9 = 3,56,19 = water})
     * and it explains only why clearing did not stick — never which row the bot ended on. So this lever cannot be eaten by
    ...
    /** How long the staged step-down gets. One ordinary +(-1) step inside a flight the bot has just
     *  walked the whole of, so a walk that needs longer than this is not slow, it is blocked — and the
     *  {@code .floorLegMissed} row is the answer worth having. */
    private static final int FLOOR_LEG_TICKS = 200;

    /**
     * Where the bot actually ended the descent, and what is under it — always, for every return.
     *
     ...
     * cannot climb one, so both trips of every cast had to be scripted, and each scripted trip failed
     * in its own way. Four separate bugs, all of them the shaft's:
     *
     * <ul>
     *   <li><b>The tower drifted.</b> {@code TowerProcess} places under the bot and jumps, and where
     *       the bot lands is pinned to nothing. Measured: {@code -9,51,21} to {@code -9,54,23} in
     ...
     * not spare — going back UP, the bot jumps from a step to the one behind it, and a jump needs
     ...
     * lesson of the breach above, applied to the step that does the digging rather than to the trip
     * that discovered it.
     */
    static void digStairsDown(SceneContext ctx, JourneyRig rig, int targetY, int budget,
        ...
        // THE BOTTOM IS THE BOT'S OWN CELL, ALWAYS. `carveTheForge` hollows the alcove from
        // `blockPosition()`, so a bottom declared for a cell the bot is merely OVER puts the alcove
        ...
        // WHERE THE FLIGHT IS, not where the bot reads. A bot one row over the step it has just cut
        ...
        // bottom and waited for the bot to drop into it — which it never did, because the bot was
        ...
        // `path-consumed` with the bot half a block short of a step it has verified open is the
        // walker calling a partial path an arrival, and asking it again is the retry that changes
        // nothing — measured as eighty identical walks, twice.
        //
        ...
        // here. Every cell of both steps is still cut, so the flight the return walks use is the
        // same flight; the bot simply does not stop on the first of them.
        ...
        if (!at.equals(body))
            ...
            // so this fires whenever the bot is anywhere other than the deepest step — including the
            // case it was built for, a bot that has not moved off the course above yet, which is NOT
        ...
        cutStairCells(rig, cut, 0, () ->
            ...
            if (now.getY() >= at.getY()) {
                // The cells are open and the bot has not stepped into them yet. That is a settle,
                ...
                // same step was retried and the retry re-read `blockPosition()`, so a bot that had
     ...
     *  world tick, and three is a bot that is not going to step down at all. */
     ...
     *
     * <p>{@code stair.N.waited} says the bot is still on the step above and nothing else, and the
     ...
     * the goal cell is not standable at all, a route exists and the bot cannot walk it, or the bot
     ...
     * step exists — and the capability flags, because a goal the bot would have to BREAK its way to
    ...
    private static void cutStairCells(JourneyRig rig, List<BlockPos> cells, int i, Runnable then) {
        ...
        // Top down. The cell two above the step is the one the bot can already see; opening it
    ...
    /** Attempts per block of depth. Four, because a step is three mines and a walk, and a bot that
     ...
     *
     * <p>Placing stays OFF across it, which is the one thing this walk still has to say: everything
    ...
    /** How many times the return may try. Three: one for the walk and two for a bot that fell into
     *  the hole its own bucket left in the lake — see the recovery walk below. */
    ...
    private static void returnToTheForge(SceneContext ctx, JourneyRig rig, int floorY, String tag,
        ...
        // mould was to be standing on its floor, and `liftInPlace` broke that: a bot up a
        ...
        // bot. Sixty ticks is longer than lava takes to spread one cell, so by the time the route
    ...
    private static void walkHome(SceneContext ctx, JourneyRig rig, int floorY, String tag,
        ...
        // see — overland. Measured: the bot walked to -9,66,34, which is the surface directly ABOVE
        ...
        walkTheFlight(rig, tag, true, () -> landOnFloor(rig, tag, () -> {
            ...
            // it is buried inside `landing` today, at the end of a format string about the bot, and
            ...
            if (here.getY() > floorY + 1) {
                // A BOT THAT CANNOT WALK TO THE STAIRS IS USUALLY IN A HOLE IT DUG ITSELF.
                //
                ...
                // the pours have to stand in, and the walk turns placing off again on its way down.
                if (tries > 1) {
                    ...
                    // pathing, then the same cell. What gets a bot out of a hole is the scripted
                    // pillar the shaft already owns, and it needs placing, which is why this walk is
    ...
    /**
     * The lake this rung is working, kept for the walks that run after the approach.
     *
