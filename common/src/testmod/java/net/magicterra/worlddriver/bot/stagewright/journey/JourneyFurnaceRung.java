...
final class JourneyFurnaceRung {
    ...
    static void furnace(SceneContext ctx) {
        ...
        rig.attempting("craft a furnace from the cobblestone already held");
        int before = rig.carrying("minecraft:cobblestone");
        rig.evidence("cobblestone.before", before);

        if (before >= FURNACE_COBBLE) {
            rig.evidence("furnace.topUp", "not needed: started with " + before + " ≥ " + FURNACE_COBBLE
                    + " (this run is still a pure crafting probe)");
            craftFurnace(ctx, rig);
            return;
        }

        int need = FURNACE_COBBLE - before;
        // PHRASED AS A SHORTFALL, NOT AS A DELIVERY. The row is written BEFORE the mine runs, so
        // wording it as "topped up N blocks" would state an intent as an outcome. Measured
        // 2026-08-24, such a row sat over `furnace.topUp.after=7`: nothing had been added, and the
        // sentence describing the run's INTENT was the one a reader quoted as its OUTCOME. What it
        // achieved is recorded below, after the only code that can know it.
        rig.evidence("furnace.topUp", "short by " + need + ": started with " + before + ", below "
                + FURNACE_COBBLE + "; an earlier rung left material behind, so this run is not pure crafting");
        ...
        rig.attempting("top up cobblestone: could not mine " + need + " stone within 32 blocks at the surface");
        rig.drive(new MineProcess(List.of("minecraft:stone"), need, 32), 12_000, () -> {
            ...
            rig.evidence("furnace.topUp.mined", (after - before) + " blocks (needed " + need + ")");
            // The miner's own reason for stopping. Without it a failing run cannot say whether the
            // scan saw nothing, could not reach what it saw, or lacked the tool — three answers that
            // read identically as "completed" in journey.helm.endings.
    ...
    private static void topUpAtTheSurveyedStone(SceneContext ctx, JourneyRig rig, int shortfall) {
        ...
        rig.attempting("top up at the surveyed stone: could not walk to column " + shaft.getX() + ","
                + shaft.getZ());
        rig.drive(new IntentProcess(new Intent(new Goal.XZ(shaft.getX(), shaft.getZ(), 0))), 8_000, () -> {
            ...
            rig.evidence("furnace.stone.arrived", at.toShortString() + ", " + Math.round(away)
                    + " blocks from the column");
            if (away > 5) {
                // Do NOT fail here. The rung's contract is a furnace, and the craft below reports
                // the shortfall by name; failing on "could not walk there" would replace the real
                // bill with a walking diagnosis and lose the cobblestone count that says how short
                // it was.
                craftFurnace(ctx, rig);
                return;
            }
            ...
            rig.evidence("furnace.stone.quota", quota + " = shortfall " + shortfall + " + shaft depth "
                    + depth + " + margin 4");
    ...
    private static void craftFurnace(SceneContext ctx, JourneyRig rig) {
        WorldDriverJourneyScenes.craftKeepingTheTable(rig, "minecraft:furnace", 6_000, () -> {
            ...
            JourneyStation.reclaimTableIfLeftStanding(rig, () -> rig.reach("obtained furnace ×" + furnaces));
        });
    }
}
