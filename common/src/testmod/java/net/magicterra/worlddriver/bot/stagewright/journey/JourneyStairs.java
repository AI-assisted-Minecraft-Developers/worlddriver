...
final class JourneyStairs {
     ...
     *
     * <pre>
     * stair.3 = -8, 66, 19 → -6, 64, 19 (two steps cut at once)   ← the step is cut at y=64
     ...
     * cell ({@code cast*.stairsMend.0} reporting the support placed, against
     * {@code lava*.stairsMend.0} reporting the same cell cleared), eighteen audits and sixteen mends
     * on the south geometry. On the east arm the fill is what ended the run:
     * {@code cast6.returnStopped} reported the bot stopped at {@code -5, 64, 19} on cobblestone, i.e.
    ...
    static String tally() { return checked + " audit(s), " + mended + " step(s) mended"; }
     ...
     * {@code lava8.stairsMend.1 = -9, 56, 32 jump clearance -9, 58, 32=cobblestone → cleared}. The
     * raise the bot was standing on came out from under it and the run ended in the flooded mould:
     * {@code lava8.upStopped} reported the bot stopped at {@code -9, 58, 34} with water below it, in
     * its own cell and above its head.
     *
    ...
    static String flightCell(BlockPos c) {
        for (int s = 0; s < cells.size(); s++) {
            ...
            if (step.equals(c)) return "is shaft-stair step " + step.toShortString() + " itself";
            if (step.above().equals(c))
                return "is the head-room cell of shaft-stair step " + step.toShortString();
            if (s > 0 && step.above(2).equals(c))
                return "is the jump-clearance cell of shaft-stair step " + step.toShortString()
                        + " (the climb up passes through it)";
        }
        return null;
    }
     ...
     * 2026-08-17: {@code cast7.ramp.noFlight} refused {@code -9, 57, 32} as the head-room cell of
     * shaft-stair step {@code -9, 56, 32}, which must not be blocked, and the tower that took over
     * filled exactly {@code -9,57,32}, which the next stair walk read back as
     * {@code cast8.stairsBroken = 1/12 steps broken}. One run of two survived it because the audit's
     ...
     * from {@code 0,58,19} — which is {@code stair.7}'s own step, {@code cast8.returnStuck2#9.column =
     * 0,19} — and the next audit read {@code 2/11 steps broken: 0, 58, 19 blocked at 0, 58,
     * 19=cobblestone, 1, 57, 19 blocked at 1, 58, 19=cobblestone}. Two steps filled, the mend could
     * not reach back through them ({@code → could not clear (bot at -1, 59, 19)}), and the rung failed
     * because the bot could not walk back into the mould: it stopped at {@code -1, 59, 19} with the
     * stair bottom at {@code 2, 56, 19}.
     *
    ...
    /** How many steps the flight has, so a row can report a zero-step flight rather than fall silent. */
     ...
     * water is live for the WHOLE walk back down — the reclaim is scheduled after the descent, so every
     ...
     * {@code cast*.stairsBroken = 1 cell(s) submerged in fluid: 2,56,20} and
     * {@code cast*.returnedY=57 (bot at 2, 57, 20)}. The ninth poured at {@code 4,61,20}, the highest
     * of the run, and the runoff reached a second cell:
     *
     * <pre>
     * cast8.stairsBroken   = … 2 cell(s) submerged in fluid: 2,56,20=water(flowing level=8),
     *                        2,57,20=water(flowing level=6)
     * cast8.returnStopped#2 = segment 3/3: aimed for 2, 56, 20, stopped at -1, 59, 20, 4.24 blocks short
     *                        - the target cell: stone below, water in the cell, water above
     * </pre>
     *
     * The walk stopped four cells short of a waypoint no player can stand in, the rung read the height
     * it finished at as a failure to walk back into the mould, and the run spent its last 13158 ticks
     * in the return's unwedge —
    ...
    record StairFault(BlockPos step, BlockPos cell, boolean missingSupport, String saw) {
        String describe() {
            String how = missingSupport ? " support below "
                    : (cell.getY() - step.getY() == 2 ? " jump clearance " : " blocked at ");
     ...
     * report every step intact. That line is the one the last three rounds of this rung quoted to
        ...
        for (BlockPos step : cells)
            for (BlockPos c : List.of(step, step.above())) {
                ...
                wet.add(c.toShortString() + "=" + level.getBlockState(c).getBlock()
                        + (fs.isSource() ? "(source)" : "(flowing level=" + fs.getAmount() + ")"));
            }
        if (wet.isEmpty()) return "";
        return "; " + wet.size() + " cell(s) submerged in fluid (" + sources + " of them source blocks): "
     ...
     *
     * <p>THE WORD "intact" IS PART OF THE RESULT, not decoration. {@link #flooding} has printed the
     * wet cells since 2026-08-15 and the 2026-08-24 ladder still died quoting a report that called all
     * 11 steps intact and then listed two submerged cells, {@code 2,56,20=water} and
     * {@code 2,57,20=water} — the clause was right there, beside a word that says the staircase is
     * fine. Eighteen audits over eleven minutes read that line and mended nothing, because nothing
     * was reported broken.
     *
     * <p>So a flooded run no longer gets to say "intact". It still does not get a FAULT: a fault is an
    ...
    static String report(ServerLevel level) {
        if (cells.isEmpty()) return "no staircase dug yet";
        List<StairFault> faults = faults(level);
        String wet = flooding(level);
        if (faults.isEmpty())
            return cells.size() + (wet.isEmpty() ? " steps, all intact"
                    : " steps, none missing - but present is not the same as walkable") + wet;
        StringBuilder sb = new StringBuilder(faults.size() + "/" + cells.size() + " steps broken: ");
    ...
    static void mend(JourneyRig rig, String tag, List<StairFault> faults, int i, Runnable then) {
        ...
        rig.settle(new IntentProcess(new Intent(new Goal.Near(f.step(), 2), List.of(),
            ...
            if (reach > MEND_REACH) {
                rig.evidence(tag + ".stairsMend." + i, f.describe() + " → out of reach (bot at "
                        + body.toShortString() + ", " + Math.round(reach) + " blocks away)");
            ...
            if (f.missingSupport()) {
                ...
                rig.evidence(tag + ".stairsMend." + i, f.describe() + " → "
                        + (put ? "support placed"
                                : (held ? "could not place (no solid face to click against)"
                                        : "no cobblestone in hand"))
                        + " (bot at " + body.toShortString() + ", cell is now "
            ...
            rig.mineCellOrGiveUp(f.cell(), 300, () -> {
                ...
                mended += open ? 1 : 0;
                rig.evidence(tag + ".stairsMend." + i, f.describe() + " → "
                        + (open ? "cleared" : "could not clear") + " (bot at "
    ...
    static void aboutToWalk(JourneyRig rig, String tag) {
        ...
        rig.evidence("stairs.sabotage", gone.toShortString()
                + " removed (rehearsal only, so that the stair audit has to find it)"
                + " - the next stair walk, " + tag + ", should report this step and fill it back in");
    }
