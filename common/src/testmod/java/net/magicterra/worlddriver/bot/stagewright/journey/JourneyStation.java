 ...
 * free supported cell beside the bot to place it in</b>.
 *
...
final class JourneyStation {
     ...
     * census and the bot position separate "the craft walked the bot out of its own radius" from
            ...
            if (onGround > 0) {
                rig.collectByHand("minecraft:crafting_table", JourneyRig.MAX_PICKUP_LEGS,
                        "craftingTable.lost", () -> {
                            ...
                            // The last pickup walk ends where the drop was, and a drop that sank
                            // ends it in water: measured on wd.journeyCraftStepsAsideForRoom, two
                            // walks came up 2-3 blocks short of a table sinking past y=217 and the
                            // third left the bot afloat, which the next craft cannot place from
                            // and the scene's end check rejects. Step back onto ground with room
                            // first; a bot already there is handed on untouched.
     ...
     *
     * <p>Recording position, drop count and bot position at the moment of the break separates them
    ...
    /**
     * Get the bot somewhere a station can actually be placed.
     *
     ...
     * climbs: {@code JourneyShaft.climbOut} towers up a one-wide pillar inside the shaft it dug, so the bot ends
     ...
     * the surface the shaft was sunk from is right there, so one short walk normally reaches it, and
     * a bot that cannot find ground in three tries has a finding worth reporting rather than a
     ...
     * before the walk instead of after it. Being stricter than the placer can only send the bot to
    ...
    /** A cell the bot could stand in that ALSO has somewhere to put a station. Searched outward, so
     *  the nearest one wins; a swamp puts the bot in water where every neighbouring support is more
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                for (int dy = -3; dy <= 3; dy++) {
                    ...
                    // would walk a bot down to it and craft there. A real bot floats; the planner no
    ...
    private static void makeRoomForAStation(JourneyRig rig, Runnable then, int left) {
        ...
        if (left <= 0) {
            ...
            // walk ends where it began, which is what `station.noGround` recorded three times), and
            ...
            // report its own error, and this line is what says the bot never had anywhere to begin.
        ...
        // moved from one bad cell to another. In a swamp that is the common case — the bot is in
    ...
    /** What the bot is standing in and on, for the rows that record a place it could not leave.
     *
     *  <p>Four readings and not one: {@code onGround} answers a different question from "the block
     *  underfoot is solid" (it reports the last {@code move()}, so it lies in both directions on the tick a bot leaves
     *  or meets the floor), and a bot afloat reads {@code onGround=false} with a perfectly solid
    ...
    /** How many short walks the bot gets to find ground a station can stand on. */
     ...
     *  a pillar, short enough that each attempt is a walk rather than an expedition. */
