...
public final class JourneyFill {
    ...
    private static void spendTheBucket(SceneContext ctx, JourneyRig rig, BlockPos aim,
        ...
        //
        // Rung 12's own fill (scoopWater, further down this file) died exactly this way on 2026-08-22
     ...
     * this class already owns — {@link #visibleSourceNear} picks the target, {@link #standToScoop}
     * picks the seat, {@link #FILL_RESEARCH} tunes both. It sat in {@code JourneyPortalRung} only
     * because that is where the ladder first needed it, and that file has since grown to within
     * seventeen lines of the 3000-line source budget, so the next evidence row this rung needs has
     * nowhere to go. Moving the water scoop next to the water-scoop helpers buys that room back
     * without shaving a single comment.
     */
    static void scoopWater(SceneContext ctx, JourneyRig rig, BlockPos water, Runnable then) {
    ...
    private static void scoopWater(SceneContext ctx, JourneyRig rig, BlockPos water, int reseats,
        ...
        rig.settle(new IntentProcess(new Intent(approach)), 2_000, () -> {
            ...
            if (clear == null && reseats > 0) {
                ...
                BlockPos seat = standToScoop(rig, pool, why);
                BlockPos here = rig.player().blockPosition();
