 ...
 * {@code blocksMotion()}. A probe that answered "is it solid" by a different predicate than the scenes
...
final class JourneyCorridorProbe {
     ...
     *
     * Diagnosing the corridor one walk per run does not converge: a walk fails, its result message
     * says "this segment was not reached", and the next round guesses at that walk. But
     * {@code FORTRESS_WAYPOINTS} are bot positions recorded from a run that BRIDGED its way across,
     * so an unknown number of them are cells that run <b>created</b> — and in a fresh world those
     * columns are open air. A walk aimed at one of them cannot succeed no matter what the walker
     * does, and it will keep reporting a walking failure because "could not reach it" is the only
     * thing a walk knows how to say.
     *
     * <p>Measured on wp4 ({@code 63,41,87}): the walker declared {@code arrived} at {@code 63,42,86}
     * with support cells {@code 63,42,86=air 63,42,87=air} — inside the two-block goal sphere, in
     * mid-air, over a shaft — and the bot then fell fourteen blocks. Four runs read that as a pathfinding
    ...
    static void auditWaypoints(JourneyRig rig, int[][] waypoints) {
        ...
        for (int i = 0; i < waypoints.length; i++) {
            ...
            sb.append("\n wp").append(i + 1).append(' ').append(cell.toShortString())
              .append(solidHere ? " cell is solid (a floor cell; the bot stands on top of it)"
                      : canStand ? " cell is empty with solid ground below (a foot cell; standable)"
                      : " **cell is empty and so is the cell below**: this cell does not exist in a fresh world")
              .append(", nearest solid face down this column ")
              .append(floor == Integer.MIN_VALUE ? "none within " + COLUMN_LOOK + " blocks (a shaft)" : "y=" + floor)
              .append(floor == Integer.MIN_VALUE ? "" : " (" + (cell.getY() - floor) + " blocks down, "
                      // NAMED, and asked about lava, because "there is a face 20 blocks down" does not say whether a
                      ...
                      + (lavaAt(level, cell.getX(), floor, cell.getZ()) ? ", **and it is a lava surface**" : "")
                      + ")");
            // ONLY FOR THE BROKEN ONES, and only because the row above is a NEGATIVE. "Cell empty
            // and the cell below empty too" says this COLUMN has no floor; it does not say no bot
            // could stand near here. "No pathfinding change can ever reach it" is a claim about the
            // neighbourhood, so the neighbourhood has to be looked at. This turns the negative into
            // the input a re-bake needs: the cell to aim at instead.
            if (!solidHere && !canStand) sb.append(nearestStandable(level, cell));
        }
        // COUNTED, NOT SPELLED. A hard-coded count in an evidence row is a snapshot of what its
        // author believed and goes stale when the table changes (a spelled-out "eighteen" once sat
        // above seventeen rows for six days after `wp4` was deleted); the array is the measurement.
        rig.evidence("fortress.waypointAudit", waypoints.length + " baked waypoints as they look in a"
                + " **fresh world**: " + hollow + " have an empty cell, of which " + unstandable
                + " are **also empty below**. The waypoint table was recorded from a run that"
                + " [built bridges], so the cells that are empty with nothing below were stone that"
                + " run placed itself and do not exist in this world. Walks aimed at them cannot"
                + " arrive however pathfinding is changed, and they will only report \"could not"
                + " reach it\". **Read this table before changing the mechanism.** (The table mixes"
                + " foot cells and floor cells, so all three readings are given without classifying"
                + " them for the reader)" + sb);
    }
     ...
     * not pedantry — a dx/dz-bounded search has already reported "the nearest lava is 77 blocks
     * down" in this suite while a pool sat 36 away, because the bound was mistaken for the
     * measurement. Here the two are reconciled by an arithmetic fact worth stating rather than
     * trusting: <b>when the winner lands at distance {@code d ≤ STAND_LOOK}, the sphere of radius
     * {@code d} is entirely inside the scanned cube, so nothing nearer can have been missed and
     * "nearest" is literally true.</b> Only when {@code d > STAND_LOOK} — possible, the cube's
     ...
     * a "not found" from a search that did. It is deliberately not a count of blocks read — the loop
    ...
    private static String nearestStandable(ServerLevel level, BlockPos want) {
        ...
        if (best == null)
            return "\n      └ nearest standable cell: **none in the cube of half-width " + STAND_LOOK
                    + "** (search region " + scanned + " cells); this means \"none within this"
                    + " radius\", not \"none\"";
        double d = Math.round(Math.sqrt(bestSq) * 10) / 10.0;
        boolean floorLava = lavaAt(level, best.getX(), best.getY() - 1, best.getZ());
        return "\n      └ nearest standable cell " + best.toShortString() + ", true 3D distance " + d
                + " blocks, standing on "
                + name(level, best.getX(), best.getY() - 1, best.getZ())
                + (floorLava ? ", **right next to lava** (standable does not mean survivable)" : "")
                + " (search region " + scanned + " cells; "
                + (d <= STAND_LOOK
                        ? "the sphere of radius " + d + " lies entirely inside the scanned cube, so this"
                          + " really is the nearest cell"
                        : "⚠️ the distance exceeds the cube half-width " + STAND_LOOK + ", so this is"
                          + " only \"the nearest among the scanned cells\"; a nearer one may exist"
                          + " outside the cube") + ")";
    }
    ...
    static void record(JourneyRig rig, String what, BlockPos from, int[][] waypoints, int i, int legs) {
        ...
        rig.evidence(what + ".probe.box", "x " + x0 + ".." + x1 + ", z " + z0 + ".." + z1
                + "; both maps are cut around the surveyed line y=" + ref + " (" + ABOVE_BAND
                + " blocks above, " + BELOW_BAND + " below, hard bounds " + Y_LO + ".." + Y_HI
                + "). **Not the topmost face of each column**: the column at 72,42,85 on segment 5"
                + " tops out at y=67 while the route passes through a cave beneath that massif, so a"
                + " map of topmost faces would say the route does not exist."
                + " Took " + ms + " ms"
                + (clipped ? "; ⚠️ this box is wider than " + MAX_COLS + ", so the far edge was clipped"
                        + " (no strided sampling: a strided map would step over a one-block gap, which"
                        + " is exactly what it is looking for)" : "")
                + ". ⚠️ This scan generates chunks: tick counts of a run with the probe and one without"
                + " are not comparable, and a later tick change is the probe, not a regression");
        rig.evidence(what + ".probe.stand", "highest standable face per column (. = no foot cell in the"
                + " y band for this column, ~ = the standing face is lava, " + legend() + "): " + stand);
        rig.evidence(what + ".probe.head", "clearance above the standing face (. = no foot cell,"
                + " 0/1 = too low for the bot to pass, 2..9 = enough, + = more than 9). **A bridging"
                + " edge needs " + BODY_HEIGHT + " blocks of clearance**, so a field of 0/1 is a roofed"
                + " tunnel; on such terrain expanded=100000 is not a budget shortfall, there are simply"
                + " no edges to expand: " + head);
    }
    ...
    private static String legend() {
        return "0-9 = y40-49, a-z = y50-75, < = below 40, > = above 75";
    }
