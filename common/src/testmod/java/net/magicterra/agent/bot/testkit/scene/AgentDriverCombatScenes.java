...
/**
 * Dogfooded agent-driver scenes — <b>P4b wave 5, the CombatSense family</b>: the 2
 * {@code AgentGameTestCombatSense} sensing tests for the dist-neutral {@link ThreatScanner}
 * core (gap #55), migrated verbatim to testkit {@code ad.*} scenes and their legacy twin
 ...
 * survival world, doDaylightCycle default; neither {@code TestkitHarness} nor
 ...
 */
public final class AgentDriverCombatScenes implements SceneProvider {

    @Override
    public List<Scene> scenes() {
        return List.of(
                Scene.of("ad.threatScanZombie", 200, AgentDriverCombatScenes::threatScanZombie),
                Scene.of("ad.threatScanHurtAttacker", 200, AgentDriverCombatScenes::threatScanHurtAttacker));
    }
    ...
    private static void threatScanZombie(SceneContext ctx) {
        ...
        ctx.await(() -> !level.getEntitiesOfClass(Zombie.class, entityBox(cx, floorY, cz)).isEmpty())
            ...
            AgentDriverCommon.LOG.info("[ad.threatScanZombie] threats={} seen={} visibleAtTick={}",
    ...
    private static void threatScanHurtAttacker(SceneContext ctx) {
        ...
        // ENTITY-VISIBILITY WAIT — see ad.threatScanZombie for the rationale (a freshly-added mob is
        ...
        ctx.await(() -> !level.getEntitiesOfClass(Wolf.class, entityBox(cx, floorY, cz)).isEmpty())
            ...
            AgentDriverCommon.LOG.info("[ad.threatScanHurtAttacker] threats={} found={} top={}",
