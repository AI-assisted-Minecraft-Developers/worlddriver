...
| ~~Q2~~ | 两条逃生反射的 `forward` 升级成 `commandMove` | ✅ **已落，`01275851`**。`ContactDamageEscape:111` / `LavaProximityEscape:105` 都是 `BotInput.driveForward(mc)` |
| ~~Q3~~ | prelude 脚基判据改眼→格心，抽进 `BotUtil` | ✅ **已落，`e48f7ee5`**。`WalkerTickPrelude:105,296` 是 `eyeWithin(p, wk.stickyDig.pos, blockReachToCentre(p))`，`BotUtil` 里五站点表也在 |
...
`bot/movement/BotInput.java:117` 就有，`10e89fda` 今天刚加的。
...
| hash | 一句话 | 节 | 状态 |
|---|---|---|---|
| `eb0385c4` | 背包枚举只写一遍，传输层两侧不可能再分叉 | K1 | **待编译** |
| `93a502c7` | 「朝向眼睛的那个面」搬进 processes 够得着的那个家 | K2 | **待编译** |
| `8892c3cb` | 工作台/熔炉的「找」也合并，「放」那半边晚了两个 gap 才合 | K4 | **待编译** |
...
`KeyMapping` / `MultiPlayerGameMode`，专用服一个都没有；`2989dc11` 那次弄红双 loader 闸
...
是真的，指回去会重演 `2989dc11`），**但「已经验过了」这个结论当时不成立** ——
...
| hash | 一句话 | 节 | 验证状态 |
|---|---|---|---|
| `e08921a2` | 逃生反射改走 walker 会覆盖的冲量通道，不走共享键位 | A-1/A-2/A-3 | 闸绿 |
| `01275851` | 伤害反射真正压过它一直号称能压过的 walker 命令 | A-3 | 闸绿 |
| `2989dc11` | 破坏管线开到只按键的两条反射上 | J1 | **弄红过闸**，已由 coordinator 修 |
| `e3201836` | 给「站位格的假想眼」一个定义，并写下五个不一致的触及半径 | B-1 | 闸绿 |
| `e48f7ee5` | 每条触及判据改成眼→格心 vs 游戏给的 range | J2 | 闸绿 |
| `ec518227` | 竞技场基线必须申报它悄悄改写的每个默认 ON 开关 | J6 | 6/6 |
| `ef9c13a3` | 只按 attack 键不驱动破坏管线的位点直接编译失败 | J1-verify | 3/3 |
| `1c15b6a1` | 那条守卫的失败信息不再推荐弄红闸的写法 | J1-verify | 3/3 |
| `769ee562` | 钉住 scheduler 链能调的客户端类 | J7 | 4/4 |
| `8d1d3bf5` | 删掉九个进程从没调过的 `BotInput` import | A-4 | 闸绿 |
| `34912b8a` `aeac5aef` `b13c7b97` `60482a97` `c27813d6` | 死 import 724 → 0，纯删除 | J5 | 末笔编译过 |
| `4f97a7d8` | 跨枚举器断言从 boolean 扩到全部字段 | J8 | 6/6 |
| `dbac93bc` | 说清 `pathLen`/`pathStep` 到底装什么（十写入者五单位） | J9 | 仅注释 |
| `b8248409` | `dayTime` 折叠收敛到一处 + `DayTimeFoldingTest` | J10/F12 | **过编译器** |
| `3e1cc555` | 三份 `yawFor` 合一，并写明为什么不是 `toYRot()` | J10 | **过编译器** |
| `3481c67e` | 改正黄昏相位那段注释：四条边界只有一条承重 | F13 | 仅注释 |
| `3795ad94` | recipe 的 station 只留 resolver 那一份 | J10 | **过编译器** |
| `b1069192` | `clearColumn` 提到 `Move`，三个对角线继承 | J10 | **过编译器** |
| `d5122553` | 下坠柱净空只问 `Move` 一次 | J10 | **过编译器**（唯一碰 A* 热路径的一笔） |
...
同一次还带了 coordinator 三笔 journey 修复（`0692b5fa` / `c16a70fa` / `fa256dc6`），
**是两边改动的合并验证，不是只验了一边。**
我把这批的风险自定级在编译期（见 §J10 与下面的等价性论证），**该风险已清零**；
剩下的运行期风险只有 `b8248409` 在 `dayTime < 0` 时的行为变化，其余逐位相同。
...
**这条断言在 `2989dc11` 之前会红，点名那两个文件。** 现在绿。
...
| 提交 | 范围 | 删除行 |
|---|---|---|
| `34912b8a` | `bot/movement/` 的 7 个 `WalkerTick*` + `ClutchController` | 157 |
| `aeac5aef` | `bot/process/` 前 8 个 | 291 |
| `b13c7b97` | `bot/process/` 余下 10 个 | 219 |
| `60482a97` | `api/` `debug/` `auto/` `scheduler/` `BotApiImpl` `ClientWorldView` 等 9 个 | 20 |
| `c27813d6` | `Walker.java` / `WalkerTickClimb` / `WalkerTickPrelude`（coordinator 收工后） | 53 |
...
### J9 `ProcessSlot.pathLen` / `pathStep` —— 一个字段，十个写入者，**五种单位**（`dbac93bc`）
...
| 提交 | 概念 | 原来几份 | 判据 |
|---|---|---|---|
| `b8248409` | `dayTime` → 相位 | **4**（`ObserveApi` / `ClientObserve` / `ClientEventDetector` / `WorldModel`） | **不一致**：两份防负数、两份不防。见 F12 |
| `3e1cc555` | `yawFor(Direction)` | 3（`BunkerProcess` / `DescendProcess` / `EscapeProcess`） | **逐字相同** → 无条件可合 |
| `3795ad94` | recipe → station | 2（`RecipeApi` / `RecipeResolver`） | **逐字相同**（只差修饰符），且 `RecipeApi` 已 import 对方并在注释里称对方是 single source of truth |
| `b1069192` | `clearColumn`（对角线的两个角柱） | 3（`Diagonal` / `DiagonalAscend` / `DiagonalDescend`） | **逐字相同**；三者都是 `Move` 的子类，**调用点一个字符没改** |
| `d5122553` | `clearFallColumn`（下坠柱全程净空） | 3（`Fall` / `FallIntoWater` / `WaterBucketFall`） | **逐字相同**（只差局部变量名 `f`/`h` 与 `foot`/`head`）；三者只在**落点**判据上不同，那部分留在各自文件里 |
...
### J7 字节码守卫 —— `SchedulerClientCallSurfaceTest`（`769ee562`）
...
我做不出 `2989dc11` 的字节码（不能编译，`run 10` 在跑）。但有更好的：
...
本轮删掉的那 9 个 `BotInput` import（`8d1d3bf5`）是这块粘贴的一角。
...
裹进了他的提交。已 `reset --soft` 拆开重提（我的那批成了 `c27813d6`）。
...
**先查了这是不是故意的 —— 一半是。** `6e02a98e`（2026-08-19）的 CHANGELOG 逐字写着：
...
3. **我那条 `ClientBreakSitePairingTest` 抓不到它，而且更糟：它在教下一个人犯同一个错。**
   ...
   已改（`ef9c13a3` 之后的一笔）：改成指向 `BotInteract.continueDestroy`，
...
- **`Parkour2` / `Parkour2Diagonal` 不用四参数 `hasRunway` 是故意的，理由成立**
  （`6e02a98e` 的 CHANGELOG 逐字：2 格在起跳范围内，
