...
> **状态（2026-08-23 更新）：§4 已建成实物**（`WorldDriverDrownRiseScenes`，三条集成拓扑场景
> + 竖直支的每 tick 仪器）。§5 仍不存在。
>
> ⚠️ **在读下面这句「一句话结论」之前先读 §4**：写完 §0–§3.5 之后又死了一具身体，
> **不是同一种死法**——那一具在敞口水面上有升力却一格没升，**根本没进破盖支**。
> 下面这句话是对 §1.5 那起含水空腔事故说的，**§3 因此继续停用**（理由见 §4.2）。
>
> **一句话结论（仅限 §1.5 那起）**：这条反射一边喊救命一边按住跳，而 `!onGround()` 是
...
## §4 场景（2026-08-23 建成，不再是设计）

写下 §0–§3.5 之后又死了一具身体，**而且不是同一种死法**。先把两起事故分开，否则 §4 会被写成
只覆盖第一起：

| | §1.5 那一起 | 2026-08-23 这一起 |
|---|---|---|
| 处境 | y=40 含水空腔，头顶 4 格内无空气 | 沼泽敞口水面，头顶**看不到**固体 |
| 走的支 | 竖直 + 破盖 | 竖直，`lidBlocksRise=false`，**没破** |
| 病因 | 按住跳 ⇒ `!onGround()` ⇒ ÷5，**挖不完** | 有升力却**一格没升**（见下） |
| 修法 | §3 的 RISE/DIG 拆相 | **不是 §3**——见 §4.2 |

### 4.1 运动学把第二起的候选淘汰到一个

「既没升也没沉」本身就是判别子，不需要新仪器就能排掉两个候选：

- 若跳从未进物理 ⇒ 身体该以尸体那个速率下沉（实测 **−0.02/tick**），261 tick 后 y≈56。
  实测两次心跳都在 **61** ⇒ 排除。
- 若跳进了物理且上方无阻 ⇒ 净升力为正 ⇒ 出水 ⇒ air 回补。实测 air **单调降到死** ⇒ 排除。
- ⇒ **有升力、被一个碰撞面钉住**。61.159 + 身高 1.8 = 顶面 **62.959**。

而这个类里的三处扫描——`cappedColumn`、`lid = blockPosition().above(2)`、`nearestBreathable`
——**全部只看 `blockPosition()` 命名的那一柱**。身体 x=−27.716 时 AABB 左沿在 **−28.016**，
有 0.016 格在**下一柱**里，那一柱谁都没问过。

### 4.2 所以 §3 继续停用，理由比原来更硬

§3 治的是「挖得慢 5 倍」。第二起**根本没进破盖支**，因为扫描报告头顶是通的。
先落 §3 只会换一种死法。**修法候选改为**：把「能不能升」这个问题从**列扫描**换成
**身体自己的 AABB 上移一点会不会碰**（`level.noCollision(p, box.move(0, ε, 0))`），
它对邻柱、半砖、楼梯、睡莲一并正确，且不需要任何列扫描。

### 4.3 三条场景（`WorldDriverDrownRiseScenes`）

`DrownEscapeChain.tick` 第一行 `if (mc == null) return;` ⇒ **专用服上这条覆盖率不是弱，是零**。
所以三条都在集成拓扑上执行，在别处响亮 SKIP 并把这句话写进 reason。

| 场景 | 布景 | 判据 |
|---|---|---|
| `wd.drownEscapeClientRisesInOpenWater` | 敞口水柱，头顶三格空气 | 头必须出水 |
| `wd.drownEscapeClientPinnedByNeighbourColumn` | 身体 x 贴着格界（AABB 跨两柱），实心块放在**扫描不看的那一柱** | 头必须出水（今天预期红） |
| `wd.drownEscapeClientStaysDownDisarmed` | 同敞口水柱 | 头**不许**出水 |

第三条为什么必须同时关 `autoDrownEscape` **和** `autoFloatWhenDrowning`：后者是
`AutoSwim.drowningSentinel`，它对 **idle** 的身体按住跳，而停在测试水柱里的身体正是 idle。
只关前者，这一臂会被哨兵浮上去而失败，且失败时描述的是错的主体。

第二条建成 **optional**（`wd.vineOverWaterClimb` 的先例）：它现在是传感器不是判词。
**修法落地那一笔里必须把它提成 required**，否则就是一条谁都不看的红。
...
按第一版改的那一笔（`239be43b`）把参数从 `LocalPlayer` 放宽成 `Player`，
...
`6d4c61dc` 之后三闸，全部**读结果文件**判的，不是读退出码：
