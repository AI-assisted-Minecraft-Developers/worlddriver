...
WorldDriver drives a body through a live world. That body is sometimes the player of a
...
behaviour, where a driven body matches a player connected over a network and where it does
not.

It exists because assuming parity that is not there has produced bugs repeatedly, and
because a scene that passes on one kind of body proves nothing about another. A reading
taken on a headless body is a reading about that body.

**This document is maintained alongside the code.** A change that alters what a body does
...
exposes locomotion and the look. What only a body with an inventory has is behind two
...
| Body | Class | Underlying entity | In `level.players()` |
|---|---|---|---|
| Client player body | `bot/body/ClientPlayerBody.java` | the client's own `LocalPlayer` | yes, as the person's player |
| Joined body | `bot/sim/ServerPlayerBody.java` over `bot/sim/JoinedPlayerBodies.JoinedBody` | a `ServerPlayer` the server joined to itself | yes |
| Adopted body | `bot/sim/ServerPlayerBody.java` over a client's own `ServerPlayer` | the server-side half of a connected player | yes |
| Mob body | `bot/stagewright/LivingBody.java` (test sources) | a `DrivenPiglin`, a mob | no — it is not a player |

**The client player body** writes the player's own `AvatarInput` rather than the shared
...
**The joined body** is the one this document is mostly about. `ServerAvatarBodies` hands
...
that awards advancements, and fired the loader's login event on the way in. A body a
...
**The adopted body** is the same `ServerPlayerBody` wrapped around a `ServerPlayer` that a
...
assumption of the form "this body's connection discards whatever is sent through it" is
false on it.
`ServerPlayerBody.step()` refuses to pump such a body with an `IllegalStateException`: its
own connection already ticks it, and pumping would tick it twice.

**How a body is addressed.** Every `mc.bot.*` verb takes an optional `body` parameter.
...
`mc.bot.*` code that runs, and `mc.bot.status` — bound straight to the body route — answers
there with no client attached. Bodies are registered by whoever creates them:
...
**The mob body** answers a different question. It is not a player and is not trying to be
one; what it tests is whether the same walker and the same pathfinder can carry a
non-player entity across terrain a player body crosses. It has no hands and no menus, and
the planner is kept from pricing digs and placements by the world view rather than by a
flag: over a body that is not a player, every break is priced as impossible and no block
counts as placeable.

There are no fake players left. NeoForge's `FakePlayer`, a Fabric copy of it, and the
system property that chose between them and joined bodies were deleted together once every
gate and every ladder topology already ran on joined bodies. The name of this document is
older than that decision; it survives because the question it answers did not change, only
the body it asks about. A comment or a scene that still speaks of a factory body or a fake
player is describing what was replaced.

## Which body a scene runs on

The rule divides by *topology*, not by what a scene is testing, and it is enforced in one
place: `SceneBody` in the test sources, which every scene goes through to obtain a body.

- **Dedicated server.** Headless joined bodies. There is no client, so there is nothing
  else to drive.
- **Integrated server with the client half of the driver in the same process.** The
  ...
  headless body beside a person's player is neither, and `SceneBody` refuses it.
- **Dedicated server with a separate client process.** Headless joined bodies again. The
  client is in the other process and a running process object cannot cross a socket.
- **Mob bodies are legal on every topology.** The rule is about headless *player* bodies on
...
**Joining rather than copying.** The earlier server body was a `ServerPlayer` that had
...
`ServerLevel.players()` so the level keeps ticking and mob AI can see the body, `ChunkMap`
...
player; the client does that and sends the result. A body that is its own client has to do
it, so `JoinedBody.aiStep` carries the crouch decision and its speed scale, the scale while
an item is in use, the push out of a block a corner of the body is inside, the rules that
...
executed scenes, every body-minting scene reporting the same null dereference.

## The boundary table

In the tables, *a driven body* means the server-side body: the joined body, or the adopted
one where a row says so. The client player body is named explicitly wherever it behaves
differently, and the mob body has a group of its own at the end.
...
them sets it back explicitly. So a row whose evidence is a scene says what the body does
when the pathfinder may not change the world, and the same body under default settings has
...
| Status | Meaning |
|---|---|
| `Parity` | The behaviour is vanilla's own code on the driven body, not a copy of it. |
...
assertions at all, which drives a body through each quantity the table makes a claim about
and records what the body actually reports. It cannot go red, deliberately — an assertion
...
Divergence here comes from one root: the body produces its own movement instead of
...
| Behaviour | A connected player | A driven body | Status |
|---|---|---|---|
...
| Minor horizontal collision | the client's eight-degree test decides whether a glancing wall stops a sprint | reimplemented on the joined body, because `Entity`'s answer on a server is always false | `Approximate` |
| Starting a sprint | a key press, with vanilla's own start rules | the driver sets the sprint flag directly, on both player bodies | `Approximate` |
...
| Position authority | the client proposes a position and the server accepts or corrects it | the body's position is produced on the server, so there is no proposal to check and nothing to correct | `Accepted` — a driven body is never rubber-banded |
...
| Tick cadence | one tick per server tick, driven by the connection | one tick per *step*, driven by whatever advances the body | `Accepted` — see below |

The cadence row is a contract, not a defect. A body no driver steps does not move, and the
level's entity loop must not move one on a schedule of its own; a scene that steps a body
hundreds of times inside a single server tick gets a body that advances hundreds of ticks.
...
The driven body has no camera and no keyboard. What a person's input passes through on its
...
| Jump | a held key; vanilla's gate decides which presses become impulses | `commandJump` is an ask; the same vanilla gate decides. The server body releases the ask after one step unless it is in water, where a held ask keeps a swim rising | `Parity` for the gate, `Accepted` for the release policy |
| Look | a mouse, with the camera following | the client body slews the camera and can be asked to snap; the server body assigns yaw and pitch outright | `Accepted` — there is no camera to move server-side |
| What the body is looking at | the client's crosshair hit result | the server body raycasts from the eye along its yaw and pitch, out to 4.5 blocks, against block outlines and ignoring fluids | `Approximate` |
...
headless body is *more* permitted than a player, not less. The correct repair is therefore
to make the server body honest, not to give the client body more power.

| Behaviour | A connected player | A driven body | Status |
|---|---|---|---|
| Breaking a block | the server's game mode applies the tool requirement, Silk Touch, Fortune, `Block#playerDestroy` and tool wear | the server body calls `Level#destroyBlock` directly, so none of those rules apply and the block yields its plain unenchanted harvest whatever is held | `Open` |
| Break duration | progress accumulates per tick and the not-on-ground and underwater penalties apply | the server body destroys in one tick unless `ServerPlayerBody.faithfulBreak` is set, which two scenes do and nothing in production does | `Open` |
| Break reach and visibility | the crosshair raycast, plus the server's distance check | the server body requires the target to have at least one face that is not a full solid render, and the eye-to-centre distance to be within the body's own interaction range plus half a block. It does not raycast | `Approximate`, deliberately |
| Break reach on the client body | the crosshair raycast and the server's distance check | the client body answers "yes" to every cell and lets vanilla's own game mode decide, because second-guessing it there could only disagree with the game | `Accepted` |
...
| Swinging | every consumed use, place and attack swings | the swing lives at the call sites, not on the body: the client body swings on each dig tick, and the combat, interaction and elytra processes swing on a consumed action. The server body's place, use and attack do not | `Open` |

The two routes described above do not agree here, and the difference is deliberate. A
`mc.bot.*` verb addressed at a named body lands in `api/BodyInteractions`, which does what
...
headless body one tick per obstructing block costs the client body a real dig with vanilla's
divide-by-five penalties for being airborne and for having its eyes underwater. The two
scenes that switch `faithfulBreak` on do so precisely to model that: with it on, a
bare-handed deepslate block takes roughly 650 ticks and a stone block mined while afloat
roughly 750. With it off, both take one.
...
same stack in slot 20 spends nothing, moves the sole from 0.168 to 0.184, and puts the body
off the ledge.
...
| Changing the selected slot while an item is in use | the handler also stops the use, so a drawn bow is released from the draw | the driven body's write does not, so a bow drawn to full stays "in use" across a slot change | `Open` |
...
| Container clicks | the click arrives as a packet and the server's container handler applies it | the driven body calls `containerMenu.clicked` directly, after checking that the container id it was given is the one that is open | `Approximate` |
| Placing a recipe into a grid | the player has unlocked the recipe; the handler refuses otherwise | a freshly joined body's recipe book is empty, so the driver unlocks the recipe first and then places it | `Accepted` — the body is given something a player would have had to earn |
...
Publishing at all was added because of the adopted body. Both halves of the split hold
...
The joined body cannot be hurt. That is one deliberate line of code, and everything else in
this group follows from it.

| Behaviour | A connected player | A driven body | Status |
|---|---|---|---|
| Taking damage | damage applies, and death follows | `isInvulnerableTo` returns true, `die` is a no-op, and the body cannot harm another player | `Accepted` |
...
| Starving | a player at zero food starves | the body gets hungry and cannot die of it | `Accepted`, as a consequence of the row above |
...
| Death and respawn | death, a respawn screen, a respawn packet | nothing: a body that cannot die needs no respawn, and there is no verb for one | `Accepted` |
...
that the body lived, and scenes that need the other reading use a vanilla player as the
victim instead. A survival-fidelity run would want the invulnerability gone — and would
need the whole package at once, because a body that can be hurt but has no respawn is
...
largest reason joined bodies replaced bodies that were merely constructed.

| Behaviour | A connected player | A driven body | Status |
|---|---|---|---|
| Presence in `level.players()` | yes | yes, from `placeNewPlayer` — so mob AI, spawner proximity, natural spawning and the dragon fight all see the body | `Parity` |
| Chunk loading following the body | the move handler ends in a chunk-map move | `pump` does the same, guarded on player-list membership because the removal path inside it dereferences an entry a departed body does not have | `Parity` |
| Advancements | awarded through the inventory-menu listener and the criteria the player's tick fires | the listener is attached by the join path, and the body's constructor attaches it again for a body it was handed | `Parity` |
...
| Leaving the world | disconnect, and the chat line that goes with it | leaving means leaving the player list; the body routes its own removal through `PlayerList.remove` behind a re-entry guard | `Parity`, with the instrumentation caveat below |
...
| Chat as a source | a player can speak | sending chat is a client verb (`mc.client.chat.send`), as is every screen-level input verb; a headless body has none of them | `Accepted` |
| Commands | run with the player's own permission level, entity and position | `mc.action.runCommand` builds its source from the first player in the list, or from the server when there is none, and always at permission level 4 | `Approximate` — no body is ever refused for permission, and the anchor may be a different body than the caller had in mind |

A departure is *required* to be silent in vanilla's channel. The arrival is announced by the
join path, but the "left the game" line is broadcast from the disconnect path a driven body
...
body logs its own leave line, shaped like the join line, so the comparison is between two
things that answer the same question.

Two measurements are worth keeping about departure, because both were expensive:

- Before removal was routed through the player list, a scene disposing of its body left it
  ...
  skipped ran against a stranded body instead.
- The per-profile body cache sweeps departed bodies on every mint rather than from a removal
  ...
  advancements, statistics and inventory: 255 bodies retained after 300 scenes on the
  dedicated Fabric suite, and the 2 GB server heap exhausted around scene 290 in two runs of
  three.

The chunk-map move is the other one. Before it was part of every step, the body kept the
chunk section it joined in, which is what feeds chunk tickets, entity tracking and the
players-nearby check in front of natural spawning. Measured on the ladder's nether legs
...
A mob body is not measured against a connected player. What it is measured against is the
player body on the same staging: the scenes walk the mob first, then walk a server player
body from the same cell along the same route with breaking and placing disabled.

| Behaviour | A player body | A mob body | Status |
|---|---|---|---|
| Locomotion channel | writes impulse, jump and sneak, then vanilla moves the entity | the same, plus a speed field a mob's move control normally fills, which the body writes from the movement attribute and scales the impulse by | `Approximate` |
...
| Idling | a body nothing steps stands still | the same | `Accepted` |

The climb-out gap is the only reading in that set that differs. Across staircase, one-block
gap, open-water swim and ladder, the two bodies land within a tick or two of each other; on
...
lie is any assertion that budgets a mob's tick count from a player body's; the scenes that
...
- [testing.md](testing.md) — the gates, the topologies, and how coverage is reconciled
  across them.
- [movement-tick-phases.md](movement-tick-phases.md) — what the walker does with a body once
  it has one.
- [bot-layering.md](bot-layering.md) — where the body seam sits among the other layers.
- [../design/body-abstraction.md](../design/body-abstraction.md) — why there is one bot
  layer over three kinds of body, what was ruled out, and which vanilla facts forced the
  shape. This document says what the bodies do; that one says why they are built that way.
- [../design/world-view-parity.md](../design/world-view-parity.md) — the matching question
  for what each body's world view believes.
