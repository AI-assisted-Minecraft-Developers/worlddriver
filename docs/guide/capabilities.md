...
| `mc.query` | Scans blocks or entities in a cube, with filters and a `select` projection. Entity rows carry the `id` that `mc.bot.attackEntity` wants. | `q` (`blocks` or `entities`), `center`, `filter.in_radius`, `filter.type` (accepts a `#tag`), `filter.is_hostile`, `filter.is_living`, `select` |
