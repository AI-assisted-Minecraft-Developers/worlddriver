...
Boss 战本质是**多阶段状态机**，每个阶段就是"编排已有原语（combat / goto / elytraFly / equip / useItem）+ 读 boss 专属状态 + 切换 T0 反射开关"。这种高层逻辑放进**已有的 Rhino 沙箱层**（`config/agent_driver/scripts/`）有三个好处：
...
- 剧本是 `mc.script.eval` 能跑的 JS，或落盘 `config/agent_driver/scripts/playbooks/*.js` 由 `/agent reload` 载入。
