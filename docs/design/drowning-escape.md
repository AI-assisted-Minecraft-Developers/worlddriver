...
A body that runs out of air dies in a fixed number of ticks, and it dies while some other
...
because vanilla gives a body 300 air, spends one per tick, deals the first drowning damage
...
death exactly: the reflex latched, and the body died fifteen seconds later.
...
are in water, and divides by five again when the body is off the ground. The worst case is
...
threshold at which a bare-handed body digs its way out of stone.**

That makes `BotConfig.drownEscapeAirThreshold` (100) the wrong dial to reach for, twice over.
Its range is capped: raising it to 300, so the reflex latches the instant the body submerges,
...
so it can be exercised as a headless matrix. The latch arms when the body is underwater and
...
**Swim sideways** when the body's own column is capped but a neighbouring one can surface.
`lateralEscapeScan` walks outward through the cells the body actually fits in and returns the
*first step* of a route it has traversed, not a destination. That distinction was paid for: the
arm used to steer at any column that could surface within five cells whether or not the body
...
the body fits in, so a stalled body on that row is a physics question rather than a routing one.
...
## What blocks the rise is asked of the body, not of a column

The obstruction is found by `BotInteract.riseBlockedCell`, which lifts the body's **own bounding
...
for a body standing in the middle of its cell. A player box is 0.6 wide, so a body pressed against
...
Sweeping the body's own box is also right for slabs, stairs and lily pads, none of which a column
scan handles.

Half a block is the probe depth because terminal rise in water is about 0.175 per tick: far enough
to see the face a rising body is about to meet, short enough never to nominate something the body
would have drifted clear of. The arm re-asks every tick.

## Two things the reflex does before reaching for a pickaxe

**Recentre.** When the rise is blocked but the body's own column is clear all the way to air, the
obstruction is in a neighbour and the fix is at most 0.3 of a block of drift, not a dig — a body
...
of water carries the body to the opposite boundary and trades one pinning neighbour for the other.
This bends the arm's no-horizontal contract by less than the lateral arm does: it never leaves the
cell the body is already in.

**Stand up to dig.** While breaking, if the cell under the feet is not open, the jump is released.
The body drops onto the floor, `onGround` becomes true, and the same dig costs a fifth. This was
measured on a real client body: one dirt lid cost 380 ticks with the body parked at y=208.235 over
...
lifting a 1.8-tall box reaches 2.3 above the feet, so in a two-block-tall pocket a grounded body
still finds the lid and releasing the jump costs nothing. In a deeper pocket the body would sink
...
and gate runs never set it, so a run in which this chain latched and the body drowned 300 ticks
...
The volume is bounded by the episode rather than being a stream: this only ticks while a body is
