# mc-testkit：跨加载器 Minecraft mod 测试框架 — 设计 spec
...
**mc-testkit 是面向 mod 开发者的跨加载器（Fabric + NeoForge）测试框架产品**，提供测试编排、断言语法接口与三种测试拓扑；agent-driver 是它的第一个用户（self-hosting）——本项目 ~130 个既有 GameTest 迁移到其上。
...
mc-testkit    编排 + 断言 DSL + harness + gradle 插件  （新产品）
agent_driver  驱动层：观察/动作/客户端输入/屏幕内省/
              RPC 通道/事件/wait —— 仪表原语唯一提供者  （既有 mod）
Minecraft     fabric / neoforge，1.21.1
```

**依赖方向铁律：mc-testkit → agent_driver，永不反向。** agent_driver 的定位是通用 agent 驱动层（观察 + 动作 + 客户端输入注入 + RPC + Rhino），客户端仪表（ClientInput/ScreenIntrospection/ClientObserve）是驱动层本职，不抽取。第三方 mod 开发者依赖两个 jar（maven 传递），得到「通用驱动层 + 测试编排层」。

### 2.1 模块划分（已拍板：本仓库平级新项目 `mc-testkit/`）

| 模块 | 形态 | 职责 |
|---|---|---|
| `core` | 多加载器 common 库 | 测试模型、断言 DSL、JSONL 结果 schema。由既有 `common/src/main/java/net/magicterra/agent/test/`（AgentTest/TestContext/yaml）迁出演化，非绿地 |
...
| `gradle-plugin` | Gradle 插件（P3） | `testkitServer` / `testkitClient` / `testkitE2E` 任务；testmod source set 惯例；shell 同一编排契约 |

**编排契约先冻结**（P1 定稿，Python 与后来的 gradle-plugin 共用）：JSONL 结果格式、退出码语义、端口文件发现协议（`agent-rpc.port`）、进程启动/终止协议、世界模板协议。
...
agent-driver 用 testkit 自测，而 testkit 依赖 agent-driver——需防「驱动层回归让测试栈连坐失明」。**驱动层 API 面一分为二**：
...
| 端口文件发现（读各进程 `agent-rpc.port`，ephemeral 回退已内建于驱动层） | 多进程拓扑、并行 run 端口冲突 |
...
| 决策 | 结论 | 理由 |
|---|---|---|
| 仓库归属 | 本仓库平级新项目 `mc-testkit/` | 共享坑史与迭代速度；将来可拆库发布 |
...
| 依赖方向 | testkit → agent_driver，不抽取仪表 | 驱动层定位（user 纠正 07-16）：客户端输入/内省是驱动层本职 |
