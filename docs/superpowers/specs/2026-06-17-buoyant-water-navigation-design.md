...
**状态**:设计待评审
**日期**:2026-06-17
**背景会话**:深水寻路 churn 整类根治(0408b2e / 4ecb972 之后)
...
| 方向 | 症状 | 现状 |
|---|---|---|
| 上爬(stepUp/diag/swimAshore from deep) | 水下蹬不起跳 | 已禁(0408b2e) |
| 下降(diagDown/stepDown into submerged) | 浮在上面振荡 | 已禁(4ecb972) |
...
- **F4(架构冲突)**:当前**故意**把水域横渡路由在 submerged 河床节点(浮力 foot 下方 ~1 格),执行器靠 `floatOverSubmerged`(`Walker.java:1313`)贴水面骑过去,**限定在水面下 ≤2.5 格**(`dyNode > -2.5`)。z3034 的 churn = A* 把节点路由到 y55-58(浮力 foot y62 下方 4-7 格,**跌出 2.5 骑行带**)→ 补偿不触发 → bot 真去下潜 → 沉底振荡。这是 535df12 调出来的机制,本设计将**作废**它。
...
z3034(河床 walk)、4ecb972(下降)、cascade(薄流水水下 stepUp)全归 D1;0408b2e 的 ascent 归 D2。
...
- 不直接改全局 `canStandAt`(memory 反复警告其调用面太广:mine/escape/stand-selection),而是在 move 层派生——与 0408b2e/4ecb972 同模式,把那两处也并入这一条(见 §4.4)。
...
- **收编**:`isFloatingWater`/`isSubmergedAscent`/`isSubmergedFoot`(0408b2e,ascent)归 **D2**;diagDown/stepDown 的 4ecb972 守卫归 **D1**——统一改为从 §3 的水位分类派生,文档说明它们是同一不变量的两个投影,而非分散的 ad-hoc 检查。
...
- **Phase 1 — 核心 head-submerged 守卫**(地面族 7 个 move + `headSubmerged` 谓词)。验:GameTest 全绿;replay z3034 不再沉河床(改走水面或绕);开阔深水穿越 arena 仍过(关键回归护栏,535df12)。
...
- **R1 重写横渡核心**:Surface-First 动 535df12 调过的开阔深水穿越平衡。**缓解**:Phase 1 即把该 arena 设为护栏,replay 开阔深水段每阶段比对。
