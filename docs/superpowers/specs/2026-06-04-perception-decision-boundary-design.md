...
- **改动**：`BotApiImpl.clientTick` +1 行 + `worldModel` 字段 + 注册 `DuskSecureChain`；`ClientWorldView` 持 worldModel 引用、`beginSearch`/`dangerCost` 注入 HazardField；`Priorities` 加 `IDLE_SECURE=40`；`BotConfig` 加 §7 旋钮；`AgentApi`+`prelude.js`+catalog 加 `mc.client.scene`（route + schema + `Agent.client.scene`）；`ThreatScanner` 作为 WorldModel 输入（不改其内部）。
