...
- `.../pathfinder/moves/StepDown.java`、`DiagonalDescend.java` — 把现有 `isWater(to)&&isWater(to+1)`(26d4ea2)守卫**替换**为统一的 `headSubmerged(to)`(更广:覆盖实心河床+头淹没)。
...
```java
     ...
     * (26d4ea2): it also catches a SOLID riverbed cell with water overhead (foot
...
### Task 1.3: StepDown + DiagonalDescend 收编 26d4ea2 → D1
...
替换为:
```java
        // D1 (was 26d4ea2's isWater(to)&&isWater(to+1)): unify under headSubmerged so
