...
1. **Renderer NPE** (`cc3abed`) — `plotYaw` used `actual ? (double)x : targetBearing(...)`; the mixed primitive/`Double` ternary auto-unboxed a null `targetBearing()` (no-target sample) → NPE *before* the null guard. No chart rendered until fixed. Split into separate statements.
2. **`mc.bot.setting` schema gap** (`e49a8f3`) — the 4 new keys existed in `BotConfig`/`SettingsCommand` but were not declared in the `mc.bot.setting` MCP input schema in `BotTools.java`, so the MCP transport silently dropped them (worked via direct route/script only). Added keys + docs.
