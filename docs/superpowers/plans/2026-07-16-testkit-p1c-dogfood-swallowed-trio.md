...
- Produces: `./gradlew :neoforge:runDogfoodServer` 起一台同时载 worlddriver + testkit harness 的专服，`neoforge/run-dogfood/testkit-results.jsonl` 输出契约 v0 记录。
...
Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/testkit-results.jsonl; echo "exit=$?"`
...
Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/testkit-results.jsonl && python3 scripts/stagewright/t0.py --loader neoforge --wall 540; echo "exit=$?"`
