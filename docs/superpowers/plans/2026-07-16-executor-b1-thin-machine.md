...
> Unit 1 (scaffolding, commit 53ce8de) STANDS — only the delegation branch semantics change.
...
- [x] **B1-1 (6d9ec88) — fall-through + episode tracking (behavior no-op):** reshape the delegation branch
...
- [x] **B1-2 (c426929; seam-arena verified via explicit AGENT_GT_ONLY execution — see the suite-integrity P0: full runs silently never execute the ascend arenas) — arm the watchdog + task#82 arena (RED→GREEN):** bounded no-progress episode →
