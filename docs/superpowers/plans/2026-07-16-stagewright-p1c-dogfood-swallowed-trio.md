# stagewright P1c：dogfood 迁移第一批（被吞三员）实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 打通 worlddriver 场景挂进 testkit T0 的完整路径（SceneProvider SPI + dogfood 专服），并把 #85 被吞三员（ascendMovementNoop / ascendDeadZoneWatchdog / diagonalAscentSpeed）迁成 testkit 场景，新旧壳 A/B 后旧门与新门并行运行。

**Architecture:** stagewright-common 加 ServiceLoader 制 SceneProvider 缝 + SceneContext 补 level()/origin()/cleanup() + PREP 等满 3×3 forceload；worlddriver neoforge 依赖 :stagewright-common（方向不逆转），自带 dogfoodServer run 配置并在 stagewright.autorun 时调 StageWrightCommon 钩子；三个被吞测试按「同步循环原样进 body 首 tick」语义换壳（保留 byte 级时序，A/B 可比），legacy @GameTest 保留至 3 轮双门全绿再删（本计划不删）。

**Tech Stack:** Java 21 / architectury-loom；ServiceLoader（META-INF/services）；既有 t0.py 编排器加 `--run-task` 泛化。

## Global Constraints

- 依赖方向：worlddriver → testkit（neoforge 依赖 :stagewright-common）；stagewright 各模块**零** worlddriver import（评审红线）。
- 编排契约 v0 不改语义；judge() 的新增 done.scenes 对账是**纯收紧**（多抓一类截断，绝不放宽）；t0.py 既有 11 条 self-test 必须全过。
- 场景名全局唯一（StageWrightHarness 重名即炸）；worlddriver 场景名前缀 `ad.`。
...
- 运行纪律：前台等待用 Bash timeout 参数；PID 清扫按 marker（dogfood 服沿用 `stagewright.autorun`，t0.py 的 sweep 直接适用）；禁 pkill；每 run 删世界。
...
3. **只迁被吞三员**：彩票家族（gearscope/descentyaw/buriedore/entityleash/selfshaftdigup）依赖 ServerWorldDriver/Process 栈或百行级 arena，归 P1.5/P2 按 family 继续；三员先证明通路。
...
| 文件 | 职责 |
|---|---|
| `stagewright/common/.../scene/SceneProvider.java`（新） | SPI 接口：`List<Scene> scenes()` |
| `stagewright/common/.../scene/Scenes.java`（改） | `all()` = 内建 5 场景 + ServiceLoader 发现的 provider 场景 |
| `stagewright/common/.../scene/SceneContext.java`（改） | 暴露 `level()`/`origin()`；新增 `cleanup(Runnable)` |
| `stagewright/common/.../harness/StageWrightHarness.java`（改） | PREP 等满 3×3 全 chunk；出口统一跑 cleanups |
| `scripts/stagewright/verdict.py` + `t0.py`（改） | judge 补 done.scenes==记录数对账 + self-test；t0 加 `--run-task`/`--results-dir` 泛化 |
| `neoforge/build.gradle`（改） | 依赖 :stagewright-common；`dogfoodServer` run 配置（runDir `run-dogfood`） |
| `neoforge/.../WorldDriverNeoForge.java`（改） | stagewright.autorun 时接 StageWrightCommon 钩子 + applyGameTestBaseline |
| `neoforge/.../testkit/WorldDriverScenes.java`（新） | SceneProvider 实现：三个迁移场景 |
...
| 文档：`docs/stagewright/orchestration-contract-v0.md`（附录小节）、`stagewright/README.md`、`TODO.md` | SPI 说明、dogfood 入口、A/B 留痕 |

---

### Task 1: stagewright-common 侧三件——SceneProvider SPI + SceneContext 扩口 + PREP 全 forceload 等待

**Files:**
- Create: `stagewright/common/src/main/java/net/magicterra/stagewright/scene/SceneProvider.java`
- Modify: `stagewright/common/src/main/java/net/magicterra/stagewright/scene/Scenes.java`
- Modify: `stagewright/common/src/main/java/net/magicterra/stagewright/scene/SceneContext.java`
...
- Consumes: 既有 Scene/Canary/StageWrightHarness。
...
`StageWrightHarness` 场景出口（写 scene 记录之前/之后均可，但必须三种 outcome 都覆盖——找到 RUN/ADVANCE 阶段判定 PASS/FAIL/TIMEOUT 后、进入下一场景前的单一汇合点）插入：
...
Run: `./gradlew :stagewright-common:compileJava :stagewright-neoforge:build -q && python3 scripts/stagewright/t0.py --loader neoforge --wall 540; echo "exit=$?"`
...
**Files:**
- Modify: `scripts/stagewright/verdict.py`（judge 内加对账）
- Modify: `scripts/stagewright/t0.py`（self-test 加 2 条；CLI 加 `--run-task` 与 `--results`）

**Interfaces:**
- Produces: judge 新规则——`done.scenes` 字段存在且 ≠ 实际 scene/check 记录数 ⇒ RED（报告行 `TRUNCATED: done.scenes=N but M records`）；t0.py `--run-task <gradle task>`（默认 `:testkit-{loader}:runStageWrightServer`）与 `--results <path>`（默认现值），供 dogfood 复用同一编排器。
...
### Task 3: worlddriver 挂载——依赖 + dogfoodServer + 钩子接线，骨架场景跑通

**Files:**
- Modify: `neoforge/build.gradle`（依赖 `:stagewright-common` + `dogfoodServer` run 配置）
- Modify: `neoforge/src/main/java/net/magicterra/worlddriver/neoforge/WorldDriverNeoForge.java`（stagewright.autorun 接线）

**Interfaces:**
- Consumes: StageWrightCommon 的钩子方法（读 `stagewright/common/.../harness/StageWrightCommon.java` 确认精确签名——onServerStarted(MinecraftServer)/每 tick 方法名以文件为准）。
- Produces: `./gradlew :neoforge:runDogfoodServer` 起一台同时载 worlddriver + testkit harness 的专服，`neoforge/run-dogfood/stagewright-results.jsonl` 输出契约 v0 记录。

- [ ] **Step 1: 依赖**

`neoforge/build.gradle` dependencies 块加（对照本文件既有 `:common` 的声明形态选同款配置名——architectury 惯例是 `common(project(path: ':stagewright-common', ...))` 与 `shadowBundle`/`implementation` 组合；以 `:common` 现有两行为模板逐字改造）：

```groovy
    implementation project(path: ':stagewright-common', configuration: 'namedElements')
```

若 `:common` 用的是 `common(...)+shadowBundle(...)` 双行惯例，则对 `:stagewright-common` 同样双行。**判据**：`./gradlew :neoforge:compileJava` 能解析 `net.magicterra.stagewright.harness.StageWrightCommon`，且 dogfood run 时 testkit 类在 classpath（Step 4 实证）。
...
注意 neoforge 的 `runConfigs.configureEach` 有 contractServer 名字守卫——dogfoodServer **不需要**守卫（-Xmx2g 与 configureEach 一致，worlddriver.runValidation 等属性无害），除非 Step 4 实证有冲突。

- [ ] **Step 3: WorldDriverNeoForge 接线**

构造器/事件注册处（对照现有 ServerStarting/ServerStarted/Tick 事件监听的写法）加 stagewright.autorun 门：

```java
    private static final boolean TESTKIT_AUTORUN = Boolean.getBoolean("stagewright.autorun");
```

- ServerStarted 事件体内：`if (TESTKIT_AUTORUN) { BotConfig.applyGameTestBaseline(); StageWrightCommon.onServerStarted(event.getServer()); }`
- ServerTick(Post) 事件体内：`if (TESTKIT_AUTORUN) StageWrightCommon.onServerTick(event.getServer());`
（方法名以 StageWrightCommon 实文件为准；若它还有 gate 内部判断 stagewright.autorun，外层门保留——双门幂等无害，且 worlddriver 侧的门让 baseline 应用与 harness 严格同条件。）

- [ ] **Step 4: 端到端——内建 5 场景在 dogfood 服上 GREEN**

Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl; echo "exit=$?"`
Expected: `VERDICT: GREEN` exit=0——5 内建场景（含金丝雀）在载有 worlddriver 的专服上原样通过。这证明：钩子接线对、双 mod 共存无干扰、编排器泛化可用。

同时验证隔离：`ps aux | grep -E '[t]estkit.autorun'` run 后为空；`neoforge/run-dogfood/worlddriver-rpc.port` 存在（worlddriver 已加载的旁证）。

- [ ] **Step 5: Commit**

```bash
git add neoforge/build.gradle neoforge/src/main/java/net/magicterra/worlddriver/neoforge/WorldDriverNeoForge.java
git commit -m "feat(testkit): worlddriver dogfood wiring — :stagewright-common dep, runDogfoodServer, autorun hooks + baseline"
```

---

### Task 4: 迁移被吞三员为 worlddriver 场景 + provider 注册

**Files:**
- Create: `neoforge/src/main/java/net/magicterra/worlddriver/neoforge/testkit/WorldDriverScenes.java`
- Create: `neoforge/src/main/resources/META-INF/services/net.magicterra.stagewright.scene.SceneProvider`
- Modify: `neoforge/src/main/java/net/magicterra/worlddriver/neoforge/AgentGameTestTerrain.java`（三员方法 javadoc 加迁移注释，代码不动）
...
- Produces: 场景 `wd.ascendMovementNoop`、`wd.ascendDeadZoneWatchdog`、`wd.diagonalAscentSpeed`（required=true, canary=NONE）。

- [ ] **Step 1: services 文件**

`neoforge/src/main/resources/META-INF/services/net.magicterra.stagewright.scene.SceneProvider` 单行：

```
net.magicterra.worlddriver.neoforge.testkit.WorldDriverScenes
```

- [ ] **Step 2: WorldDriverScenes 骨架 + watchdog 场景（完整模板）**
...
```java
package net.magicterra.worlddriver.neoforge.testkit;

import java.util.List;
import net.magicterra.stagewright.scene.Scene;
import net.magicterra.stagewright.scene.SceneContext;
import net.magicterra.stagewright.scene.SceneProvider;
// ... worlddriver imports 按需（ServerPlayerAvatar, LevelWorldView, Walker, Goal,
...
新壳：
Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl; echo "exit=$?"`
Expected: `VERDICT: GREEN` exit=0，JSONL 含 8 场景记录（5 内建含金丝雀 + 3 wd.*，吞金丝雀无记录），三个 wd.* 全 PASS。
...
```bash
git add neoforge/src/main/java/net/magicterra/worlddriver/neoforge/testkit/ neoforge/src/main/resources/META-INF/services/ neoforge/src/main/java/net/magicterra/worlddriver/neoforge/AgentGameTestTerrain.java
# grantWaterEffects 若提公有: git add neoforge/src/main/java/net/magicterra/worlddriver/neoforge/AgentGameTestSupport.java
git commit -m "feat(testkit): dogfood wave 1 — swallowed trio migrated to wd.* scenes (SceneProvider), legacy kept for dual-gate A/B"
```
...
**Files:**
- Modify: `docs/stagewright/orchestration-contract-v0.md`（附录：SceneProvider 与 done.scenes 对账两小节，语义澄清不 bump 版本）
- Modify: `stagewright/README.md`（dogfood 入口 + SPI 三行示例）
...
Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl && python3 scripts/stagewright/t0.py --loader neoforge --wall 540; echo "exit=$?"`
...
2. **占位符扫描**：watchdog 模板给出映射规则+完整结构；noop/diagonal 按同规则直译并点名行号与保留数值（500/900 循环、ALLOC_COUNT=0、ascBps≥2.5）；唯一开放点（Scene.of 工厂精确参数、StageWrightCommon 钩子方法名、avatar discard 方法名）都指明了「以实文件为准」的读取对象——这是对既有代码的引用而非留白。
3. **类型一致性**：SceneProvider.scenes() 与 Scenes.all() 的合并类型一致；ctx.cleanup(Runnable) 与 pin::close/fp::discard 兼容；t0 的 --run-task/--results 在 Task 3/4/5 的调用串一致；场景名 wd.* 三处（WorldDriverScenes/legacy 注释/TODO）一致。
