# stagewright P2b — T1 拓扑 + 客户端仪表扩展 实现计划
...
**Architecture:** T1 壳 = fabric 客户端 JVM 带 `-Dstagewright.autorun=true`：integrated server 开世界即触发既有 `StageWrightCommon.onServerStarted`（两 loader 事件钩子都是通用 server 生命周期，P2b 零 harness 改动预期），9 个 wd.* 场景原样跑在 integrated server 上=拓扑证明。编排器 `t1.py` 负责 xvfb、gradle 客户端启动、端口文件发现、GUI 进世界驱动（复用 `into_world.py`/`react_smoke.py` 的 widget 树点击库）、世界模板 copy→run→delete、done footer 收割。客户端仪表检查为独立 `instrument_client.py`（instrument.py 的 client 版：裸 RPC 打 in-world 客户端）。**范围决策**：T1 先 fabric-only（唯一有实证客户端工具链的 loader，neoforge 客户端 parity 单列后续）；in-game 形态的 UI 场景（@SceneTest 体内做屏幕断言）不在本阶段——client 断言经编排器/仪表面完成，避免场景体内跨线程阻塞（执行模型铁律），in-game UI 授权形态随 P2c/JUnit5 再议。两决策写进 TODO 偏差声明。
...
- **T1 世界纪律**（spec §5 制度化）：模板世界一次生成缓存（gitignored 目录），每 run copy→enter→run→**删副本**;世界名固定 `StageWrightT1`;禁复用脏世界（持久世界污染病史）。
- **客户端进程纪律**：编排器全程 PID 追踪,退出路径=quit-to-title→窗口关闭或显式 PID kill;**禁 pkill**（smoke-test-client.sh 的 pkill 行不是可抄先例）;GL-hang 签名自愈=TitleScreen 停留超时即按 widget 点击穿透（spec §5 行,into_world 已有 wait 逻辑）;`worlddriver-rpc.port` 端口文件发现,不 hardcode 39801。
...
| 文件 | 责任 |
|---|---|
| `fabric/build.gradle` | `stagewrightClient` run 配置（stagewright.autorun=true + ephemeral 端口 + run-t1 目录;configureEach 守卫名单 +`stagewrightClient`） |
| `scripts/stagewright/t1.py`（新） | T1 编排器:xvfb 起停（自管 DISPLAY,PID 追踪）、gradle stagewrightClient 启动、端口文件轮询、GUI 进世界（import react_smoke/into_world 的 helper 或抽小库 `guidrive.py`）、模板世界生命周期、done footer 收割、exit code 语义与 t0 相同、self-test |
| `scripts/stagewright/guidrive.py`（新,如抽库） | 从 react_smoke/into_world 抽出的可复用 widget-树驱动函数（find/click/wait_until/world-select）——两脚本原文不动,库为增量 |
| `scripts/stagewright/instrument_client.py`（新） | 客户端仪表契约:in-world 前置探针+检查族（下表）+2 金丝雀;`--rounds N` 支持复用完备性 |
| `docs/stagewright/instrument-contract-v0.md` | client 附录:检查清单+T1 拓扑语义+reset 完备性协议 |
| `common/.../WorldDriverCommon.java` | task#89:requireSchemasFor+setParamsValidator 前移至 RpcServer 构造**之前**（保 hard-fail-outside-catch 性质,详 Task 4） |
| `stagewright/README.md`、`TODO.md` | T1 节+P2b 条目+两偏差声明 |
...
### Task 1: T1 壳 — stagewrightClient 配置 + t1.py 编排器 + 场景跑通

**Files:** `fabric/build.gradle`、`scripts/stagewright/t1.py`（新）、`scripts/stagewright/guidrive.py`（新,建议抽库）
**Interfaces:**
- Consumes: into_world.py/react_smoke.py 的 title→世界驱动逻辑;StageWrightCommon.onServerStarted（integrated 上天然触发,零改动预期——若实测不触发=BLOCKED 上报,禁在 harness 侧偷改）;expected-scenes-fabric.txt（9 名照用）。
- Produces: `python3 scripts/stagewright/t1.py --wall 900`（默认 fabric;--expect-file 同 t0 语义;exit 0/1/2/3 同契约）;世界模板缓存目录 `scripts/stagewright/.t1-world-template/`（gitignored,首跑经 GUI 创建 StageWrightT1 后归档,此后每 run 拷入 saves/ 用毕删）。

- [ ] **Step 1: gradle 配置**（镜像 dogfoodServer 形态:autorun property/vmArg、ephemeral 端口、run-t1 目录;fabric configureEach 守卫加名）
- [ ] **Step 2: t1.py**（xvfb 自起自收[PID 记账,占用检测复用或新起 DISPLAY]→gradle 后台启动+前台有界轮询→port file→GUI 进世界[模板存在=拷贝后直选;不存在=GUI 创建一次并归档模板]→integrated server 起=autorun 场景自动跑→轮询 run-t1 的 stagewright-results.jsonl done footer→verdict 复用 verdict.py→退出路径 quit/kill+删世界副本;TitleScreen 停留自愈;self-test 若干条纯函数）
...
**Files:** `scripts/stagewright/instrument_client.py`（新）、`docs/stagewright/instrument-contract-v0.md`（client 附录）
**Interfaces:**
- Consumes: t1.py 的客户端启动/进世界（抽公共函数或 t1.py 提供 `--hold` 模式:进世界后不跑场景保持在线,供检查脚本打）;rpc.py 线格式;检查族清单（File Structure 表）。
- Produces: `python3 scripts/stagewright/instrument_client.py --wall 900`（自起 T1 客户端或 `--attach` 复用在线客户端;9+2 检查;JSONL+exit code 同 instrument.py 语义）。
...
**Files:** `scripts/stagewright/instrument_client.py`（--rounds）、`docs/stagewright/instrument-contract-v0.md`（协议节）
...
**Files:** `common/.../WorldDriverCommon.java`
...
3. **一致性**：t1.py 的 --hold/--attach 在 T1 定义、T2 消费;guidrive 抽库 T1 建、T3 复用;StageWrightT1 世界名/模板目录名全文一致;9 场景=P2a 后 fabric 清单现值。
