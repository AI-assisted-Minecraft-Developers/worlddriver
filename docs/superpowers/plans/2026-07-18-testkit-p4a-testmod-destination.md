...
**Interfaces:**
- Consumes: 现 `runGameTestServer`/`run_gametests.sh` 正门；AgentGameTestSupport 对 `net.magicterra.worlddriver.bot.testkit.SimProbes` 的三处委托（SimProbes 本 task **仍在 common main**，跨源集可见性不变）。
...
- Move: `common/src/main/java/net/magicterra/worlddriver/bot/testkit/{WorldDriverScenes,SimProbes}.java` → `common/src/testmod/java/net/magicterra/worlddriver/bot/testkit/`；`common/src/main/resources/META-INF/services/net.magicterra.stagewright.scene.SceneProvider` → `common/src/testmod/resources/META-INF/services/`
