# stagewright P1.5b：彩票家族 driver 三员迁移 + 期望清单化 + task#86 签名门 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把彩票家族 driver 形态三员（gearScope、buriedOre、entityLeash）迁成 testkit 场景（建立 ServerWorldDriver/Process 类 arena 的移植模式），期望门从手打名单升级为 checked-in 清单（--expect-file），task#86 传感器升级为 golden-failure 签名门。

**Architecture:** 三场景走既有 SceneProvider/dogfood 通路，身体一律 `ServerWorldDriver.createIsolated`（#48 修复模式的 driver 版），manager 生命周期进 `ctx.cleanup`（targeted unregister + fp discard）；entityLeash 的手动 `level.tick(()->true)` 直译移植（场景在 ServerTickEvent.Post 执行，重入位置比 legacy GameTest 更安全），失败再降级为 await 步；--expect-file 读 `scripts/stagewright/expected-scenes-neoforge.txt`（与 --expect-scene 并集）；wd.selfShaftDigUp 从 optional 翻回 required，断言改钉已知失败签名（bug 精确复现=PASS，修好或漂移=RED 大声报）。
...
| 文件 | 职责 |
|---|---|
| `scripts/stagewright/t0.py`（改） | `--expect-file`（换行/逗号分隔、`#` 注释行），与 --expect-scene 并集去重 |
| `scripts/stagewright/expected-scenes-neoforge.txt`（新） | checked-in 期望清单（先 5 名，随 Task 3-5 各自追加） |
| `scripts/stagewright/verdict.py`（改，docstring only） | parse「no escape to GREEN」表述加第三方 harness 限定句 |
| `stagewright/common/.../harness/StageWrightHarness.java`（改） | 显式槽位 < 1024 ⇒ IllegalStateException（防低位钉槽挤动 auto 区） |
| `neoforge/.../testkit/WorldDriverScenes.java`（改） | wd.selfShaftDigUp 签名门化 + 三新场景 + porting map 更新 |
...
- Create: `scripts/stagewright/expected-scenes-neoforge.txt`
- Modify: `scripts/stagewright/verdict.py`（docstring only）
...
# One name per line. Every migrated wd.* scene MUST be listed here in its migration commit —
...
wd.ascendMovementNoop
wd.ascendDeadZoneWatchdog
wd.diagonalAscentSpeed
wd.selfShaftDigUp
wd.descentYaw
```
...
Run: `python3 scripts/stagewright/t0.py --self-test; echo t0=$?`
Expected: 19/19 PASS exit 0。

Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl --expect-file scripts/stagewright/expected-scenes-neoforge.txt; echo "exit=$?"`
Expected: `VERDICT: GREEN` exit=0（5 名单场景全在册，wd.selfShaftDigUp 仍 fail(optional)——签名门是 Task 2）。

- [ ] **Step 5: Commit**

```bash
git add scripts/stagewright/t0.py scripts/stagewright/expected-scenes-neoforge.txt scripts/stagewright/verdict.py
...
**Interfaces:**
- Produces: wd.selfShaftDigUp 变 required 签名门；harness 显式槽 < 1024 拒绝。
- Consumes: P1.5a 的钉槽/withRequired 基建。

- [ ] **Step 1: 签名门改造**

wd.selfShaftDigUp 场景：**删掉** `.withRequired(false)`；断言逻辑改为钉已知失败签名——保留完整 arena/循环/采样，结尾判定换成：
...
```java
            ...
            if (!knownSignature) {
                ctx.fail("task#86 signature broke: reached=" + reached
                        ...
                        + " flip wd.selfShaftDigUp to the strict assertion and close #86)");
            }
```
...
StageWrightHarness.assignSlots pass-1 里显式槽检查：
...
Run: `./gradlew :stagewright-common:compileJava :neoforge:compileJava -q && python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl --expect-file scripts/stagewright/expected-scenes-neoforge.txt; echo "exit=$?"`
Expected: `VERDICT: GREEN` exit=0，**wd.selfShaftDigUp 现在是 required PASS**（签名匹配），JSONL 该场景 outcome=PASS。

- [ ] **Step 4: Commit**

```bash
git add neoforge/src/main/java/net/magicterra/worlddriver/neoforge/testkit/WorldDriverScenes.java stagewright/common/src/main/java/net/magicterra/stagewright/harness/StageWrightHarness.java
git commit -m "feat(testkit): task#86 sensor -> required golden-failure signature gate; origin-slot floor guard 1024"
```

---

### Task 3: 迁移 wd.gearScope（probe 提升 + driver 模式确立）

**Files:**
- Modify: `neoforge/src/main/java/net/magicterra/worlddriver/neoforge/AgentGameTestServer.java`（probeSwing:2507/probeHurt:2532 private→public static + serverAvatarGearScopeProbeArena javadoc 注释；代码零动）
- Modify: `neoforge/.../testkit/WorldDriverScenes.java`（新场景 + porting map 更新：driver 模式条目 + failure-msg prefix 约定注记）
...
Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl --expect-file scripts/stagewright/expected-scenes-neoforge.txt; echo "exit=$?"`
Expected: GREEN，wd.gearScope PASS，probe 数值与 legacy solo 同值（写进报告）。

- [ ] **Step 3: Commit**

```bash
git add neoforge/src/main/java/net/magicterra/worlddriver/neoforge/AgentGameTestServer.java neoforge/src/main/java/net/magicterra/worlddriver/neoforge/testkit/WorldDriverScenes.java scripts/stagewright/expected-scenes-neoforge.txt
git commit -m "feat(testkit): dogfood wave 2b — wd.gearScope migrated (driver pattern established), probe helpers promoted"
```

---

### Task 4: 迁移 wd.buriedOre（MineProcess + manager 循环）
...
- [ ] **Step 3: Commit**（`feat(testkit): dogfood wave 2b — wd.buriedOre migrated (MineProcess/manager loop pattern)`）

---

### Task 5: 迁移 wd.entityLeash（两阶段 + 手动 tick 直译）

**Files:**
- Modify: `WorldDriverScenes.java` + `AgentGameTestServer.java`（javadoc）+ `expected-scenes-neoforge.txt`（+wd.entityLeash）
...
- Produces: `wd.entityLeash`。**手动 tick 决策**：直译保留 `level.tick(()->true)`——场景在 ServerTickEvent.Post 执行（所有 level 已 tick 完），重入比 legacy（GameTest 在 tick 内跑）更安全；javadoc 记录该论证。**降级预案**（仅当直译实测异常——崩溃/实体索引仍失败）：把两处手动 tick 换成 `ctx.await(实体可查询).within(5)` 真实 tick 等待，两阶段拆 await 步，porting map 记录语义偏差与理由；降级属 sanctioned，但必须先试直译并留证据。
...
- [ ] **Step 3: Commit**（`feat(testkit): dogfood wave 2b — wd.entityLeash migrated (two-phase, manual-tick direct port)`）
...
**Files:**
- Modify: `stagewright/README.md`（正门命令换 `--expect-file scripts/stagewright/expected-scenes-neoforge.txt`）
- Modify: `docs/stagewright/orchestration-contract-v0.md`（附录：expect-file 小节，语义只收紧仍 v0）
- Modify: `TODO.md`（P1.5b 条目）

- [ ] **Step 1: 全量验收**

Run: `python3 scripts/stagewright/t0.py --loader neoforge --wall 540 --run-task :neoforge:runDogfoodServer --results neoforge/run-dogfood/stagewright-results.jsonl --expect-file scripts/stagewright/expected-scenes-neoforge.txt && python3 scripts/stagewright/t0.py --loader neoforge --wall 540; echo "exit=$?"`
Expected: dogfood GREEN（13 场景记录：5 内建 + 8 wd.*，全 required 全 PASS——签名门含）+ 纯 T0 GREEN。
...
```bash
git add stagewright/README.md docs/stagewright/orchestration-contract-v0.md TODO.md
