# mc-testkit P1.6 — Fabric 对齐 实现计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 让 agent-driver 的 fabric loader 获得与 neoforge 同级的 testkit dogfood 门（8 个 ad.* 场景 + 13 记录验收），同时把从未被测过的 fabric 服务端驱动层第一次置于测试之下。

**Architecture:** sim 核心（ServerPlayerAvatar/ServerAgentDriver/ServerAgentManager）与场景（AgentDriverScenes + probe helpers）从 neoforge 模块搬入 common；FakePlayer 的 loader 差异用「loader 注入 body 工厂」seam 解决（repo 无 @ExpectPlatform 先例，不引新依赖）：neoforge 注入 FakePlayerFactory（行为字节级不变），fabric 注入 common 自造的 vanilla-only `AgentFakePlayer`。neoforge 原 FQN 全部留薄 shim（协变返回 FakePlayer），legacy AgentGameTestServer 约 3000 行**零源码改动**。重构护栏 = P1.5 系列建立的字节级指标门。

**Tech Stack:** architectury 多 loader（loom.platform per module）、ServiceLoader SPI、t0.py --expect-file 正门。

## Global Constraints

- 依赖方向：mc-testkit 永不依赖 agent-driver loader 模块；agent-driver（common）→ :testkit-common 是 P1c 已批的测试消费关系，本阶段扩展到 common 模块合法。
- **legacy 零改动**：`AgentGameTestServer.java`/`AgentGameTestSupport.java` 除新增 delegate 与 javadoc 注记外源码不动；neoforge `sim/` 三类保持原 FQN + 原 API（含 `FakePlayer` 返回类型），`/agentserver`、`AgentDriverNeoForge.tickAll`、全部 legacy 测试**不改一行即编译通过**。
- **neoforge 字节级指标门**（每个重构 Task 的验收；任一漂移=STOP/BLOCKED，禁止调门）：
  - `ad.descentYaw`: 871°/53（golden 双记之迁移版）
  - `ad.selfShaftDigUp` 签名门 PASS 且 `worstBackslide=20.252203415101263`
  - `ad.gearScope`: bare=0.94000053 sword=5.9040003 ATTACK_SPEED=1.5999999046325684 ATTACK_DAMAGE=6.0
...
| `neoforge/.../sim/ServerAgentDriver.java`(125 行) | `common/.../bot/sim/ServerAgentDriver.java` | 同 FQN 薄 shim（协变访问器 + createIsolated 委托） |
...
| `neoforge/.../testkit/AgentDriverScenes.java` | `common/.../bot/testkit/AgentDriverScenes.java`（import 换 common sim;probe 调用换 SimProbes） | 删除（provider 以 common 版注册） |
...
| `neoforge/src/main/resources/META-INF/services/net.magicterra.testkit.scene.SceneProvider` | `common/src/main/resources/META-INF/services/...`（内容=common 版 FQCN） | **删除** |
| — | `scripts/testkit/expected-scenes-fabric.txt`（8 名,同 neoforge 清单） | — |
...
**Files:**
- Create: `common/src/main/java/net/magicterra/agent/bot/sim/{AgentFakePlayer,ServerAgentBodies,ServerPlayerAvatar,ServerAgentDriver,ServerAgentManager}.java`
- Modify: `neoforge/src/main/java/net/magicterra/agent/neoforge/sim/{ServerPlayerAvatar,ServerAgentDriver,ServerAgentManager}.java`（改写为 shim）、`neoforge/.../AgentDriverNeoForge.java`（init 安装 neoforge 工厂）

**Interfaces:**
- Consumes: 现 neoforge sim 三类源码（逻辑逐行搬,唯 FakePlayer 类型改 `ServerPlayer`）；NeoForge `FakePlayer`/`FakePlayerFactory`（shim 与工厂用）。
- Produces: `ServerAgentBodies.install(BodyFactory)`（loader init 一次性,重复 install=抛）;`BodyFactory { ServerPlayer shared(ServerLevel); ServerPlayer unique(ServerLevel, GameProfile); }`;common `ServerPlayerAvatar.create/createUnique` 经 seam 取身体;common `ServerAgentManager` 是唯一注册表（shim 全委托,`AgentDriverNeoForge:91` 的 tickAll 经 shim 落同一张表）。
...
- [ ] **Step 3: neoforge shim 化**（同 FQN extends common 版;协变 `FakePlayer fakePlayer()` 强转返回;静态工厂返回 shim 类型;`ServerAgentManager` shim 静态全委托;`AgentDriverNeoForge` init `ServerAgentBodies.install(...)` 走 FakePlayerFactory）
...
**Files:**
- Create: `common/.../bot/testkit/{AgentDriverScenes,SimProbes}.java`、`common/src/main/resources/META-INF/services/net.magicterra.testkit.scene.SceneProvider`
- Delete: `neoforge/.../testkit/AgentDriverScenes.java`、`neoforge/src/main/resources/META-INF/services/net.magicterra.testkit.scene.SceneProvider`
- Modify: `common/build.gradle`（+:testkit-common,镜像 mc-testkit 内部 common→common 消费法）、`AgentGameTestServer.java`（probeSwing/probeHurt 改一行 delegate→SimProbes）、`AgentGameTestSupport.java`（grantWaterEffects 同）

**Interfaces:**
- Consumes: Task 1 的 common sim;现 AgentDriverScenes 全文（含 porting map/裁决注记 javadoc,逐字搬,唯 import 与 probe 调用点换）。
- Produces: `SimProbes.probeSwing(ServerLevel, ServerAgentDriver, ServerPlayer, ItemStack, int,int,int)`/`probeHurt(ServerPlayer, boolean)`/`grantWaterEffects(ServerPlayer)`（签名 common 化;neoforge 原静态签名不变,内部 delegate + 类型收窄由 shim 协变保证）。
...
- [ ] **Step 5: Commit** `refactor(testkit): ad.* scenes + probe helpers to common, SceneProvider service relocated (dup-gate verified)`

### Task 3: fabric 接线（dogfood 配置 + hooks + 清单）

**Files:**
- Modify: `fabric/build.gradle`（+:testkit-common 两行依赖,transformProductionFabric;+runDogfoodServer 配置镜像 neoforge/build.gradle:128-140;configureEach 名字守卫加 `dogfoodServer`）、`fabric/.../AgentDriverFabric.java`
- Create: `scripts/testkit/expected-scenes-fabric.txt`（8 个 ad.* 名 + 头注释同 neoforge 清单格式）

**Interfaces:**
- Consumes: neoforge 侧接线全样（`AgentDriverNeoForge.java:33-92`:TESTKIT_AUTORUN 双门、applyGameTestBaseline、TestkitCommon.onServerStarted(server,"fabric")、onServerTick、tickAll）;fabric 事件 API（`ServerLifecycleEvents.SERVER_STARTED`/`ServerTickEvents.END_SERVER_TICK`,AgentDriverFabric:24-26 已有挂点）。
- Produces: `:fabric:runDogfoodServer`（run-dogfood 目录,ephemeral 端口,testkit.autorun=true）;fabric init 安装 `ServerAgentBodies.install(AgentFakePlayer 工厂)`（unique=profile 键控,shared=level 缓存,语义对照 FakePlayerFactory javadoc）。

- [ ] **Step 1: build.gradle**（依赖+运行配置+名字守卫;fabric 守卫先例在 :106）
- [ ] **Step 2: AgentDriverFabric 接线**（镜像双门与调用序:tickAll 在 TestkitCommon.onServerTick **之前**,与 neoforge:91-92 同序;工厂 install 放 onInitialize）
- [ ] **Step 3: 清单文件**
- [ ] **Step 4: 结构门**：`python3 scripts/testkit/t0.py --loader fabric --wall 540 --run-task :fabric:runDogfoodServer --results fabric/run-dogfood/testkit-results.jsonl --expect-file scripts/testkit/expected-scenes-fabric.txt` 跑通到 done footer:13 记录全在场（PASS/FAIL 皆可,本门只验组装:名字齐、金丝雀三员判定正确、无 SWALLOWED/TRUNCATED/MISSING-EXPECTED）。场景级 RED 本 Task 不裁,留 Task 4。
...
**Files:**
- Modify: `common/.../bot/testkit/AgentDriverScenes.java`（仅 javadoc:fabric golden 双记）;（视裁决）`scripts/testkit/expected-scenes-fabric.txt` 注记
...
**Files:**
- Modify: `mc-testkit/README.md`（loader 矩阵:两 loader 正门命令并列）、`TODO.md`（P1.6 条目）;契约 v0 无语义变更不动。
...
4. **风险入案**：AgentFakePlayer 连接 stub 不全→场景崩（Task 1 骨架+Task 3 首用分离,崩溃归因窄）;service 双注册假 RED（重名门自证,Task 2 Step 4 预案）;fabric 事件序 END_SERVER_TICK 与 neoforge Post 的场景执行位差→指标漂移（Task 4 ×3 定性 + BLOCKED 协议）;common→testkit-common 依赖若引 loom 解析问题→镜像 mc-testkit 内部消费法（Task 2 Step 3 编译门早失败）。
