...
> 分支:`feature/executor-permove-ascend`,tip = `9eb0524`。**未合 master**(B1 未过 live 验证,flag 默认 OFF)。
...
### 2. B1-1 已提交 `6d9ec88`(结构改动,行为 no-op)
...
### 3. B1-2 已提交 `c426929`(task#82 dead-zone 修,核心交付)
...
- 提交链:`53ce8de`(Unit1,上会话)→ `6d9ec88`(B1-1)→ `c426929`(B1-2)→ `9eb0524`(docs)。
...
- **git**:分支 `feature/executor-permove-ascend` tip `9eb0524`;master 在 `f67caaa`(gap#81)。
