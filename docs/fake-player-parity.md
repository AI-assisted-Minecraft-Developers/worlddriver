...
`ServerPlayerAvatar.mirrorPlayerTick()`（`:769`）**曾经**是通道(二)尾巴的**人工部分重实现**。它的注释里列了镜像了什么、以及故意不镜像什么。**这份清单是手维护的**——这就是为什么下面绝大多数「只能近似」条目都是这一段的推论，而不是各自独立的缺陷。

**2026-09-14 起**：清单删了。`JoinedBody.pump` 直接调 `doTick()`，通道(二)原样跑；通道(三)里 `handleMovePlayer` 的尾巴（落差、已知位移、冲量上下文、`ChunkMap.move`）由泵补上；尾巴里的移动统计不补，`ServerPlayer.travel` 已经算过。仍然不在的是通道(二)的**入口**：泵由驱动的步调用，不由连接 tick 调。§4 里凡是这一段推论出来的条目，逐条标了现状。
...
| T6 | `invulnerableTime` 永不递减（新发现 N4） | **已修（2026-09-14）**：泵先调 `ServerPlayer.tick()`，那一行就在里面。`wd.bodyParityCensus` joined 列置 20 后逐步读到 `19,18,17,16,15`；只按真实服务器 tick 采、不驱动时仍是 20（`serverInvulnerableTime`），因为关卡实体循环进 `JoinedBody.tick()` 不做事。以下是原文。`LivingEntity.baseTick():468-470` 显式写着 `&& !(this instanceof ServerPlayer)`，只有 `ServerPlayer.tick()` 减它——而 tick 是空的。补法：镜像那一行 | 依赖先解开免伤（见 X2-3）。验证：连打两下，断言第二下在 i-frames 内**不掉血**、10 tick 后能掉。**这条与 T1/X2-3 互锁，不能单独验收** |
| T7 | `takeXpDelay` 永不递减（新发现 N5） | **已修（2026-09-14）**：`Player.tick` 随 `doTick()` 跑。`wd.bodyParityCensus` joined 列扔 3 颗，`totalExperience 0→9`、`takeXpDelay=0`、20 步后未吸收 0 颗（此前 `0→3`、剩 2 颗）。以下是原文。`Player.tick():292-293` 是唯一减它的地方；`ExperienceOrb.playerTouch:234` 只在 `takeXpDelay == 0` 时给经验，`:239` 又设回 2 → **这具身体一辈子只吸一颗经验球**。补法：镜像那一行 | 在脚下扔 3 颗经验球，等 10 tick，断言 `fp.totalExperience` 等于三颗之和。缺陷存在时只有一颗 → 红。**不能只扔一颗**，一颗恰好是缺陷放行的那颗 |
| T8 | `updatePlayerPose()` 从不运行（新发现 N6） | **已修（2026-09-14）**：随 `doTick()` 跑。`wd.bodyParityCensus` joined 列持续潜行 5 步后读到 `CROUCHING/1.50`（此前 `STANDING/1.80`）。姿态一活，`wd.waterStepDownFloat` 头顶的睡莲就把身体压成 0.6 格高的游泳姿态、从墙下钻了过去，那一条的钉住原来靠的正是这个缺陷，布景已改。以下是原文。`Player.tick():363`（定义 `:447`）。碰撞箱永远是站立的 1.8 格。补法：镜像 | 造一个 1.5 格高的通道，命令潜行穿过，断言身体到达对面。缺陷存在时卡在入口 → 红。**不能只断言 `getBbHeight()` 变小**——那只测了字段，没测它对碰撞的影响 |
...
| T10 | 移动统计从不累加 | **对 `JoinedBody` 不成立**：`ServerPlayer` 覆写的 `travel` 自己调 `checkMovementStatistics`，旧 `step()` 直调 `fp.travel` 时就在累加（`wd.bodyParityCensus` 的 joined 列 `walk_one_cm 0→227`），2026-09-14 起经 `doTick()` 跑到同一个 `travel`。只对已删的假人成立（它的 `awardStat` 是空覆写）。泵**不能**再补 `handleMovePlayer` 里那一次：同一段位移在 `travel` 里已经算过 | 走 100 格，断言 `Stats.WALK_ONE_CM` 大于 0。缺陷存在时恒 0 → 红。现成读数：`wd.bodyParityCensus` 的走路探针（`walk_one_cm 前→后`），重复计数时约为位移厘米数的两倍 |
...
| T16 | 装备属性同步 | **2026-09-14 起是 vanilla 的**：泵经 `doTick()` 跑到 `LivingEntity.tick` 的装备变更检测，手写的 `EQUIP_MEMO` + `syncEquipmentAttributes()`（原 `:668-711`，gap #46）随之删除。`wd.gearScope` 在一次普通 `step()` 之后读 `ATTACK_DAMAGE`/`ATTACK_SPEED`/`ARMOR`，前两个有断言 | 换上钻石靴断言 `Attributes.ARMOR` 变化；脱下断言回落。两条臂 |
| **T17** | **水底起跳：闸问错了量（新发现 N21，行号锚 `af040f1b`）** | **现状**：判据在 §6.8.2 那一轮改成了 vanilla 的流体高度复合谓词，只是拿 `footed`（`soleOnSolid > 0`）代替 `onGround()`；2026-09-14 起连这个替身也删了，整个闸随 `doTick()` 跑在 `LivingEntity.aiStep` 里。以下是原文：跳跃闸是 `soleOnSolid(...) > 0`（`ServerPlayerAvatar.java:1054-1055`）→ `fp.jumpFromGround()`（`:1087`），**只问脚底贴没贴住实心，从不问水有多深**。vanilla 的判据是流体高度：`LivingEntity.aiStep` 的 jump 分支里 `bl && (!onGround() \|\| g > h)` → `jumpInLiquid`（+0.04），只有 `onGround() \|\| (bl && g <= h)` 才 `jumpFromGround()`，其中 `h = getFluidJumpThreshold()`（`Entity`：`eyeHeight < 0.4 ? 0.0 : 0.4`，玩家 = 0.4）。补法：把 `footed` 改成 vanilla 那个复合谓词，`getFluidHeight(WATER)` 在 `step()` 里是活的（`fp.baseTick()` 每 tick 跑） | **实测已在手**（§6.8）：同一格 `64,61,60`、同一 tick、同一 `支=stepUp`，服务端首 tick **+0.420**、客户端 **+0.035**。验收臂见 §6.8 末尾的 `bottomedDeep` 预登记：踩池底、按住跳，断言**没有**单 tick 抬升 > 0.3。今天红，修好转绿。**注意 `wd.buoyantJumpStaysABob` 现有的 `bottomed` 臂断言的是 vanilla 没有的行为，必须一起改，否则 T17 一修它就红** |
| **T18** | **起跳没有冷却（新发现 N22）** | **已修（2026-09-14）**：闸不再是驱动器自己的。泵经 `doTick()` 跑到 `LivingEntity.aiStep`，`onGround` 与 `noJumpDelay` 两个齿都是 vanilla 的；`lastJumpTick` 删了，`dbgLastJumpTick` 改读 `JoinedBody.jumpFromGround` 覆写记下的游戏时间。原文：vanilla 每次 `jumpFromGround()` 后置 `noJumpDelay = 10`，并以 `noJumpDelay == 0` 为闸；`ServerPlayerAvatar` 写了 `lastJumpTick`（`:884` 声明、`:1058` 写入）却**只被一个调试读数读**（`:881` `dbgLastJumpTick`），**从来不是闸** | 按住跳 30 tick，断言 `jumpFromGround` 触发次数 ≤ 3。缺陷存在时每 tick 一次 → 红。**必须和 T17 分开验收**：T17 的臂在水里，这条的臂在干地上，否则两个自变量混在一起 |
...
| # | 条目 | 差在哪一格 | **哪些测试会因此撒谎** |
|---|---|---|---|
| A1 | 手写物理 | **2026-09-14 起不再手写**：`step()` 只交输入，`JoinedBody.pump` 跑 `ServerPlayer.tick()` + `doTick()` + `handleMovePlayer` 的尾巴；`LocalPlayer.aiStep` 里服务端不跑的那一半（潜行与用物品降速、四角推出方块、冲刺停止规则、水里潜行下沉）移植到 `JoinedBody.aiStep`。原来的五个近似点（跳跃 0.42、冲刺前冲 0.2、水面上浮 +0.04、潜行下沉 −0.04、每 tick 重播 `speed`）随手写物理一起删除。还剩的近似：起跑规则（驱动直接置冲刺位，与客户端身体一致）、创造飞行、跳键开鞘翅、骑乘跳没有移植；`handleMovePlayer` 的移动校验（`moved wrongly`）不跑 | **任何断言「真玩家过得去／过不去」的跑酷、跨沟、爬坡场景**。现有 `wd.physicsParity`（`WorldDriverCoreScenes.java:591`）和 `wd.waterPhysicsParity`（`WorldDriverWaterBankScenes.java:144`）钉的只是四个标量，不是一致性 |
| A2 | `mirrorPlayerTick()` 是手维护清单 | **2026-09-14 起消失**：清单删了，泵直接调 `doTick()`，通道(二)的尾巴原样跑，子集边界不再由人决定。原文：`:769-783`，它**永远**是通道(二)的一个子集 | 任何依赖「玩家每 tick 会发生的事」的模组行为。还剩的差别只在节拍：`doTick()` 由驱动的步调用而不是由连接 tick 调，所以通道(二)按**步**推进，驱动不步这具身体时它就不走（NeoForge 的 `PlayerTickEvent` 每步一次） |
...
| A7 | 菜单是手工造的 | **2026-09-14 起不再手工造**：服务端身体只剩 `JoinedBody`，原版 `openMenu` 自己开菜单；`openStationMenu` 在两个 loader 的专用服闸上一次都没触发（其中 16 条要开菜单的场景全过），删除。`useBlock` 在原版拒开时不再补一个——剩下能触发的只有原版故意拒的情形（潜行且手里有方块、箱子被挡），补上就是给身体开了玩家开不了的菜单。以下是原文：`openStationMenu`（`:580-594`）自己 `new` 一个菜单再赋给 `fp.containerMenu`（`:585`），滚动 id 1..99；A/B 的 `openMenu` 直接返回 `empty`（`AvatarFakePlayer.java:82`）。C 是唯一走 `super.openMenu` 的（`JoinedPlayerBodies.java:237-239`） | **所有模组容器的打开路径**。`Avatar.java:180-183` 的 javadoc 已经承认「服务端路径退化成一次优雅的超时」。这一条直接决定了模组包测试能摸到多少东西 |
...
| A10 | **饥饿**：`foodData.tick()` 不镜像 | **2026-09-14 起大半是 vanilla 的**：`foodData.tick()` 在 `Player.tick` 里随 `doTick()` 跑，自然回血跟着回来；跳跃的消耗走真的 `jumpFromGround`，游泳、水下行走、冲刺的消耗走 `ServerPlayer` 覆写的 `travel` 里那次 `checkMovementStatistics`。攻击与挖掘的消耗取决于那两个动词各走哪条路（见 T11、N23），这一轮没动。身体仍然免伤，所以是一具会饿、饿不死的身体。以下是原文：`mirrorPlayerTick()`（`:769-783`）把 `foodData.tick()` 列在**故意不镜像**的一栏里。而且光镜像那一行还不够：饥饿消耗来自 `causeFoodExhaustion` 的**每一个**调用点（跳跃 `Player.jumpFromGround:1516-1523`、冲刺、攻击、挖掘），而这些点 avatar 全都绕过了——所以这条不是「补一行」，是「补一族」 | 任何断言「撑不撑得住 / 走得动走不动」的长程场景。`JourneyRig.java` 的类注释已经诚实写过：饥饿、落差、溺水、怪物威胁都在这条轨道观察不到。**这一条和 X2-3 是同一个包**：要一具会死的身体，就必须同时有可关的免伤、`invulnerableTime--`（T6）、饥饿、重生，缺一条都会得到一具「半死不活」的身体，比全免伤更难解释 |
...
| **N22** | **起跳没有 `noJumpDelay` 冷却** | vanilla `LivingEntity.aiStep` 置 `noJumpDelay = 10` 并以它为闸；`ServerPlayerAvatar.lastJumpTick`（`:884`/`:1058`）只被 `dbgLastJumpTick`（`:881`）读，不是闸 | 同一个闸的第二个齿。**2026-09-14 起闸是 vanilla 的，这一齿随之在位**；仍没有单独的实测。归 T18 |
...
| **N21** | 水底起跳给了 0.42 | `ServerPlayerAvatar.java:1054-1055` 的闸是**驱动器自己写的**。`JoinedBody` 走同一个 `step()`，同一个闸。**2026-09-14 起驱动器不再有自己的闸**，泵里跑的是 vanilla 的 |
| **N22** | 起跳没有 10 tick 冷却 | 同上，同一个闸，同一天随之消失 |
...
| 跳跃没有 `noJumpDelay` 十 tick 冷却（**T18 / N22**） | **服务端特权** | 同一个闸的第二个齿，本例未单独证实（2026-09-14 起闸是 vanilla 的，见 §4 T18） |
...
#### 6.8.3 预登记命中了，但**别把它读成「T17 一修这一级就绿」**（2026-08-22，协调者转述的实测）

上面预登记的是「掉，且先掉在**水域段**，第 3 级（WOOD）最可能首先受影响」。
**集成服真梯那一趟第 3 级确实死在这个闸上**（协调者转述，非我亲测）：
客户端 `LocalPlayer` 浮在沼泽水面上挖岸壁，正是 `ServerPlayerAvatar.java:1054` 那条闸描述的形状。

**但这条读数有一个边界，不写下来一定会被误用**：

| | |
|---|---|
| T17 修的是什么 | **只让服务端那具身体变诚实**——把「踩没踩住实心」换成 vanilla 的流体高度谓词。§6.8 末尾已经把这条定性成「服务端身体有特权」，修法方向是**收紧**，不是补能力 |
| T17 **修不好**什么 | **客户端那一级不会因此转绿。** 客户端缺的是上层计划，不是起跳物理：`common/src/main/java/net/magicterra/worlddriver/bot/movement/WalkerTickClimb.java:1039` 的注释**逐字预言过**这个形状（「bobs forever hand-mining the bank」） |
| 谁在查 | **那半边归协调者，parity 不动 `bot/movement/**`**（那本来也不在 §0 的产权里，同 T19 的处理） |

> **为什么这一段值得单列**：T17 的验收判据是 §6.8.2 那三条臂的读数，**不是「第 3 级转绿」**。
> 一条真梯的红恰好落在被修的那个闸上，是**迄今为止 T17 最强的一条独立证据**——
> 但如果有人把「第 3 级」写进 T17 的验收条件，那条件**在 T17 完全正确的情况下也会失败**，
> 然后 T17 会被回滚掉。**同一格里坐着两个缺陷，修掉其中一个不会让那一格转绿**，
> 这正是 §6.5「N19 单列：一个缺陷被另一个缺陷完整遮住」的同一形状，方向相反。
...
- **空闲。** 被驾驶时关卡实体循环只记一笔（`DrivenPiglin.tick`），没人步进的 NPC 与没有驱动器的服务端身体一样整个停住。
  2026-09-15 拍板不替它们空步（设计文档 §6 第 5 条），所以这不是待修的差异，是契约。
