...
Every entry documents a Walker-execution branch family that the stagewright arenas
...
| `pathfinderFrontierCommit` wait/arrive branches | `Walker.frontierHoldOrArrive`, `WalkerTickSearch` | Needs a search that dies at an **unloaded-chunk frontier**; stagewright slots force-load their whole grid window, so no frontier exists. | Live long-distance journeys (wjourney runs routinely cross load borders). |
