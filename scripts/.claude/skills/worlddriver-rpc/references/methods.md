...
| `mc.bot.follow` | `entityType?` or `name?` (≥1 req), `radius?` (1–16), `maxIdleTicks?`, `awaitMs?`; `route?` = the same object as `mc.bot.goto` minus `via`, and with `mode:["fly"]` / `leash.entity` rejected (a follow already tracks its entity) | follow an entity; recomputes ~1.5s. |
