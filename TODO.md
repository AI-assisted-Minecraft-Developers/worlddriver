...
## 2026-07-16 task#85 suite-integrity P0 落地 + 全量基线重建

- ✅ **P0 四件套已合 master**:①in-game JSONL manifest(76362e5,registered/enter 双record,`gtOnlySkips` 埋点处发 enter 探针);②`scripts/gt_reconcile.py` 对账门(574639a + a812103,registered-vs-entered SWALLOWED/DRIFTED 双向门 + BUILD/required verdict,9 条内嵌 self-test);③`--audit-source` 源码审计模式(d56c54f + 8bcdfe9,每个 `@GameTest` 必须在自己方法体内挂 guard,防漏埋);④`scripts/run_gametests.sh` 统一入口(288b396,PID sweep→世界清→`timeout --kill-after=30`硬顶→对账验收,永不信 mod reporter 的 `TOTAL:` 行)。
- ✅ **Step1 全量实测(HEAD=288b396,`scripts/run_gametests.sh` 单次跑完,3m51s,无需重跑/无挂死)**:`registered=130 entered=130` → **SWALLOWED 名单为空**(#85 在当前 HEAD 不复现,门依然武装 armed;#85 原调查候选 `ascendMovementNoop`/`ascendDeadZoneWatchdog`/`diagonalAscentSpeed` 三者本次全部正常 entered,无一被吞)。`build_success=False required_failed=True` → **VERDICT: RED**,4 required 失败:`serveravatargearscopeprobearena` / `deepwatercrossarena` / `descentyawarena` / `agentrpcsmoke`;1 optional 失败:`vineoverwaterclimbarena`。⭐**判类=全部落在已文档化的 task#48「shared-body 跨 arena 彩票」族**(见 `AgentGameTestServer.java:2340-2367` 源码注释实证:`FakePlayerFactory.getMinecraft(level)` 全 arena 共享一个 FakePlayer,导致失败 SET 逐 run 漂移,该注释点名的历史受害者含 `entityLeashRepath`/`deepwaterClimbout`/`buoyantWall`/`descentOvershootResync`/`gearScope`/`agentRpcSmoke`/`horizonArena`,与 TODO.md 07-14/07-15 历史记录的"leash/deepwaterclimbout/gearscope/descentYaw 轮流当受害者"完全同源);本次 4+1 名字全部是该族已知成员的具体实例(gearscope/descentYaw 为文档反复点名的确定成员;deepwatercrossarena/agentrpcsmoke 为 `AgentGameTestServer.java` 源码注释直接点名的同族漂移名单成员) → **零真正新名,按"回归只加失败不换人"审计规则判定为已知彩票,非回归**。suite-wide 隔离修复(task#48 后续)待专门 test-framework rework,不在本次 P0 范围。
- ✅ **Step2 被吞候选显式基线(`AGENT_GT_ONLY=ascendMovementNoopArena,ascendDeadZoneWatchdogArena,diagonalAscentSpeedArena GT_TIMEOUT=1800 scripts/run_gametests.sh`,19s,`registered=130 entered=130 build_success=True required_failed=False` → **整体 VERDICT: GREEN**,`exit=0`)**:三个点名的候选者本次**全部有 manifest enter 记录且真实执行**(非早退 no-op)——①`ascendMovementNoopArena`:**PASS**(`step=ARRIVED reachedTop=true ctxAllocated=0`);②`ascendDeadZoneWatchdogArena`:**PASS**(纯逻辑断言测试,5 阶段全过——PREP→giveUp 拍 RUNNING→UNREACHABLE→终态清空重 PREP→active-dig 豁免→post-dig 时钟满额重置→monotonic dy 高水位不被同 apex bob 重置,`helper.succeed()` 无异常);③`diagonalAscentSpeedArena`:**PASS**(`step=ARRIVED reachedTop=true diagBps=3.00 ascSprint%=43`)。**这是被吞名单(本次为空)对应候选的真实 GREEN 基线,供 P1 dogfood 迁移直接引用。**
