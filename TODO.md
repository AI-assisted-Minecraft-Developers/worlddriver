...
| 🟡 判：验（修法已落 `c34027cd`，ladder-15 在跑） | J104 | 真梯 ladder-14 断在第 11 级（浇筑站位差 2 格、只打得到 south 面）。真因：收紧半径那条腿与 re-pick **共用 `tries` 预算**，没用的 re-pick 先花光，而幂等证据把三轮白走折叠成一条藏住了它。改法两笔：证据正向编号 + 从收紧那道门删掉 `tries > 0`。⛔ 验收只认 `cast.landsElsewhere.N` 且 **N ≥ 4**（`approach = 3 - tries + 1`，N≥4 ⇔ `tries≤0`，旧门那时是关的）；`rePickBlocked` 开火而 N 只到 3 = 无场合，不许记成通过 | 我 |
