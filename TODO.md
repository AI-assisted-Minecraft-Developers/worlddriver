...
## 2026-07-18 🚧 P4c Server 巨类按 family 分波迁移(进行中,branch `feature/executor-permove-ascend`)

- **✅ wave 6 (P4c-T1) Station 波** `2003a64`:craft/smelt/recipe/observe **15 迁**(59→44),新 provider `AgentDriverStationScenes`;driver 载体走 `ServerAgentDriver.createIsolated`;furnace/grid 状态机手塞 `containerMenu`(#64);dogfood 71→86 ×2×2 恒等,legacy reconcile 44/44 GREEN。
- **✅ wave 7 (P4c-T2) Scheduler 语义矩阵波** `a4977fd`:#54P1 回归卫士 **11 迁**(44→33),新 provider `AgentDriverSchedulerScenes`。名单=retreatGateMatrix / walkerTerminalReportMatrix / antiSuffocateShouldTriggerMatrix / chainEpisodeCancelMatrix / combatGraceMatrix / frailBlockedMatrix / urgentBidMatrix / duskSecureHeldProcessLifecycle / cancelRouting / manualSlotGraceMatrix / nearestFirstScanMatrix。**139 矩阵行逐条一对一**(每个 legacy `BiConsumer check` helper 字节拷,场景喂 `(ok,msg)->ctx.fail`);9 纯逻辑(1 tick 解析,无 world/avatar/walker)+ 2 触 world:`ad.retreatGateMatrix`(真 Skeleton(RangedAttackMob)+Zombie 只作 Threat 引用**不** getEntitiesOfClass 扫→**无需 entity-visibility await** + `enclosed` 几何腿)、`ad.duskSecureHeldProcessLifecycle`(26 行 matrix + gap#75-b 真 `BunkerProcess` 挖到 SEALED 的 world 腿,`driver.tick()` 同步跑体内,**零 `level.tick()`**)。删 11 twins + 12 孤儿 helper + 9 孤儿 import(`antiSuffocateSuffocatesBlockMatrix`/`antiSuffocateWaterNotSuffocatingArena`/`serverMineCanopyRadiusArena` 是中段幸存者,保留)。**四门绿**:①compile+双 build SUCCESSFUL;②neoforge dogfood ×2 恒等 72/73s;③fabric ×2 恒等 + fabric==neoforge 字节恒等 70/69s(97 场景,11 新全 PASS,仅既有 canary+2 optional sensor 非 PASS);④legacy reconcile `registered=33 entered=33` 0-swallowed **"All 33 required tests passed"** GREEN 25s 无 livelock,删除名零复现。无 flaky、无调阈、无 withRequired(false)。
- **残余**:wave 8 (P4c-T3) Survival+Avatar 18 名(33→15,underwaterBase 挂死惯犯=escape-hatch 候选);wave 9 (P4c-T4) Process 核心 15 名(15→0)+ 三类全删(含 3 处 `level.tick()` 雷直译 bounded await);P4c-T5 验收。
