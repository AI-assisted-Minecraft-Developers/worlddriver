...
| 🔴 判：查（12 级现在的死因，第 12 趟到场） | J89 | **偏柱是塔垒出来的，不是走出来的**：`cast8.raiseTo` 走到 `距 2,20 0 格`，而 `cast8.raisedY = 59/59（停在 0,20，指定柱 2,20）`。最终失败时身体在 `-2,66,19`，离目标柱 6 格远 6 格高。⇒ 先查 `raiseInColumn` 的塔，⛔ 别先改 `walkToColumn` 的容差——那条修的是另外四条腿，够不到 `cast8` | 我 |
| 🔴 判：查（同上，紧邻） | J90 | `raiseTo3D` **这一趟轮到了并且把柱选对了**（`cast7.raisedY = 58/58 同一柱`），随后被 `JourneyRamp` 挪走（`ramp.rampedY = 57/58，不是同一柱`）。⚠️ 我写过「三维目标那条腿一次都没轮到过」——那是从第 10 趟一个样本外推的，**已证伪**。⛔ 不许再按「放宽 `raiseRowTooHigh` 那道闸」去改 | 我 |
