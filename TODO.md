...
### ⬜ 下一个人从这里接（两条候选，都要先写下判据）

1. **`scoop`：settle 之后再瞄一次。** 现在是 `aimAtBlock → settle(2) → 读 aimedAt`；
   改成落定后重算角度（`aimAtBlock → settle(1) → aimAtBlock → settle(1) → 读`），
   代价是每次装桶多一 tick。判据：`.fromHere` 与 `.aimsAt` 两行的眼睛 y **相等**。
2. **`riseToTakeItBack` / `visibleSourceNear`：别在半空里问。** 前置判断先等 `onGround`
   再问，否则「看得见」是一个身体已经离开的位置的答案——这一趟它正是这样答的「看得见」，
   于是升排整趟没跑（本趟无 `recover9.rise`）。

**不要**只把 `visibleSourceNear` 换成 `aimedAt` 那条射线就算完：两条射线名义上是同一条线
（都从 `getEyePosition()` 出发指向格中心），换了也挡不住「问的时候身体在别处」。
