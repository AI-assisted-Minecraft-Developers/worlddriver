...
| 🔴 做（现在的下一笔） | J124e | **切短第 5 腿**，不是「换掉走不到的路点」——目标可达（一次 `reached=true steps=36 end=74,41,95`），死因是搜索代价：**全预算档 4 次里 3 次撞满 `maxNodes=100000`**，唯一解出的那次花了 88646 节点（预算的 89%）。⇒ 在 wp4 `72,42,85` 与 wp5 之间插中继点让每跳变短；`74,41,95` 可直接当新的 wp5（实测可站、实测到达过）。⚠️ 挑中继点要地形图 ⇒ 依赖 J124i | 我 |
