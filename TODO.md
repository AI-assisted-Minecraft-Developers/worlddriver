...
| 🟡 第二笔修法在验（壁龛天花板 y=63 以上按 `LIP_TAX` 加价）；第一笔「事后走回来」实测无效已回退。⚠️ 真梯第 12 级同族死法（身体 `-2,66,19` 浇 `4,60,20`）⇒ 这是真梯阻塞不是排练独有 | J97 | 抬升腿把身体送出井口站到**地表**：`recover8/9.rise.raiseTo.arrivedY 64（净升 7~8），脚下=grass_block`（壁龛地板 y=56、井口 y=66），随后隔着 9.6 m 收水失手。机制在产码注释里：`walkToColumn` 用 `Goal.XZ`，忽略 Y，而指定柱的地表格也属于那一柱、且是井底出发最便宜的一格。**浇侧已修**（`JourneyPour:232` 重来那趟改 `Goal.Near`，且点名不能用 `Goal.Block`——`Walker.snapGoalToStandable` 会吸回地表）；**收侧故意排除**，理由是别抢掉 `buildTo(exactRow)` 的 turn（[[a-fix-that-never-gets-its-turn]]）。⛔ 我先前写的「判柱不判排」**是错的，排判了**：`scoopRowHigh` 就在 `JourneyPour:199` 且逐字开了火（[[a-guard-i-assumed-absent-was-running]]）。真正坏的是那条补救的**前提**：它拿 `recover6 59/58`（高 1 排、身体已在井里）验的，这里被交的是高 4~5 排、身体在井外，于是 `ramp.laid 0/4 级（OUT_OF_REACH 2,56,18）`——[[zero-of-one-is-not-a-smaller-zero-of-nine]]。修法按**量级**分流不按侧分流。⛔ fl1 死的是 `water9.lift`（另一个调用点，同形），先读调用图再落。判据 `j97-raise-leaves-the-shaft.md` | 我 |
