...
| 状态 | # | 事 | 归属 |
| 🟠 待验（等真实整合包客户端） | J133 | **带修饰键的 keybind 那条路径没有闸覆盖。** `mc.client.input.keybind` 在发原始事件前临时清掉绑定的修饰键，而闸里造不出这个场合：原版没有带修饰键的绑定，场景层又不碰客户端类。验收读数来自装了 `ALT+Y` 绑定的整合包客户端，且要在窗口有焦点时取 | 我 |
