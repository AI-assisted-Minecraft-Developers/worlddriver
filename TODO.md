...
| 🟢 回测过（3/3 逐字相同），闸认证在跑 | J93 | `浇不到 4,57,19`，13 趟占 4（#6#8#9 + noaim2）。真因：`finishTheFlight` 的 `down()` 判格号不判身体，脚一进末路排就跳过「等落地」，浇筑于是在身体下坠 0.92 格的途中决定瞄准、又低 0.69 格开火。修法 `JourneyStairwell` 改判位置（`SETTLED_SLACK`）。⛔ 「回落无视守卫」（`at = settled != null ? settled : planned`）是**另一条真缺陷**，下一轮单独落——同时落会让本轮主测量 `picks.3` 消失。⚠️ `finishTheFlight` 是共享的（`JourneyLandingScenes` 的 drop/lip 两臂直接断言 `flightLastStepSettled`／`flightLastStepEnd`），所以排练回测绿了**只到「回测」**——要跑专用服闸（命令以 `AGENTS.md` 为准）才算认证。判据 `j93-fix-preregistration.md` | 我 |
| 🔴 判：查（降级——三趟逐字复现但三趟都幸存，不是死因） | J96 | `cast8` 起塔被自己浇的水冲：`climb.0.stalled stuck (no Y gain in 60t: placed=0, shortJumps=15)`、`state onGround=true inWater=true y=57.00`、`washedOffUpstream 还在喂它的水源：4, 61, 20`、`verdict 没垒成 —— 落在 0,20 而不是指定柱 2,20`。产码已经把机制诊断完了（`washedOffFed` 明说「上游有源在喂，重试等不到那一刻」），欠的是**处置**：起塔柱被活水占住时该换柱还是先断源。⛔ 不是「不该钉」：`JourneyShaft:229` 的 `climbPinned ? want : towerColumnClearOfTheFlight(…)` 说明钉住整个跳过避让检查，而钉住有正当理由（换柱等于换射线）；后备腿也不算处置——`JourneyShaft:266` 的 `Goal.YLevel(surfaceY)` **只要高度不要柱**，读数逐字兑现（`endedIn 0,20`、`脚下=air`）。⚠️ **降级依据**：修法后三趟 `cast8.raisedY 59/59（停在 0,20，指定柱 2,20）` 逐字复现，而三趟都越过它跑到了第 9 格 ⇒ 是真缺陷、不是死因。我曾拿这条腿的判词当 fix-land-1 的死因写进本表（[[a-verdict-has-upstream-verdicts]]），已改。判据 `j96-tower-washed-off.md` | 我 |
| 🔴 判：改（fix-land 三趟共同死因） | J97 | 抬升腿把身体送出井口站到**地表**：`recover8/9.rise.raiseTo.arrivedY 64（净升 7~8），脚下=grass_block`（壁龛地板 y=56、井口 y=66），随后隔着 9.6 m 收水失手。机制在产码注释里：`walkToColumn` 用 `Goal.XZ`，忽略 Y，而指定柱的地表格也属于那一柱、且是井底出发最便宜的一格。**浇侧已修**（`JourneyPour:232` 重来那趟改 `Goal.Near`，且点名不能用 `Goal.Block`——`Walker.snapGoalToStandable` 会吸回地表）；**收侧故意排除**，理由是别抢掉 `buildTo(exactRow)` 的 turn（[[a-fix-that-never-gets-its-turn]]）。⛔ 我先前写的「判柱不判排」**是错的，排判了**：`scoopRowHigh` 就在 `JourneyPour:199` 且逐字开了火（[[a-guard-i-assumed-absent-was-running]]）。真正坏的是那条补救的**前提**：它拿 `recover6 59/58`（高 1 排、身体已在井里）验的，这里被交的是高 4~5 排、身体在井外，于是 `ramp.laid 0/4 级（OUT_OF_REACH 2,56,18）`——[[zero-of-one-is-not-a-smaller-zero-of-nine]]。修法按**量级**分流不按侧分流。⛔ fl1 死的是 `water9.lift`（另一个调用点，同形），先读调用图再落。判据 `j97-raise-leaves-the-shaft.md` | 我 |
| 🔴 判：查，先补仪器不补修法 | J95 | `走不上楼梯`，13 趟占 4（#5#10 + noaim1#3），死在取第 2 格岩浆的爬腿：`lava2.upStopped 停在 <x>,56,19，起跳格 <x>,58,19=stone（挡着，跳不起来）`，身处 water。不变量是 **y=56、z=19**（楼梯轴在 z=20），x 有 `-2` 与 `1` 两个值。对照：3 趟 PASS 各完成 10 条爬腿、J93 族 3 条爬腿 **0 次 upStopped** ⇒ 与 J93 是两条病。⛔ 身体怎么走到 z=19 **没有读数**——`lava2.up` 只记起点格，要先补「精确坐标+流体+onGround」。判据 `j95-stairs-family.md` | 我 |
| 🔴 判：查（仪器已落，等读数） | J94 | 点不着：`frame.obsidian=10/10`、`portal.slag=门洞六格都是空气`、`light.cellAfter=fire`、`portal.cells=0/6`。#2 与 #7 在七个点火键上**逐字相同**，与 PASS 只差最后两个。真因看不见，因为两个上游读数都在 `clearTheDoorway` 里取、而 `strike()` 随后要走最多 1500 tick，且 `portal.cells` 只印计数不印内容。`4d29b247` 加了 `portal.cellsNow`／`frame.obsidianNow`（贴着打火重读）。判据 `rehearse8-doorway-criteria.md`。⚠️ 为拿样本链的三趟（#8#9#10）**一趟都没走到点火**，`portal.cellsNow` 至今零样本 —— 6/10 到不了那一步 | 我 |
