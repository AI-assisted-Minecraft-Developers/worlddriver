...
| ✅ **已验** | Q15 | 12 级挖掘走位拆自己楼梯：`e5b7c7f8` 的 `NoBreak()` **真梯上验成** —— `forge.carved` 64/67 → **67/67**，楼梯完好走回井底，`stairsBroken` 一次没写 | 我 |
| 🟡 已改待验 | Q19 | **12 级：浇之前手上的东西被换走了**。`cast6.hand=lava_bucket` → 隔 10 tick → `cast6.atUse=dirt`（两端都是），一浇没发生，六条腿之后才在 `recover6` 报「没空桶」。塔握 dirt 把桶挤出快捷栏 ⇒ hold 走背包 swap ⇒ 一个槽两个作者。`78648259`（浇前重拿＋拿不到就停）＋ `8209566e`（任意一格没铸成就停）。**先前那条「桶内容当状态」的判断已推翻** | 我 |
