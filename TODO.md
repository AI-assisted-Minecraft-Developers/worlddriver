...
病因被埋掉）。缺的是**补救**：这正是 J51/J53 在**取水**侧解决过的同一个形状 ——
`visibleSourceNear` 挑一个看得见的目标、`standToScoop` 换一个看得见的座位。
**浇筑侧有射线检查、没有换座位。** 身体在 `y=55` 却要浇 `y=60`，差五格，
从那个座位没有任何瞄法能把流体放进目标格。
⇒ 修法方向：给浇筑一个 `standToPour`（照 `standToFill` 的形状：站得住 + 射线落在目标格），
够不着就换座位再问一次。**不要**去放宽射线判据。
