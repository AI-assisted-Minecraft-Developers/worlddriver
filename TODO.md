...
| # | 判据 | 已验 / 未触发 / 证伪 |
|---|---|---|
| **I1** | 场景 PASS ⇒ 换座位那一支**被点名考过一次**，J53 才可以从 🟡 改 ✅ | |
| **I2** | 控制组 A 红（`staged.visible` 不是 null）⇒ 岸没挡住射线，**是布景没造出陷阱**，不是修法坏了。办法是把岸抬高一格，**不许改判据去迁就** | |
| **I3** | 控制组 C／D 红（`staged.seat` 是 null，或就是脚下这格）⇒ `standToScoop` 在这个几何下给不出座位，**那才是修法本身的问题** | |
| **I4** | E／F 红（`waterFill.reseat` 没写，或写的是「换不了座位」）⇒ 这一支没跑到，J53 **证伪** | |
| **I5** | G 红（用桶那一刻人不在换到的座位上）⇒ 第二次 `Goal.Near` 把身体带走了。这正是我刚落的 `Goal.Block` 再入修法要解决的事 —— 真红了反而**证明那笔修法是必需的而不是防御性的**，下一闸复判 | |
| **I6** | H 红而 E／F／G 全绿 ⇒ 换了座位仍装不上，说明座位**不是唯一原因**，回去读 `waterFill.atUse#2` 的空桶线 | |
