...
| **I5** | G 红（用桶那一刻人不在换到的座位上）⇒ 第二次 `Goal.Near` 把身体带走了。这正是我刚落的 `Goal.Block` 再入修法要解决的事 —— 真红了反而**证明那笔修法是必需的而不是防御性的**，下一闸复判 | ⚪ **未触发**：这一趟挑到的座位离塘 `distSqr=2`，本来就在 `Goal.Near(2)` 里，所以第二次接近**无事可做**。⇒ 这一条今天**没有测到** `Goal.Block` 那笔修法，它仍是「编译＋闸绿」而不是「被观察到起作用」 |
