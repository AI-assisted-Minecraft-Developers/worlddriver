...
| 🔴 判：查（ladder-20 的死因，离第 14 级最近） | J124 | **死因是烧伤 + Piglin，不是路点表。** 身体在 wp4 的到达格 `72,43,84` 上烧掉 7 血（`inFire@65`→`onFire@173`），第 5 条腿开始后**第 8 tick** 被 Piglin 一下 4.0 打死（`death.food = 6/20` 所以零回血）。⛔ **更正我自己**：`death.leg = fortress.wp5.band` 不等于「死于第 5 条腿走不通」——它自己写着「写它时是本段第 173 tick；死在本段第 8 tick…死因链跨了腿」，**这一趟没走过第 5 条腿**。wp5 不存在是待处理的结构问题（见 J124c），不是这一趟的死因 | 我 |
| 🔴 做（下一笔，等 ladder-21 落地） | J124c | **审计只探路点正下方那一柱**，所以它说的是「这一柱没有地板」，不是「附近没有能站的格」——拿它当「无论怎么改都走不到」用是越权。⇒ 给审计加一项：每个坏路点**最近的可站格**（真 3D 距离，不是 dx/dz 界，见 [[a-radius-is-not-a-distance]]），把负结论变成重测的输入。断的是 wp4 `72,42,85` → wp11 `132,43,165` 之间 **100 格**，两端都是实地；wp5/wp9/wp10 是竖井（24 格内无实心面），wp6/7/8 下方 19–21 格有 netherrack **但是岩浆面**——两种坏不一样，别混。验收闸：`fail(optional)` 断言**零个**路点「脚下也空」，今天是 6 个，重烤成功那天自己变绿 | 我 |
| 🔴 判：查（等 ladder-21） | J124a | 走廊火的 **(a) 规划时就在 / (b) 规划后才有** 还没判。仪器已落（`JourneyFireCensus`，双闸绿）。还欠第二件：**逐 tick 的身体格 vs 当前路径节点**，(a) 内部的切角与击退才分得开 | 我 |
| 🔴 做（等 ladder-21 落地） | J124b | 食物是**供给**不是时机。**最小修法：在 FURNACE 级把生牛肉烤了**——级序是 FOOD→BED→**FURNACE**→IRON，那一级手里正好既有 5 块生牛肉又有炉子。生牛肉 3 饱食/1.8 饱和 ×5 = 15；熟牛肉 8/12.8 ×5 = **40**。不加级、不加引擎能力（`mc.bot.smelt` + `SmeltProcess` 早就有），照抄 `WorldDriverJourneyScenes.smeltIron` 的四步：`ensureCarrying(熔炉)` → `makeRoomForAStation` → `SmeltProcess` → **先沉降再计数**（那条注释记着六个铁锭被判成零的原因）。⚠️ 写「进下界还余几块」这种数之前先 grep `JourneyFeed` 的**停食谓词**：吃到 20 就停，烤肉才买到余粮；固定吃 5 口的话只买到饱和度 | 我 |
| 🟡 做（等 ladder-21 落地再改，运行中不编译） | J125 | `JourneyCorridorProbe` 的审计证据行写死「十八个烘入路点」，而 `FORTRESS_WAYPOINTS.length` 是 **17**（腿自己印 `/17`）⇒ 改成算出来的；同文件 line 13 的 javadoc「eighteen cells」同病 | 我 |
| 🟡 做（同上） | J126 | `death.blow` 静默截断：`BLOWS_KEPT = 8` + `blows.remove(0)`，14 记打击只印 8 条且不说。⇒ 补一句「另有 N 次没列出」，照 `JourneyFireCensus` 的做法 | 我 |
