...
| 🔴 做（现在的下一笔） | J124e | **重烤 wp5–wp10**：拿 J124c 的读数把六个坏路点换成各自最近的可站格。断的是 wp4 `72,42,85` → wp11 `132,43,165` 之间 **100 格**，两端都是实地；wp5/wp9/wp10 是竖井（24 格内无实心面），wp6/7/8 下方 19–21 格有 netherrack **但是岩浆面**——两种坏不一样，别混 | 我 |
...
| 🔴 做（现在的下一笔之二） | J124b1 | **在 FURNACE 级把生牛肉烤了。** 级序 FOOD→BED→**FURNACE**→IRON，那一级手里正好既有 5 块生牛肉又有炉子。生牛肉 3 饱食/1.8 饱和；熟牛肉 8/12.8。不加级、不加引擎能力（`mc.bot.smelt` + `SmeltProcess` 早就有），照抄 `WorldDriverJourneyScenes.smeltIron` 的四步：`ensureCarrying(熔炉)` → `makeRoomForAStation` → `SmeltProcess` → **先沉降再计数**（那条注释记着六个铁锭被判成零的原因） | 我 |
| 🔴 做（与上一条捆绑，缺一等于没落） | J124b2 | **第 14 级没有任何进食路径**：`JourneyFeed.eatIfLow` 全工程只有一个调用点（`WorldDriverJourneyScenes.java:2111`，砂砾级）。⇒ 走廊里要加一次 `eatIfLow`。⚠️ 两条必须一起落：只烤不吃 = 熟肉在包里当死重；只吃不烤 = 包里本来就空 | 我 |
| — | J124b-证据 | 两趟归档逐字相同：`gravel.feed.bite0..bite4` 五口把饱食推到 20、收尾写着 `手里=minecraft:air` ⇒ **五份生肉在第 10 级就吃光了**，第 14 级 `death.food=4/20` 不是「没吃」而是「没得吃」。停食谓词是 `foodLevel >= 20`（`JourneyFeed.java:126`），`REGEN_FOOD=18` ⇒ 整条走廊都在回血阈值以下，所以 `回血 0 次` | — |
