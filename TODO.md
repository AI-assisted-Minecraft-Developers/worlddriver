...
#### 第三个读数：同一秒里客户端自己给出两个位置

| 时刻 | 来源 | 线程 | 身体 |
|---|---|---|---|
| 12:26:41 | `[journey] 心跳`（读 `driver.fakePlayer()`，ServerPlayer） | Server | `62,62,64` |
| 12:26:41 | `[pathfinder] search-begin`（读客户端 `LocalPlayer`） | Render | `65, 70, 63` |
| 12:26:42 | 同上，同一来源 | Render | `65, 62, 66` |

**第三行否掉了「端间不同步」这个解释**：客户端自己在同一秒内既报 y=62 又报 y=70。
两个 start 来源，其中一个是**从不刷新的固定值**——`65, 70, 63` 在 12:26:40–42 出现 **43 次**，
坐标一字不差，而且预算是 `maxNodes=100000 maxMs=4611686018427387903`（`Long.MAX_VALUE`，无上限），
与正常那条 `maxNodes=600 maxMs=80` 完全不同档。同族：[[a-lagging-reading-became-the-crime-scene]]、
[[a-retry-that-changes-nothing]]（同一个问题问 43 遍）。

⇒ 排查顺序（**别混成一条**）：

1. **先查那 43 次无预算搜索的 start 是从哪来的**——它是不是缓存下来就不再更新。
   这一条同时解释了「回家 2484 tick 不收敛」和「y=70 从哪冒出来」。
