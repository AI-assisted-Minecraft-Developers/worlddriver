...
（`east` 模腔的射线被门框压住 / 爬不出自己挖的 36 格井）。上一轮 11、12 级也各红过一次并被
判为方差 —— 现在是四趟里三趟红在 10–12 这一段。**把它当方差之前，先数一数。**
...
### ⚠️ 12 级：这一趟第一次抽到 `east` 类模腔，上一轮那条待回测的分支**执行了**，而且**不够**
...
2. **同一个量在 `JourneyEndRungs.march`（17 级去要塞）里还在用位移**：
   `flatDistance(at, now) >= WEDGED_UNDER`，`at` 是这一段的起点，形状一模一样，摆动同样看不见。
   16–20 级还没有 staging 配方、ladder 也从没爬到 17，所以这一轮按边界没碰。**这是第三个现场。**
