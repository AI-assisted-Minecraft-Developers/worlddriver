...
| ✅ 已验（第 9 趟：`drain.0..7` 八次全部「壁龛已排干」，超时不再出现），本行可删 | J70 | 排水那一腿从 `HoldStill(DRAIN_TICKS / 2)` 改成 `HoldStill(DRAIN_TICKS)`，判词印的 200 从此就是等的 200。⚠️ 原行写着「与 12 级回程死因链无关」——**那句是错的**，第 7 趟的 `drain.7` 超时正是那条链的起点（见 J85）。**验收**：下一趟 12 级 `drain.7` 是否翻成「已排干」 | 我 |
...
| 🟠 判：先补测量（等第 10 趟确认候选集形状，再选 A/B） | J85 | **12 级两趟都铸到第 9 格才倒，而排水已经不是原因**（第 9 趟 `drain.0..7` 八次全「已排干」，J70 已验）。真因：`water8` 的水源浇在 **y=61**，落差五格把流体等级重置，到楼梯底仍是 `level=8`；它必须一直站到 `cast8` 浇完才被 `recover8` 收走 ⇒ 楼梯井在整个往返期间是淹的，起塔被冲下（`cast8#9.climb.0.washedOffFed`）。⚠️ 决定成败的是**站哪一柱**：装水选到了 `3,20` 并成功（`water8.raisedY=60/60`），浇岩浆 `cast8.raiseOffTheFlight=只有楼梯那一柱 2,20 验得过射线，别无选择 —— 抬升多半会被冲下来`，**它预言了自己的结局然后照做**。砍掉其它候选柱的是壁龛**外**的土（`射线停在 4,60,19 dirt`／`4,61,19`／`4,62,20`），而清障按设计只清壁龛内。⛔ 不许再加第五道补救腿——判词说的是候选集只剩一个。三态判据在 `ladder9-verdict.md` | 我 |
