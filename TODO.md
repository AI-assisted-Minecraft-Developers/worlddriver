...
|---|---|---|---|
| 🟠 待验 | Q15c | **集成服 NeoForge 上 PREP 的 ENV_FAIL（每趟 2–13 条）：StageWright 的停滞判据已改为把整个 level 的区块加载与变 ticking 也算作进度。** 待验：发布后集成服 NeoForge 闸跑两趟，ENV_FAIL 应为 0，`prep.ticks` 的长尾应落在收养真玩家的场景之后；仍有停滞就读那条 ENV_FAIL 尾部的采样栈 | 我 |
