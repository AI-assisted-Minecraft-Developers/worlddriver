...
11503 tick 和 14179 tick，两次独立通过（上一版是 8 浇 / 9 开）。修的三件事见 CHANGELOG：
背板补回来、爬升不再被自家的水吓退、起塔柱按射线选并钉住。

**但这是排练，不是攀爬**（`staging.calls=12`）：它说的是这一级自己的活儿站得住，
没说从底下十一级真爬上来之后还站得住。**真梯没跑过。** 下一步就是它。
...
```
drain.7 = -11, 56, 37 = water     drain.8 = -9, 57, 37 = water     drain.9 = -10, 57, 36 = water
```

十次 `recover` 全部 CONSUME，源块都收回来了，等了 200 tick 还是湿的。没查过。
它至少解释了两件事：`pinnedLost` 那次身体是从哪儿被冲下去的，
以及为什么 `standToPour` 老是要处理「身体浮起来一格」。
