...
- **删掉的东西一条都没丢，全在 git 里。** 重写前那一版的完整内容在提交
  `58ec568dae7d4577deab4061ef2eaa9e1367e105` 里，一条命令就能翻回全部历史读数：

  ```bash
  git show 58ec568dae7d4577deab4061ef2eaa9e1367e105:TODO.md
  ```
...
| ❌ 判：不做（仪器已武装，首现即判读） | Q16e | 6 级**间歇**失败（3 趟里 1 趟）。`1a4efeac` 的 `kill.kills`／`kill.swings`／`kill.preyVitals`（`WorldDriverJourneyScenes:1232-1245`，results-ladder14 起成行）已把「谁按了 keyAttack、打了几下、打死没有」按交战归因；此后 6 级连续 PASS。**重开条件**：6 级再次 FAIL **且** `kill.*` 三键在那一趟有行 ⇒ 当场按行归因；**再次 FAIL 而三键零行则是另一件事**（仪器没跑到），先查为什么，别改战斗 | 我 |
...
| ❌ 判：不做（关闭） | J4 | `keyAttack` 五取用者协议——只做诊断表，**不要发明全局仲裁器**。诊断表已被 `1a4efeac` 的三键仪器替代，6 级此后连续 PASS。**重开条件**：Q16e 下次发作且归因不清再重开 | janitor |
| ❌ 判：不做（闸已武装，首现即判读） | Q21 | 三次进近逐字相同：`liftInPlace` 的闸是 `y >= wantY`，而失败的是**柱**。闸已拆三支（`be58b518` 的 `.liftSkipped.N`／`.liftSideways.N`／`.liftIsHere.N`），在 j48 以来全部归档 results 与 j57 已写部分里 **0 行**。零样本上改重试逻辑没有判据可言。**三键常驻，首现即判** | 我 |
...
`d321eac4`（`JourneyFill.eyesFor` / `eyeBlocked`：身体已经站着的那一格问**真眼**，要走过去的格要求
**四角＋中心全都看得见**，外套严格→回退）曾落地，随后在一次 A/B 里被判为看门狗死因而撤回（`a191f87f`）。
后来那个 A/B 结论**自己也被撤回**（对照臂不等价于绿参照，差分没有解释力，真机制是 J65），
但原文写死：**`a191f87f` 的撤回照旧保留**（撤回一个没验证过的改动不需要理由）。
...
⚠️ 最小版**不需要预算闸**：`J60-C`（`946a77c2`）已经证明这一族可以「**换原点，不是加射线**」——
...
**凭什么改判**：`de3a4311` 那趟（两道禁挖闸都关掉之后，18011 tick FAIL）把水直接读成了死因：
...
- ~~上限是 **11/20**，12 级三趟排练都 FAIL，所以**押 12 级仍然不过**。~~
  **押错了，而且是好的那个方向**：2026-08-26 第 2 趟真梯 **13/20**（12 级 17295 tick 真过、13 级进下界），
  新前沿是 14 级。排练三趟全 FAIL **没有**预测到真梯会过——两者不是同一具身体的同一段历史，
  这一条以后不要再当预测用。**下一趟的上限按 13/20 押。**
...
#### 🏆 真梯第 2 趟（2026-08-26 02:0x→02:46）：**13/20，新纪录，零布景**

```
wd.journey09Iron      -> PASS ( 6262 tick)      ← 上一趟摔死的那一级
wd.journey11Obsidian  -> PASS (10120)
wd.journey12PortalLit -> PASS (17295)           ← 12 级首次在真梯上通过
...
2. 所以**动手之前先对 HEAD**：`grep` 它自己会写的那一行、读产码、量文件行数。
   ...
   今日一量是 **2819 行**，早就不超了。
3. 行号一律相对 `git show 58ec568dae7d4577deab4061ef2eaa9e1367e105:TODO.md`，
   **对当前文件无效**。
