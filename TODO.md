...
身体在 3,70,16`——身体 **y=70**。同趟 `cast8#2.climb.0…39`（**40 级 drift**，上一趟 `.climb` 零次），
　而塔的账自相矛盾：`gained=2/2 block(s)`（垒成）对 `verdict=没垒成 —— 落在 0`，
　`endedOn=脚格/脚下都是 air`（结束时悬空），`walkerFallback=true`。
　⚠️ **40** 正对上 [[jumping-and-momentum]] 的「腾空判『垒高了』把四十次拒绝读成四十次成功」。
　⇒ 下一笔查这里。**别动浇筑闸**——它这趟判得对（从 `3,70,16` 瞄 `5,60,19` 确实落进 `3,69,15`）。
