...
| 🔴 做（现在的下一笔） | J124e | **切短第 5 腿**，不是「换掉走不到的路点」——目标可达（一次 `reached=true steps=36 end=74,41,95`），死因是搜索代价：117 次里 103 次撞满 `maxNodes=600` 空手而回。⇒ 在 wp4 `72,42,85` 与 wp5 之间插中继点，让每跳落进 600 节点预算内；`74,41,95` 可以直接当新的 wp5（实测可站、实测到达过） | 我 |
| 🟡 判 | J124g | 那次成功的路**没被走过**（第 5 腿握过的最长路径 18 个节点）。搜索耗 3952 ms，起点 `65,43,78` 算出的路返回时身体已在 `69,43,84`。⚠️ 单样本，别当定论；要证伪就看下一趟有没有第二次 `reached=true` 而心跳里出现 `路=N/36` 量级的路 | 我 |
