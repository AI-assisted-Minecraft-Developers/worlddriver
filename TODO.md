...
## ⬜ 15 级：先走到扭曲森林再猎（已写，**未编译回测**）

上一次也是唯一一次执行报的是：

```
enderman.found = 0/6      hunt.biome = minecraft:nether_wastes
level.players = 1         gamerule.doMobSpawning = true
```

**这一行有三个不同的世界会印出来，它自己一个都分不清**，所以先加了 `census`
（48 格内 / 128 格内的末影人数，加 128 格内全部怪物的分类计数，并标明本级钉了几个区块）：

| 世界 | census 会长什么样 | 要的是 |
|---|---|---|
| 什么都不刷 | 128 格内怪物总数 0 | 开关 / 难度 / `level.players()` |
| 刷得挺欢，就是没末影人 | 怪物几十只，末影人 0 | **换生物群系** |
| 有末影人，但在 48 格外 | 近 0 远 >0 | 更大的搜索盒（原版 24–128 格刷怪） |

改了三件：
1. **走到扭曲森林**（`findClosestBiome3d`，半径 256）。`nether_wastes` 里末影人权重是
   ~170 分之 1，扭曲森林的怪物表基本只有末影人、一窝四只。**找不到 / 走不到都不算失败**，
   各报一行就地猎 —— 那句话说的是走位，不是这一级的成败。
2. **干等一轮只花一轮，不再花掉整级**。原来一次 1200 tick 没等到就直接进 verdict，
   于是「六轮」实际只等了一分钟，预算剩十一万八千 tick 没花。
3. 失败句子里**带上 census**，而不是只放在证据行里。

半径 256 是**行走预算写成的搜索半径**：14 级那条 399 格的路整条走失败过
（见上一节的三条 `goto`），比这更远的答案就算是真的也用不上。
