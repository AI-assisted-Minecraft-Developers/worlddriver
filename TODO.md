## 🔬 第 14 级：**唇沿税减半了钉数，也开出了一种新死法**（2026-08-21 两趟排练，未定案）

`-Prehearse=BLAZE_ROD` 两趟，只差一个自变量（`23b5f31c` 的唇沿税）：

```
              footing guard 行   进岩浆   段数    停在离要塞   死因
第 1 趟（无税）      37            0      9/40     255 格     连着 4 段没比纪录更近
第 2 趟（有税）      15            2      6/40     277 格     身体泡在岩浆里
```

**税做到了设计要做的事**——`footing guard` 37 → 15，这是 advisor 预写的确认信号，幅度不是噪声。
`STOP cause=` 两趟都是 0，所以税**没有**把搜索推到节点帽（那是预写的第二种反咬读法）。

**但它开出了一种此前不存在的死法。** 待验的机制猜想：`BotConfig.lavaDangerPenalty` 也是 **300**，
我给「可能掉进去的危险」标了和「正站在旁边的危险」一样的价，于是把本来让身体远离岩浆的
那道梯度**抹平了**。若成立，判据是「同一个数字站在等式两边」，修法是把唇沿税压到岩浆税之下
（危险的次序必须是：正站在旁边 > 可能掉进去 > 平地）。

**⚠️ 两趟各 n=1，落点距离在这套不可复现的系统里排不出名次**（见「真梯不可复现」）。
钉数是机制量、可信；进岩浆是 1 次事件、不可信。**定案前要先估这个死法的复发率，
而不是再跑一趟看落点。**
