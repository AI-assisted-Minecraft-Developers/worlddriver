...
| 🟠 判：做（族已定，2026-08-26 第 7 趟） | J80 | **12 级浇线上的水没人排**：`drain.0=壁龛已排干` 与浇线上 `3,59..61,20`／`2,59..60,20` 全是 `water(壁龛内)` **同趟并存**，第 6、7 趟同形（第 7 趟还同时带着壁龛外的 `1,60..62,20=dirt`，两族一起出现，不再是交替）。`drain.i` 只扫 `forgeCorridor` 走廊格，十二个框架格按设计 `left SOLID here`（`JourneyPortalRung:392`），**那句话从来没断言过浇线** ⇒ 排干的取样范围要盖住浇线 | 我 |
