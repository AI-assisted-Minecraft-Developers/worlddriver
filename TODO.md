...
留在账上的观察（这一条不因为不可归因而作废）：

| | parkour | step | unstuck | wiggle | 恢复跳 |
|---|---|---|---|---|---|
| run 9 | **6** | 16 | 0 | 3 | 15 |
| run 10 | **0** | 10 | 6 | 3 | 7 |

**分布换了形状**，不是同一分布的两次采样。janitor 认为该记 `e110fcb3` 的账（它是动 jump 的那一笔）；
**我读完 diff 之后认为相反**：`getJumpPower()` 在下界地狱岩上就是 0.42、冲刺推力手抄里本来就有、
`causeFoodExhaustion` 以 `abilities.invulnerable` 为闸而这具身体 `免伤=true`、两个仓库里
jump 事件监听器一个都没有——**`e110fcb3` 对这具身体的物理几乎是惰性的**。
而我的修法有一条真实路径通向它：改「哪些计划活下来」＝改「走哪条路线」＝改「路线里有没有 parkour 边」。
...
把 `consumed > 0` 临时改成 `false`（等价于关掉修法、保留全部仪表），跑同一趟排练。
**两臂只差这一个自变量，`e110fcb3` 在两臂里都在。** 比 git revert 干净，也不用给一个
我认为正确的修法加永久配置面。判据：wp4 在关掉修法的那一臂能不能过。

待办：① janitor 的双 loader（Fabric 先跑）——302 条场景是比一趟排练大得多的样本
② 上面那个单变量排练 ③ 给精确腿挂 `JourneyFlight`（文件已交还）。
