## ⬜ 12 级：垫脚已经落地，接下来是**站错了一排**和**再高两排要楼梯**

`Near(cell,2)` 这条门槛已经写成 `DIG_ARRIVE`，落脚点是选出来的不是猜的，
dy=0/1 直接走过去，dy=2 垫一块圆石。三次垫脚**都读了世界回读**（不是读返回值）：
`cell.4.step / wet.4.step / cell.5.step … → 站得住了（cobblestone）`。
...
高度对了（y=57），**排错了**：停在 z=36（靠竖井那一排）而不是 z=37（贴门框那一排），
2.24 还是 >2。这条读数是「回读身体最终位置」换来的 —— 不然它看起来就是「垫脚没用」。

往下查的三个方向，都还没试：

1. 垫脚只垫了 `-11,56,37` 一格，身体从 `-11,56,36` 跳上去时**落在了 36 排的哪里**？
   多半是它站上垫脚后又被 `Goal.Block` 的到达判定放走了。先看 `[walker]` 那一段。
2. 把 `-11,56,36` 也垫一格，让两排同高，身体就没有「站矮的那排」这个选项。
3. 落脚点直接要 `behind`（z=37, y 同 cell）而不是 `lower` —— 但那要两块，回到楼梯问题。

### ⬜ 再往上两排（dy=3/4，共四格）确实需要楼梯

已经会自己报了，带坐标带距离：

```
cell.6.noStand = -8, 59, 38 够不着：身体 -9, 56, 36 距 3.74 格（>2），… 这一格要的是楼梯不是一块砖
cell.8.noStand = -9, 60, 38 够不着：身体 -9, 56, 36 距 4.47 格（>2），…
wet.8.noStand  = -9, 61, 38 够不着：身体 -9, 58, 38 距 3.00 格（>2），…
```

**堆成一柱是墙，爬不上去。** 要的是沿 x 方向（壁龛有 5 格宽）铺的阶梯，
或者干脆换个思路：把壁龛的挖掘顺序改成**自上而下**，这样身体一路踩着还没挖的石头下来，
每一排都够得着 —— 代价是 `corridor()` 的顺序注释里写的那条「每一格都挨着已经挖开的空气」
要重新论证。

### 现状（三次排练一致）

`casts CONSUME = 8`，`opened = 9`，终点都是第 9 格 `-9,60,38` 的浇线被自己的水挡住。
垫脚这一版**没有回归**，也**还没有推进**格数 —— 推进要等上面两条里的一条。

## ⬜ 12 级：门框上半部分的格子，身体根本站不到够得着的地方

**这一条是量出来的，不是猜的**，两条读数就够：

```
dig.cell.5 = -11, 58, 38 仍是 granite：身体 -11, 56, 36（壁龛内），距 2.2m，canBreak=true，
             该站的壁龛格 -11, 58, 37=air，手上 cobblestone；
             六邻 down=obsidian(实心) up=granite(实心) north=air south/west/east=granite(实心)
```

- 身体已经在**壁龛内**了（挖进门框那个毛病已经修掉，见 CHANGELOG）。
- 但它在 **y=56 的地板上**，格子在 **y=58** —— `-11,56,36` 到 `-11,58,38` 是 **2.83 格**。
- `ServerWorldDriver.mine(BlockPos)` 的到达门槛是 `Goal.Near(cell, 2)`。**2.83 > 2，永远不算到达。**
- 该站的那一格 `-11,58,37` 是空气，但它**没有地板**：`-11,57,37` 也是壁龛，也是空的。

所以门框第二排往上的格子，壁龛里**没有一格**落在 `Near(cell,2)` 里。底排（dy=0）和第二排
（dy=1，1.41 格）没问题，dy>=2 的六格全靠运气 —— 有时候身体踩在已经浇好的黑曜石上够到了，
有时候够不到。这解释了为什么同一排的 cell 4 开了而 cell 5 没开。

### 要的是一级台阶，不是更长的等待

三条路，都还没试：

1. **在壁龛里垫一格站台**。身体带着圆石，`allowPlace` 在浇筑期是关的 —— 但 12 级已经有
   `placeInto`（补楼梯用的，走 `useItemOn`，不受 `allowPlace` 影响）。开挖上排格子之前先在
   `behind.below()` 垫一块，挖完由 `tidyTheAlcove` 收走（它本来就在清壁龛里的圆石）。
2. **把壁龛的挖掘顺序改成留台阶**：`corridor()` 现在是整层整层往上掏，掏完人就没处站了。
3. **不要用 `mine(BlockPos)`**，改成「站好 → `avatar` 直接敲」的写死步骤 —— 这一级已经这么
   做了好几处（`mineCellOrGiveUp` 之外的路）。风险是绕开了走位逻辑。

**先做 1**，它最便宜，而且 `tidyTheAlcove` 的清理逻辑已经在那儿了。

### ⬜ 排练里 12 级的终点仍然是「浇线上有自己的水」

最近三次排练（改动前 / 第一版 standBehind / 加了地板判据之后）分别停在
cast 9 / cell 5 / cast 9 —— 中间那次是**第一版 standBehind 自己造成的回归**，已经修掉，
记在 CHANGELOG 里，别再走一遍。

现在的终点：

```
浇不到指定格：想浇 -9, 60, 38，浇线上是 -9,59,37 / -9,60,37 / -9,61,37 = water(壁龛内)
wet.8.stillShut.1/2/3 = -9, 61, 38=dirt：这一格从头到尾没开过
```

注意 `wet.8` 也是「没开过」，和上面 `dig.cell.5` 是同一个毛病（第 8 格 dy=4，最高的一排）。
**这两条大概率是同一件事**，先修站位，再看浇线还堵不堵。
...
已修：在流体里 / 还在下坠的身体不再消耗重试次数（CHANGELOG）。
`surroundings` 现在还报 `onGround`、坠落距离、血量、脚下到实心的距离。

**还没查的**：为什么一条 399 格的路线会**在洞穴上方悬空结束**。
下一次跑 14 级，先看 `fortress.around.1` 里的 `脚下到实心`——
如果它一开始就是个大数，说明规划阶段就把身体送上了天花板，而不是走着走着掉下去的。
别猜，等这条读数。
