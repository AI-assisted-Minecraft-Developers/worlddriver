...
| 🔴 判：查（ladder-20 的死因，离第 14 级最近） | J124 | **下界寻路不避火，烧掉的血又没有任何东西补。** `hp.trace = 掉血 14 次、回血 0 次`，其中 `t21 −1.0→13.0 @71, 43, 70 脚下=netherrack 身处=fire 着火160t`；`death.blow = inFire −1.0→10.0@65；onFire ×6 一路到 4.0@173；mob（Piglin）−4.0→0.0@8`。**三个可分开的原因**：①~~路线从火里穿过去~~ ⛔ **已推翻，别照着修**：火**早就是 hazard**——`BotUtil.isHazardState:234` 一行 `if (s.is(BlockTags.FIRE)) return true;`，三个 `WorldView` 实现全走它，而 `WorldView.canStandAt` 明写 `if (!isPassable(foot) || isHazard(foot)) return false;` ⇒ 规划器**拒绝**把火当落脚格。写「不避火」是没 grep 就下的判词（[[a-guard-i-assumed-absent-was-running]] 第六次）。**读数换个读法**：`身处=fire` 全趟只出现一次且那一刻已 `着火160t`（香草进火当 tick 即设 8 秒），而**下一条 `t31 @71,43,69 身处=air 着火170t`——格号是空气、着火计数还在涨**，那是 `BaseFireBlock.entityInside` 按**碰撞盒**判的。⚠️ **但盒子只解释「烧着不灭」，不解释「怎么进去的」**：`身处` 印的是 `blockPosition`，**身体中心进了那一格**，0.3 格的盒沿蹭不出这个读数。⇒ 进去只有两族：**(a) 火在规划时就在** ⇒ 规划器拒绝它当节点 ⇒ 身体是**离开计划**进去的（切角，或被 `t12 −1.0 @70,43,69` 那记无名伤击退）；**(b) 火是规划之后才有的**——恶魂火球爆炸会**放置火方块**，所以「恶魂」不单列、它属于 (b)。⛔「恶魂直接点燃身体」已被读数否掉：首个着火标恰是 `160t`，即进火那 tick 的赋值。**判别式**：规划那一刻走廊里有没有那一格火——有 ⇒ (a)，没有 ⇒ (b)；(a) 内部再靠「掉血那 tick 的身体格 vs 当前路径节点」分切角与击退。⇒ 下一步是**加仪器不是加修法**。🟢 **走廊火格普查已落**：`JourneyFireCensus`（外接盒扫 `BlockTags.FIRE`，印**格子清单**不印「有/无」，`FIRE_PAD=1`），直行腿在 `rig.settle` **之前**写 `.fire`、绕行重问写 `.fireAgain`（两个盒重叠不重合，只有**同时落在两个盒里**的那格才被日期定性）。闸 `wd.netherCensusNamesTheFireInTheCorridorBox` 四条臂、双闸绿。⚠️ **盒不是路**，空手只等于「规划这一刻这一带没有火」。⇒ 还欠第二件仪器：**逐 tick 的身体格 vs 当前路径节点**，(a) 内部的切角与击退才分得开；②烧掉 7 血零回血——`death.food = 6/20`，`< 18` 按机制自然回血就不会发生；③4 血时 Piglin 一下 4.0，而 `journey.steer` 每级都印着「反射链一条都没武装…**所以受到攻击不会有任何抢占**」。⇒ 先量哪一个：①有现成判据（走过的格里 `身处=fire` 的 tick 数）。⭐ **②已经量了，不是「没调用吃」，是包里一块肉都没有**：第 6 级猎一头牛得生肉 ×5（`rung.FOOD`），第 10 级为清砂砾**一口不剩地吃光**（`gravel.feed.bite0…bite4`，饱食 7→20，`bite4.trace` 末尾「手里=minecraft:air`」），中间到下界没有任何补给 ⇒ 第 13 级已经是 `饱食 9/20 ⚠️ <18` 且无粮，**这一梯在需要它的四级之前把全部食物花光了**。所以②的修法不是「什么时候吃」而是「哪来的」（第 8 级有熔炉，顺手烤是一条；再猎一次是另一条）。⭐ **第四堵墙，同一屏证据里，别等 ladder-21 把它当新发现**：`fortress.waypointAudit = 十八个烘入路点在全新世界里的样子：本格空的 14 个，其中 6 个脚下也是空的……那些是那一趟自己摆出来的石头，这个世界里没有——瞄准它们的段无论寻路怎么改都走不到`。⇒ **火与吃饭修完，下一堵是路点表本身**。⚠️ `death.leg` 自己写着死因链跨了腿（`写它时是本段第 173 tick；死在本段第 8 tick`），读 `hp.trace` 要连着它读。⛔ 别一上来就武装反射链——那是补引擎能力 | 我 |
