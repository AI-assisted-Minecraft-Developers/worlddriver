...
| 🟠 判：做（等第 11 趟验 `finishTheBite`，再决定挂不挂别的级） | J84 | **饱食从 6 级起就低于自然回血线 18**（第 11 趟逐级：20/20/20/20/16/11/6/4），而全梯唯一的进食调用点是 `WorldDriverJourneyScenes:2112`（10 级砾石那一步）⇒ 7、8、9 级和 11、12 级都没有进食机会，6 级猎到的生肉 ×5 一路带到顶上才吃。⛔ 在那一口被证明真能喂饱之前，不要去别的级挂第二条同样吃不下去的腿。判据 `feed-finish-criteria.md` | 我 |
...
| 🟠 判：先补测量（`raiseVeto` 已在 HEAD；第 11 趟**没到门框**就死了，判据顺延到下一趟到达的跑） | J85 | **12 级连三趟都倒在第 9 格**（第 7/9/10 趟；那是全环最高一格，壁龛地板 y=56 而它在 y=61）。第 9 趟是塔被自己浇的水源冲下，第 10 趟是三条抬升腿都够到了高度、**都停在别的柱**（`water8.raisedY=60/60（停在 -1,20，指定柱 3,20）`）⇒ 不是同一族，⛔ 别按单趟认因。⛔ 不许加第五道补救腿。三态判据在 `ladder9-verdict.md` | 我 |
| 🔍 判：查（等排练 #3 的落点读数） | J86 | **解卡跳的闸有一环宽的洞**：`Walker.wiggleHop` 用 `ring <= WalkerGeometry.HOP_RANGE(2)`，弧长实测 3.47 格。两侧都在排练里复现：ring 2 连压 16+ 次卡成确定性循环，ring 3 放行、第 11 趟从这里进的岩浆湖。⛔ 不许加宽半径（只是把洞挪成停滞）。⛔ 也不许照 `hopBearingRow` 那份「92% 背着」立闸——那行印的是**相机**，起跳推力沿 `driveTargetYaw`，两通道设计上解耦。现在候选改法是**落点规则**（沿驱动角 2/3/4 格取柱），仪器已在 HEAD，判据 `rehearse3-landing-criteria.md` | 我 |
| 🟠 判：做（跟 J86 同一笔落地，⛔ 不许只修一条） | J87 | `WalkerTickRepath:197` 的 `unstuck` 突进是**第二个**消费 `lethalDropWithinHopRange` 的地方，同一个洞开在那扇门上；它有现成的 `wk.unstuck.burstYaw` 可以直接喂给落点规则 | 我 |
| 🟠 判：做（额度，挡住 J86 的改法） | J88 | `Walker.java` **2999/3000**，只剩一行；落点闸必须整个写在 `WalkerGeometry` 里，否则先得给 `Walker` 腾地方。⛔ 刮注释不算腾地方 | 我 |
...
| ❌ 判：不做（重开＝`JourneyRig` ≥ 2900 行） | J42 | `JourneyRig` 的门面缝是真的且干净：`1856–2410` 这一带 27 个方法里 **20 个是纯查询**，不纯的 7 个还**连续**（收集那一族 `2024–2178`）。代价 **192 个调用点、13 个文件**，收益 **0**。重开条件从「逼近 3000」改成数字：`JourneyRig.java` 行数 **≥ 2900**。⚠️ **2026-08-26 晚复核已经是 2876**，离触发只剩 **24 行**（本行先后写过 2606／2745，都已过期）。到那一刻 `1856–2023` + `2179–2410` 这约 400 行就是现成的搬运单，而 `2024–2178` **必须留下**（它要 `settle`）。顺带记着 `JourneyRig:1297-1312` 那处孤儿 javadoc（写的是 `heartbeat`，挂在 `sinceHeartbeat` 上），切缝那一笔顺手改 | janitor 查，topology 决定 |
