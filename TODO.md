...
　⛔ **根因是重来无效，不是闸缺失**：`JourneyPour:189` 的 `raiseRowTooHigh` **开火了**，
　但 `:196` 的重来回调是 `raiseTo(…, attempt+1, …)`——同一目标、同一条 `walkToColumn`
　（`:164`，只给 XZ）。身体确实被带回 `1,57,19`（`returnedY=57`、`landing=1.83/57.00/19.52`
　已验），再问一次答案还是 `3,64,20`（`arrivedY=64，起 57，净升 7，脚下=grass_block`；
　`gotoEnd=预算用完时进程还在走`，判到达用「距 2,20 一格、容差 5」）。
　⇒ [[a-retry-that-changes-nothing]]：重来必须改掉一个变量（带 Y 的目标／限高），
　否则按定义无效。`:161-163` 的注释已自承「`Goal.XZ` … it ignores Y」。
