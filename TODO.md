...
1. **量 parkour 的落点误差**：起跳格、目标格、实际落点、水平过冲量，逐次记。
   预登记里那次是 z 上冲 3 格；这次是从角上起跳直接失去落点。**两者是不是同一个数在变，要有数据。**
2. **问一个更前面的问题**：身体为什么会站在一个自己脚下是 air 的格子上（接触 0.15/0.36）？
   `strideFloorGuard` 的阈值是 0.18，**0.1532 < 0.18 本该被钉住**——
   查它在起跳那一 tick 有没有被跳过。
3. 前两条有读数之后再谈修法。
