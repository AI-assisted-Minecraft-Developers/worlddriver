...
| 状态 | # | 事 | 归属 |
| 🔴 判：先补测量（⛔ 读数到手前不许动 walker） | J114 | **关卡说合格的柱，walker 三次都只走到它的邻格。** ladder-16 第 11 级：`shaft.stepEnd.*` 三条腿净挪 **8／0／6 格**、全部停在**距目标 1 格**、且**没有一条报到达**（全是 `end=预算用完时进程还在走`，`Goal.XZ` 容差本来就是 0）。⇒ 不是判据太松，是走位走不完最后一格。领先假设：关卡问的是 `whyNotDiggable`／`columnIsSafeToSink`（答「合格」），walker 问的是 `canStandAt`（可能答「站不得」）——**同一个「身体能不能在这」问了两个谓词，选柱只问了其中一个**（J110 的具体发作现场）。**缺的读数＝`canStandAt(选定柱)` 及其各子句的值**，印值不印谓词 | 我 |
...
| 🔴 判：查（⛔ 仪器先行，end-reason 行存在之前不许动 walker） | J109 | 真梯 ladder-15 断在第 11 级下井阶段：选定柱 `-10,51` 本身**合格**（`whyNotDiggable=null`），身体三次都没站上去——`stepping.2/3` 起点逐字相同 `-10,66,50` 零位移（钉住），back-off 后那腿走了 8 格停在**差 1 格**（判据错配）。**这是两个方向相反的子失败**，而 `shaft.stepping.*` 不印 end reason 所以分不开。⛔ 对照：同趟 `lava.gotoEnd.1` 距目标 **0 格**，所以「walker 做不到精确格」是伪结论。第一步：把 `lava.gotoEnd` 的格式复制给每条 stepping 腿（end + 传入容差 + 本腿判据，并排一行） | 我 |
