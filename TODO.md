...
| 🔴 判：查（ladder-20 的死因，离第 14 级最近） | J124 | **走廊在第 5 条腿断掉**：死在 `72,43,84`，`death.leg = fortress.wp5`，目标 `74,41,97` 被审计判「本格空且脚下也空」。⇒ 要么重测一条落在实体地形上的路线，要么让 wp5 起的这几条显式是造桥腿、有自己的预算与守卫。⛔ 别把「这几个路点该不该存在」当新题——`FORTRESS_WAYPOINTS` 的表注释 08-22 起就写着；新的只有「死亡坐标与第一条断腿对上了」。火/饱食/Piglin 都是这条腿上的遭遇。理由与判别式在 `JourneyFireCensus` javadoc、`FORTRESS_WAYPOINTS` 表注释、CHANGELOG，不复制到这里 | 我 |
...
| 🔴 做 | J124b | 食物是**供给**不是时机：13 点饱食要撑 20 级，梯子没有熔炼步骤。第 8 级有熔炉，顺手烤是一条；再猎一次是另一条 | 我 |
