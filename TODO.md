...
| 🟠 判：做（先量后修，量归 janitor） | J81 | **场景之间不复位 `BotConfig`，所以一个场景开局的开关值由跑在它前面那个场景决定。** testmod 里 `allowPlace` 有 **129 处**运行期写入、只有 5 处存旧值；`BotConfig.pinnedBaseline()`／`snapshotAll`／`restoreAll` 造好了却 **零调用点**，而它自己的 javadoc 点名的正是让 `descentYawArena` 飘了六十天的那次泄漏。journey 的 10 级就在 `gravelForFlint` 里翻 `allowPlace=false` 再在回调里设回——那一级 FAIL 时回调跑不到。⛔ 别把 124 当成 124 个缺陷，先按清单顺序排出真正泄漏的分母 | janitor |
| 🔍 判：查（两趟已实证，等第三趟定族） | J80 | **12 级浇线上的水没人排**：第 6 趟浇不到 `4,60,20`，浇线上 `3,59..61,20`／`2,59..60,20` 全是 `water(壁龛内)`，身体在 `10,64,18`（水平 6 格、高 4 格）。`drain.i=壁龛已排干` 只扫 `forgeCorridor` 走廊格，十二个框架格按设计 `left SOLID here`（`JourneyPortalRung:392`），**那句话从来没断言过浇线**。第 3 趟同一级挡路的却是壁龛**外**的土 ⇒ 两族交替，先分清哪族更常见 | 我 |
