...
| 🔍 判：查（第 11 趟 12 级死因，排练已复现两侧） | J86 | **解卡跳的闸有一环宽的洞**：`Walker.wiggleHop` 用 `ring <= WalkerGeometry.HOP_RANGE(2)`，而这道跳自称弧长 ~3 格。同一趟排练里两侧都出：ring 2 连压 16+ 次、身体在装料点旁卡成确定性循环（同坐标每 30 秒一轮）；ring 3 放行，第 11 趟正是从这里进的岩浆湖。⛔ 不许简单加宽半径（那只是把洞挪成停滞）——方向是给这道无朝向的跳加朝向。判据 `rehearse-lava-criteria.md` | 我 |
