...
| 📏 已量待判 | Q7 | V1 冻屏：**病因第二次改判**，见下方「冻屏的真实形状」。修法要按量出来的分布选，不按最坏值选 | 我 |
| ⏭ 排队 | Q8 | V2 不低头：挖掘瞄准必须**经过** `LookController`，不是绕过它写 `setXRot` | 我 |
| 📏 已量待判 | Q9 | V6：清点完了，**没有一处是"瞬移"**——10 处是同格居中吸附，见下。真正的位移是开场那 7.9 格，已有读数 | 我 |
| 📏 已量待判 | Q10 | V5 工作台/掉落物：**证据行早就有**，量出来是「四个合成级丢了一次」。两种互斥解释未分辨，见下 | 我 |
