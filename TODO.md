...
### V1 挖一块冻一下：渲染线程上的同步 A\*

952 次搜索**全部**在 `Render thread` 上跑完，中位 1ms，**p90 309ms / p99 465ms / 最慢 1107ms**，
超 100ms 者 43 次。299 次的 owner 是 `mine`：**破一块 → 世界变 → 立刻重搜**。
放大它的是 `JourneyRig.generousPathfinding()` 的 `pathfinderMaxMs = Long.MAX_VALUE / 2`
（墙钟上限被整个去掉）。那两行上面的注释断言「`STOP cause=` 出现零次，这没买到任何可测量的东西」——
**本趟出现 47 次，那句话已经过期**，它是在无头专用服上写的，那里没有画面可冻。
→ 修：给真梯一个真实的 ms 上限；顺手改掉那段已失效的断言。
