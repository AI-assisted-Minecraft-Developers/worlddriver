...
| 🔴 判：查（12 级现在的死因，第 12 趟到场） | J91 | **身体下不去壁龛**，一趟里两次：`cast8.return` 第 2/3 段该到 `0,58,20` 停在 `0,65,22`（高 7 排）；`cast8.lift` 该到 `2,56,22` 停在 `2,64,20`（高 8 排）。于是三次浇筑全从身体当时所在地乱浇。排练同一条 `return` 的 `returnedY=57`（下去了） | 我 |
| 🟠 判：等下一趟真梯的读数（仪器已落） | J92 | 12 级的走路腿路程/净位移中位数 **4.14**（排练 1.28/1.41），但排练腿只有 2–3 格、**对长腿是瞎的**；而 `cast8.rimTax=767、半径 12` 给了良性解释（绕坑 ≈75 格）。`14048fc7` 已让心跳印 `目标=… 路=N/M`：**N 单调爬向 M = 绕远路，反复重置 = 计划在被重算**。⛔ 在读到这个字段之前不许判 | 我 |
