## ⬜ 12 级：**九浇十开**，下一刀在最后一格的水位

2026-08-13 排练（8943 tick）：`casts CONSUME = 9`，`opened = 10`（上一版是 8 / 9）。
推进这一格的是两条**都能单独读出来**的修法，见 CHANGELOG：背板补回来了
（`cast8.backingMend … → 补回来了（cobblestone）` → `cast8.result=CONSUME`），
爬升不再被自家的水吓退（`exit.gained=3/3`、`water9.raisedY=60/60`，上一版是 `1/2`、`58/59`）。

### ⬜ 下一刀：第 10 格的水位浇不进去，因为塔垒歪了

```
water9.raise      = -9, 57, 37 → y=60（在 -10,37 这一柱上垒台阶）
water9.raisedY    = 60/60                     ← 高度到了
climb.3.drift     = -7, 58, 37 偏离起塔柱 -9,37，先走回去再垒
climb.3.driftKept = -8, 58, 37 走不回 -9,37，改以这一柱为准
water9.stand      = -8, 60, 37 瞄 -10, 61, 39
water9.picks      = -8, 61, 38 dirt face=north → 落进 -8, 61, 37（身体 -7, 60, 37）
```

**选的柱子（x=-10）是对的**：从 `-10,60,37` 或 `-9,60,37` 出发，射线擦着已经开好的
`-9,61,38` 过去，正落在 `-10,61,39` 的北面 —— 手算过。丢掉这一格的是**塔垒到了别处**：
`walkToColumn` 差 1 格（`raiseTo.arrivedDistance=1`），`TowerProcess` 又漂了 2 格，
`driftKept` 于是把 x=-8 认成了新柱。身体高度对、位置错，而壁龛里 y=60 这一层
**只有这根漂过去的柱子有地板**，所以 `standToPour` 选的落脚格根本走不到。

三次 approach 全都从 `-7,60,37` 发射同一条射线 —— 这就是本文件自己写过的
「a retry that changes nothing」。往下查的方向（都没试）：

1. 让 `climbOut` 认死 `standLevelWith` 指定的那一柱，`driftKept` 的「改以这一柱为准」
   对**浇筑**来说是错的策略：换柱子就是换几何。
2. 或者接受漂移，但让 pour 的重试**排除已经走不到的落脚格**，第二次选下一个。
   （注意：这一轮 y=60 层没有第二个有地板的格子，所以单靠这条大概不够。）
3. 或者垫两块把身体从 `-7,60,37` 桥到 `-9,60,37`。
