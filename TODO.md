...
| 🔴 判：查（12 级排练最常见的死法，**确定性**） | J93 | `浇不到指定格：想浇 4, 57, 19（瞄 5, 57, 19），射线会把流体放进 3, 58, 19，身体在 BlockPos{x=2, y=57, z=20}` —— #6 与 #8 **逐字相同**（连身体坐标），⇒ 不是随机是确定性。8 趟里 2 趟死于此 | 我 |
| 🔴 判：查（仪器已落，等读数） | J94 | 点不着：`frame.obsidian=10/10`、`portal.slag=门洞六格都是空气`、`light.cellAfter=fire`、`portal.cells=0/6`。#2 与 #7 在七个点火键上**逐字相同**，与 PASS 只差最后两个。真因看不见，因为两个上游读数都在 `clearTheDoorway` 里取、而 `strike()` 随后要走最多 1500 tick，且 `portal.cells` 只印计数不印内容。`4d29b247` 加了 `portal.cellsNow`／`frame.obsidianNow`（贴着打火重读）。判据 `rehearse8-doorway-criteria.md` | 我 |
