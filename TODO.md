...
| 🔴 判：查（ladder-20 的死因，离第 14 级最近） | J124 | **下界寻路不避火，烧掉的血又没有任何东西补。** `hp.trace = 掉血 14 次、回血 0 次`，其中 `t21 −1.0→13.0 @71, 43, 70 脚下=netherrack 身处=fire 着火160t`；`death.blow = inFire −1.0→10.0@65；onFire ×6 一路到 4.0@173；mob（Piglin）−4.0→0.0@8`。**三个可分开的原因**：①路线从火里穿过去（`wp3` 轨迹经 `71,43,70`）；②烧掉 7 血零回血——`death.food = 6/20`，`< 18` 按机制自然回血就不会发生；③4 血时 Piglin 一下 4.0，而 `journey.steer` 每级都印着「反射链一条都没武装…**所以受到攻击不会有任何抢占**」。⇒ 先量哪一个：①有现成判据（走过的格里 `身处=fire` 的 tick 数）。⭐ **②已经量了，不是「没调用吃」，是包里一块肉都没有**：第 6 级猎一头牛得生肉 ×5（`rung.FOOD`），第 10 级为清砂砾**一口不剩地吃光**（`gravel.feed.bite0…bite4`，饱食 7→20，`bite4.trace` 末尾「手里=minecraft:air`」），中间到下界没有任何补给 ⇒ 第 13 级已经是 `饱食 9/20 ⚠️ <18` 且无粮，**这一梯在需要它的四级之前把全部食物花光了**。所以②的修法不是「什么时候吃」而是「哪来的」（第 8 级有熔炉，顺手烤是一条；再猎一次是另一条）。⭐ **第四堵墙，同一屏证据里，别等 ladder-21 把它当新发现**：`fortress.waypointAudit = 十八个烘入路点在全新世界里的样子：本格空的 14 个，其中 6 个脚下也是空的……那些是那一趟自己摆出来的石头，这个世界里没有——瞄准它们的段无论寻路怎么改都走不到`。⇒ **火与吃饭修完，下一堵是路点表本身**。⚠️ `death.leg` 自己写着死因链跨了腿（`写它时是本段第 173 tick；死在本段第 8 tick`），读 `hp.trace` 要连着它读。⛔ 别一上来就武装反射链——那是补引擎能力 | 我 |
