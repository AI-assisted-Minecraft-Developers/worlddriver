...
20 级那趟的 `[walker] t=… p=(x,y,z) … stuck=N totStuck=M` 逐 tick 都有。读 **`@116`**（123 往前 7）那一行：

| 读到 | 结论 |
|---|---|
| `stuck` 落在 (10,18) | **`wiggle` 坐实**，(u1) 成立，课题变成「平台上为什么会卡」 |
| `stuck ≤ 10` | 我这条排除有漏，回来重排 —— 先别改步进 |
| 那一行的 `p=` 是平台正中(x≈100) | **(u2) 是半径不够**（radius 2 vs 弧长 3） |
| `p=` 已经在 x≤99 | 半径够了却没拦住 ⇒ 病在 `lethalDropWithinHopRange` 的**扫描本身**（虚空为什么没被判致命） |
