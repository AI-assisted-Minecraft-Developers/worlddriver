...
| 🔴 **挡路** | Q22 | 12 级下楼那一段在地表打转（同一目标 `-4,62,20`，起点在 y=65~67 换了十几格）→ 掉进岩浆湖 `-10,63,19` → 在岩浆里重搜 42 次烧死。**这一级反射链一条都没武装**，泡在岩浆里没有任何抢占 | 我 |
| ❓ 待拍板 | Q23 | 真梯这一级要不要武装 `autoHeal` / 逃生反射？还是让寻路根本不踏进源块？前者破坏「零布景」的纯度，后者是产品代码要单独发闸 | advisor |
...
所以泡在岩浆里**没有任何东西会抢占**。这不是缺陷报告，是**下一步要先问的那个设计问题**：
真梯要不要给这一级武装 `autoHeal` / 逃生反射，还是要让寻路根本不踏进源块。
