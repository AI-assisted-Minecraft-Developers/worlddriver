...
| 🟠 判：先补测量（收源那笔等点名，2026-08-25） | J52 | **模腔的水顺楼梯流到底。** 仪器半已在 HEAD（`DRAIN_UPSTREAM=8`，`JourneyDrain:102`），**2026-08-26 第 7 趟拿到第一条读数**：`drain.7.upstream=周围 8 格内没有水源块` ⇒ 是纯退水，不是「有源在喂」，所以「收走被点名的源」这条修法**在这一趟没有对象**（真链条见 J85）。⚠️ 别把 J70 排进这条因果（实测趟内的水是 `water0` 自浇、`JourneyStairs:250` 收水排在下降之后）。⚠️ 12 级现在**同时**挡在浇筑那一族和这摊水上：`cast8#10` 里它一口气做了五件事——淹掉楼梯底（`stairFoot`）、用 `wouldOpenFluid` 掐断**两次**起塔、让垫台阶的 `placeOn` 拒绝、占住要清的那几格、最后把身体泡在水里 | 我 |
...
| ✅ 已落，等下一趟验（占位到读数为止） | J70 | 排水那一腿从 `HoldStill(DRAIN_TICKS / 2)` 改成 `HoldStill(DRAIN_TICKS)`，判词印的 200 从此就是等的 200。⚠️ 原行写着「与 12 级回程死因链无关」——**那句是错的**，第 7 趟的 `drain.7` 超时正是那条链的起点（见 J85）。**验收**：下一趟 12 级 `drain.7` 是否翻成「已排干」 | 我 |
...
| 🟠 判：先补测量（第 7 趟 12 级的直接死因） | J83 | **抬一行的那条腿净升 8 格，四道补救都没把它带回来。** `cast7.landing=1.78/57.00/20.50` 落对了，`cast7.raiseTo.arrivedY=65（起 57，净升 8），脚下=vine`；`raiseRowTooHigh` → `raiseRowRetry.returnedY=57` → `raiseTo3D` → `ramp.standShort=没走到`，最终判词身体仍在 `0,65,20` 而目标在 y=60。⇒ 与 `Goal.XZ 丢 y` 同族，但**那族的解法（三维目标）已经用上了还是没成** ⇒ 先量「四道补救各自把身体放在哪一柱」，别再加第五道 | 我 |
| 🟠 判：做（第 7 趟量出来的，与 J79 同一条腿） | J84 | **12 级整级饱食 0/20，血 18.0→9.0**（`body.vitals`）。饱食 0 持续掉血、≤6 不能疾跑，而 11、12 两级各一万五千 tick。`JourneyFeed` 只接在 10 级砾石那一步 ⇒ 11、12 级没有任何进食机会。⛔ 修法要等 `Bite` 先说出 10 级那一口为什么没喂饱（判据 `bite-trace-criteria.md`），**不要先去 11/12 级挂第二条同样吃不下去的腿** | 我 |
| 🔍 判：查（顺带发现，今天无害） | J82 | `JourneyShaft.climbFrom:210` 无条件 `allowPlace = true`，于是**成功的爬出一直在替降井做复原**——三个降井级的正确性挂在一个不相干 helper 的副作用上 | janitor |
| 🟠 判：先补测量（因果链已换，见 J85） | J80 | **原判「浇线上的水没人排」的取样根据不成立**：`drain.N` 用 `firstFluid` 只报**第一格**，所以它没说过浇线那几格干不干，`2,60,20`／`3,60,20` 是不是 `forgeCorridor` 成员**还没查**。第 7 趟测到的真链条在 J85。⇒ 下一步只有一件事：grep `forgeCorridor` 的成员集，看浇线那几格在不在里面 | 我 |
| 🟠 判：做（第 7 趟测全了，J70 已落一半） | J85 | **12 级铸了 8 格才倒**：`water0..7`／`cast0..7`／`recover0..7` 全 SUCCESS，`drain.0..6` 全「已排干」；`drain.7` 超时且 `upstream=周围 8 格内没有水源块`（J52 那个零读数的键**第一次有读数**）⇒ 纯退水。这一级把超时当放行，把第 9 个水源浇到**更高的一排** `4,61,20`，流下来的水把楼梯底钉死（`cast8#12.climb.0.washedOffFed=上游有源在喂 —— 它不会自己退`），身体再也没回到浇排。**已落**：`JourneyDrain` 那一腿从 `DRAIN_TICKS/2` 改回 `DRAIN_TICKS`（J70）。**还开着**：`drain` 超时之后照旧浇下一排，这一步要不要闸 —— 等下一趟看 200 tick 够不够再判 | 我 |
| 🟠 判：做（真梯主线，第 5 趟 9/20 的死因） | J79 | **一级的伤在下一级致命，而两级各自都「正常」**：9 级挖铁摔三次（−4/−7/−3，三行全是 `身处=air`）血剩 **4.0**，9 级判据只问铁锭、照样 REACHED；10 级第一 tick `hpNow<=MINE_HP_CRITICAL=4` 立即 abort，`broke 0/64` ⇒ 砾石 0 ⇒ 燧石 0 ⇒ FAIL。整趟 `hp.trace` 两行都写 **回血 0 次**，身体带着 5 生肉和一台熔炉从没吃过。⛔ 别动 `MINE_HP_CRITICAL`（它正在阻止身体挖死）、⛔ 别翻 `autoHeal`／`autoEat` 全局开关（会给二十级同时引入抢占，下一趟任何变化都归不了因）。修法是一条显式的腿。**腿已在 HEAD（`JourneyFeed`），而它吃不下去**：第 7 趟 `饱食 8→8`，启动与结束都发生了、中间没有；`JourneyFeed.Bite` 已补，判据在 `bite-trace-criteria.md` | 我 |
