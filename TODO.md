...
| 🟡 修法已编译，欠回测 | J93 | `浇不到 4,57,19`，13 趟占 4（#6#8#9 + noaim2）。真因：`finishTheFlight` 的 `down()` 判格号不判身体，脚一进末路排就跳过「等落地」，浇筑于是在身体下坠 0.92 格的途中决定瞄准、又低 0.69 格开火。修法 `JourneyStairwell` 改判位置（`SETTLED_SLACK`）。⛔ 「回落无视守卫」（`at = settled != null ? settled : planned`）是**另一条真缺陷**，下一轮单独落——同时落会让本轮主测量 `picks.3` 消失。判据 `j93-fix-preregistration.md` | 我 |
