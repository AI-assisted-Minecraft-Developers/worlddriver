...
⬜ 待答（只读追查中）：parkour 一族的前置条件**有没有建模助跑／动能**；虚空缺口上
`bridgePlace` 是**没被生成**还是**更贵**；执行侧有没有「速度不够就不要起跳」的闸；
以及 `sprinting` 为什么在滞空第一 tick 掉。
⚠️ `到得了目标=false` ⇒ 这是 **best-effort 局部路径**，解读"A\* 选了 parkour"时要带上这一条。
