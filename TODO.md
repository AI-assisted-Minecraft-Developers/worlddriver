...
| 🟡 做 | J124d | 读数器把火判在了错的字段上：`hp.trace` 的 `身处=fire` 只在身体**中心**进火时为真，而 ladder-21 四记 `inFire` 全是 `身处=air`（中心在空气、碰撞盒在火里、没被点着）⇒ 权威信号是 `death.blow` 的**伤害类型**。⇒ 下一版 `read21.py` 以类型为准、拿 `hp.trace` 配坐标 | 我 |
