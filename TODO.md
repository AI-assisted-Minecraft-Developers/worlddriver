...
| 🟡 判：改（数已量全，只欠实施与预登记） | J105 | 被盖住且横向无路时**开火太晚**：实测破一格土 379 tick（`wd.drownEscapeClientBreaksTheLidWhenOpenWaterIsWalledOff`），而 `drownEscapeAirThreshold=100` 开火后只剩约 290 tick（气 100 + 19 血 ÷ 每 20 tick 2 血）。抬到闲时哨兵的 240 可得约 440 tick。⛔ 只许在 `cappedColumn` 为真且横向无路时抬——类 javadoc 明写 100 是为了不抢占有计划的下潜 | 我 |
| 🔴 判：查（⛔ 别动那几道瞄准守卫，它们是对的） | J104 | 真梯 ladder-14 断在第 11 级：`cast.walk` 停在 3,60,60，距计划的浇筑站位 3,62,60 差 2 格，`end=unavailable/预算用完时进程还在走`。从那个站位射线只打得到 south/east 面，流体落进 2,61,60 而不是 2,62,59。**缺的是「站位没走到」这一步自己报失败，以及走不到时改挑一个看得见 UP 面的站位** | 我 |
