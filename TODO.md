...
| 🔴 判：查（ladder-20 的死因，离第 14 级最近） | J124 | **下界寻路不避火，烧掉的血又没有任何东西补。** `hp.trace = 掉血 14 次、回血 0 次`，其中 `t21 −1.0→13.0 @71, 43, 70 脚下=netherrack 身处=fire 着火160t`；`death.blow = inFire −1.0→10.0@65；onFire ×6 一路到 4.0@173；mob（Piglin）−4.0→0.0@8`。**三个可分开的原因**：①路线从火里穿过去（`wp3` 轨迹经 `71,43,70`）；②烧掉 7 血零回血——`death.food = 6/20`，`< 18` 按机制自然回血就不会发生；③4 血时 Piglin 一下 4.0，而 `journey.steer` 每级都印着「反射链一条都没武装…**所以受到攻击不会有任何抢占**」。⇒ 先量哪一个：①有现成判据（走过的格里 `身处=fire` 的 tick 数），②③要先问「这一级本来该不该吃东西」。⛔ 别一上来就武装反射链——那是补引擎能力 | 我 |
