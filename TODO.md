...
| 🟡 修法留下（天花板 y=63 以上按 `LIP_TAX` 加价）。机制已达成：出井抬升 对照 2/4 → 税后 1/14。**还开着的只有一件——跑真梯，读浇筑那一刻身体的 y** | J97 | 抬升腿把身体送出井口站到地表，随后隔着 9.6 m 收水失手。⛔ fl1 死的是 `water9.lift`（另一个调用点，同形），先读调用图再落。行为变更记在 `CHANGELOG.md`，判据 `j97-raise-leaves-the-shaft.md` | 我 |
